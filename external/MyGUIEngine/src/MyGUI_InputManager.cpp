...
namespace MyGUI
{
		...
		if (isFocusMouse())
		{
			...
			if (nullptr != mWidgetMouseFocus)
			{
				if (MouseButton::Left == _id)
				{
					...
					{
#if 0
						// 11_02_2023 Lax: Removed by Lukas because it causes, that one click makes 2 clicks and this causes crash in a lot of situations, e.g. A state is loaded twice!
#endif
	...
	void InputManager::setMouseFocusWidget(Widget* _widget)
	{
		...
		// устанавливаем новый
		if (_widget && _widget->getNeedMouseFocus())
		{
			_widget->_riseMouseSetFocus(mWidgetKeyFocus);
		}
