...
namespace MyGUI
{
	...
	void TextBox::setTextOffset(const IntPoint& _value)
	{
		if (getSubWidgetText() != nullptr)
			getSubWidgetText()->setTextOffset(_value);
	}

	IntPoint TextBox::getTextOffset()
	{
		if (getSubWidgetText() != nullptr)
			return getSubWidgetText()->getTextOffset();
		return IntPoint();
	}
