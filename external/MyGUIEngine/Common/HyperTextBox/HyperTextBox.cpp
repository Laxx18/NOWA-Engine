...
namespace
{
	Ogre::String removeAll(Ogre::String str, const Ogre::String& from, int count)
	{
		size_t startPos = 0;
		while ((startPos = str.find(from, startPos)) != Ogre::String::npos)
		{
			str = str.erase(startPos, count + 1);
			startPos += count;
		}
		return str;
	}
...
namespace MyGUI
{
	...
	HyperTextBox::HyperTextBox() :
		...
		mBreakHeight(0),
		mUrlColour(MyGUI::Colour::Black)
	{
		if (true == mTextSelectable)
			mTextSkin = "EditBoxEmpty";
	}
	...
	void HyperTextBox::parseText(Widget* _parent, const std::string& _value)
	{
		...
		else
		{
			...
			for (VectorString::const_iterator item = result.begin(); item != result.end(); ++ item)
			{
				...
				if (false == mTextSelectable)
				{
					text = _parent->createWidget<TextBox>(mTextSkin, IntCoord(0, 0, defaultSize, defaultSize), Align::Default);
				}
				else
				{
					text = _parent->createWidget<EditBox>(mTextSkin, IntCoord(0, 0, defaultSize, defaultSize), Align::Default);
					text->castType<EditBox>()->setEditStatic(false);
					// text->castType<EditBox>()->setEditReadOnly(false);
					text->eventMouseButtonReleased += newDelegate(this, &HyperTextBox::OnMouseButtonReleased);
					text->eventKeyButtonPressed += newDelegate(this, &HyperTextBox::notifyKeyButtonPressed);
				}
	...
	void HyperTextBox::OnMouseButtonReleased(Widget* _sender, int _left, int _top, MyGUI::MouseButton _id)
	{
		EditBox* editBox = _sender->castType<EditBox>(false);
		if (nullptr != editBox)
		{
			eventTextSelected(this, mSelectedText);
		}
	}

	void HyperTextBox::notifyKeyButtonPressed(MyGUI::Widget* _sender, MyGUI::KeyCode _key, MyGUI::Char _ch)
	{
		EditBox* editBox = _sender->castType<EditBox>(false);
		if (nullptr != editBox)
		{
			std::string textSelection = editBox->getTextSelection();
			textSelection = editBox->getTextSelection();
			textSelection = removeAll(textSelection, "#", 6);

			eventNotifyInsideKeyButtonPressed(this, _key, _ch, textSelection);
		}
	}
	...
	void HyperTextBox::setUrlColour(const MyGUI::Colour& urlColour)
	{
		mUrlColour = urlColour;
	}

	void HyperTextBox::setTextSelectable(bool textSelectable)
	{
		mTextSelectable = textSelectable;
		if (true == mTextSelectable)
			mTextSkin = "EditBoxEmpty";
		else
			mTextSkin = "TextBox";
	...
	std::string HyperTextBox::getSelectedText(void) const
	{
