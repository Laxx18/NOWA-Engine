...
struct __declspec( dllexport ) dtCompressedTile
{
...
struct __declspec( dllexport ) dtTileCacheObstacle
{
	union
	{
...
struct __declspec( dllexport ) dtTileCacheParams
{
	float orig[3];
	float cs, ch;
	int width, height;
...
struct __declspec( dllexport ) dtTileCacheMeshProcess
{
...
class __declspec( dllexport ) dtTileCache
{
public:
	dtTileCache();
	~dtTileCache();
...
private:
	...
	struct __declspec( dllexport ) ObstacleRequest
	{
		int action;
		dtObstacleRef ref;
	};
