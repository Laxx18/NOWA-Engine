...
namespace OgreNewt
{
    ...
    public:
        ...
        void setUserData(const OgreNewt::Any& data)
        {
            m_userdata = data;
        }
        const OgreNewt::Any& getUserData() const
        {
            return m_userdata;
        }
#else
        ...
        Ogre::SceneNode* getOgreNode() const
        {
            return m_node;
        }
        Ogre::ManualObject* getDebugCollisionLines()
        {
            return m_debugCollisionLines;
        }
        ...
        template <class c> void setContactCallback(OgreNewt::function<void(c*, OgreNewt::Body*, OgreNewt::Contact*)> callback, c* instancedClassPointer)
        {
            setContactCallback(OgreNewt::bind(callback, instancedClassPointer, std::placeholders::_1, std::placeholders::_2));
        }

        void removeContactCallback()
        {
        ...
        /**
         * @brief	Sets a desired one-shot impulse velocity to be applied in the next
         *          onForceAndTorqueCallback invocation via ndBodyDynamic::ApplyImpulsePair.
         *          Safe to call from any thread (logic thread). The impulse is consumed
         *          exactly once by Newton at the correct substep time and then cleared.
         * @param[in]	desiredVelocity		The target velocity change (world space, m/s)
         */
        void setDesiredImpulseVelocity(const Ogre::Vector3& desiredVelocity);
    ...
    protected:
        ...
        // Desired one-shot impulse velocity set from the logic thread.
        // Applied once inside onForceAndTorqueCallback via ApplyImpulsePair then cleared.
        // Protected by m_impulseMutex for thread safety per Julio's recommendation.
        Ogre::Vector3 m_desiredImpulseVelocity;
        std::mutex m_impulseMutex;
