...
namespace OgreNewt
{
	...
	namespace Converters
	{
		...
		void MatrixToQuatPos(const dFloat* matrix, Ogre::Quaternion& quat, Ogre::Vector3 &pos)
		{
		...
		//! Take a Quaternion and Position Matrix and create a Newton-happy dFloat matrix!
		...
		dVector OgreToNewtonVector(Ogre::Vector3 vec)
		{
			return dVector(vec.x, vec.y, vec.z);
		}
	...
	namespace CollisionTools
	{
		...
		int CollisionCollide(const OgreNewt::World* world, int maxSize,
			...
			dLong* const attributeA, dLong* const attributeB,
			int threadIndex)
		{
			dFloat matrixA[16];
			dFloat matrixB[16];
...
#else
			return NewtonCollisionCollide(world->getNewtonWorld(), maxSize, colA->getNewtonCollision(), matrixA,
				colB->getNewtonCollision(), matrixB, &retContactPts[0].x, &retNormals[0].x, retPenetrations, attributeA, attributeB, threadIndex);
#endif
		}
	...
	namespace OgreAddons
	{
		...
		*/
		...
		//                else
		//                    currPos = Ogre::Vector3(left - (len / 2.0f), top, -1.0f);
		//                min.makeFloor(currPos);
		//                max.makeCeil(currPos);
		//                maxSquaredRadius = GetMax(maxSquaredRadius, currPos.squaredLength());
		...
		//                dFloat currentWidth = (left + 1.0f) / 2.0f;
...
//void getMeshVertexInformation(const Ogre::MeshPtr mesh, size_t& vertexCount, Ogre::Vector3*& vertices, const Ogre::Vector3& scale)
//{
//	//First, we compute the total number of vertices and init the buffers.
...
//			else
//			{
//				throw Ogre::Exception(0, "Vertex Buffer type not recognised", "getMeshVertexInformation");
//			}
//			subMeshOffset += subMeshVerticiesNum;
//			vao->unmapAsyncTickets(requests);
//
//			// No indices required for collision
//		}
//		subMeshIterator++;
//	}
	}
