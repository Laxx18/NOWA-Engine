...
namespace OgreNewt
{
	...
	enum CollisionPrimitiveType
	{
		...
		DeformableType = SERIALIZE_ID_DEFORMABLE_SOLID,
		ConcaveHullPrimitiveType = 16
	};
	...
	*/
	...
	public:
		...
		//! constructor
		Collision(const NewtonCollision* collision, const World* world, bool ownsNewtonCollision = true);


		//! destructorGetIdentityMatrix
		virtual ~Collision();
		...
		*/
		const NewtonCollision* getNewtonConstCollision() const
		{
			return m_col;
		}
	...
	public:
		...
		{
			NewtonConvexCollisionCalculateInertialMatrix(m_col, &inertia.x, &offset.x);
		}
