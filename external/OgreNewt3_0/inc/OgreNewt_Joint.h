	...
	*/
	...
	public:
		...
		*/
		...
		//! Setup a joint as such, that it is not as respectful to mass of objects
	...
	protected:
		...
		static void _CDECL destructorCallback(const dCustomJoint* me);

		// ! submitConstraint callback glue
		// static void _CDECL submitConstraintCallback(const dCustomJoint* const me, dFloat timestep, int threadIndex);

		//! SetSupportJoint
		void SetSupportJoint(dCustomJoint* supportJoint);

		//! GetSupportJoint
		dCustomJoint* GetSupportJoint() const;
