...
*/
    class _OgreNewtExport RayCastVehicle: public Joint
    {
        ...
        */
		...
		void setSteeringTorque(Ogre::Radian angle, Ogre::Real torque);

		void setTireBrake(int index, Ogre::Real torque);

		void setTireTorque(int index, Ogre::Real torque);

		void setTireSteerAngleForce(int index, Ogre::Radian angle, Ogre::Real turnForce);

		// set and get the combined vehicle resistance to move due to tire rolling resistance, engine resistance, Transmit loses, etc
		// valid values are between 0.05 >= x <= 1.0
		void setTireRollingResistance (int index, Ogre::Real rollingResitanceCoefficient);
		...
		*/
		virtual void showDebugData(Ogre::SceneManager* sceneManager, Ogre::SceneNode* debugRootNode);
