...
namespace NOWA
{
	...
	TerraComponent::TerraComponent()
		...
		center(new Variant(TerraComponent::AttrCenter(), Ogre::Vector3(0.0f, 0.0f, 0.0f), this->attributes)),
		...
		basePixelDimension(new Variant(TerraComponent::AttrBasePixelDimension(), static_cast<unsigned int>(64), this->attributes)),
		useDigging(new Variant(TerraComponent::AttrUseDigging(), false, this->attributes)),
	...
	void TerraComponent::createTerra(void)
	{
		if (nullptr == this->terra && nullptr != AppStateManager::getSingletonPtr()->getCameraManager()->getActiveCamera() && true == this->postInitDone)
		{
			...
			Ogre::Vector3 min = Ogre::Vector3(startX - this->center->getVector3().x, -(this->dimensions->getVector3().y * 0.5f) + (this->center->getVector3().y), startZ - this->center->getVector3().z);
			Ogre::Vector3 max = Ogre::Vector3(endX - this->center->getVector3().x, (this->dimensions->getVector3().y * 0.5f) + (this->center->getVector3().y), endZ - this->center->getVector3().z);
	...
	void TerraComponent::actualizeValue(Variant* attribute)
	{
		...
		else if (TerraComponent::AttrUseDigging() == attribute->getName())
		{
			this->setUseDigging(attribute->getBool());
		}
	...
	void TerraComponent::setUseDigging(bool useDigging)
	{
		this->useDigging->setValue(useDigging);
		if (nullptr != this->terra)
		{
			this->terra->setUseDigging(useDigging);
		}
	}
