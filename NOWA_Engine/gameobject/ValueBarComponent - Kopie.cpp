...
// Attention: How about parent id etc. because when a widget is cloned, its internal id will be re-generated, so the parent id from another widget that should point to this widget is no more valid!
	...
	void ValueBarComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc, const Ogre::String& filePath)
	{
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->innerColor->getVector3())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "9"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "OuterColor"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->outerColor->getVector3())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "BorderSize"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->borderSize->getReal())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->orientationTargetId->getULong())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "Width"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->width->getReal())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->maxValue->getUInt())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "CurrentValue"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->currentValue->getUInt())));
		propertiesXML->append_node(propertyXML);
	}
	...
	bool ValueBarComponent::getActivated(void) const
	{
	...
	void ValueBarComponent::drawValueBar(void)
	{
		...
		if (nullptr != this->orientationTargetGameObject)
		{
			...
			direction.normalise();
			so = MathHelper::getInstance()->faceDirection(this->gameObjectPtr->getSceneNode(), direction);
		}
		else
		{
		...
		// Outer rectangle
		{
			...
			// 1) lt
			this->manualObject->position(p + (o * (so * (sp + (so * Ogre::Vector3(-w - bs, h + bs, 0.0f))))));
			...
			// 2) lb
			this->manualObject->position(p + (o * (so * (sp + (so * Ogre::Vector3(-w - bs, -bs, 0.0f))))));
			this->manualObject->colour(Ogre::ColourValue(tempOuterColor.x, tempOuterColor.y, tempOuterColor.z, 1.0f));
			this->manualObject->normal(0.0f, 0.0f, 1.0f);
			this->manualObject->textureCoord(0, 1);
		...
		// x = game object position
		{
		    ...
		    Ogre::Real sw = this->width->getReal() * 0.5f;
			...
			// Upper half for outer rectangle
			// 1) lt
			this->manualObject->position(p + (o * (so * (sp + (so * Ogre::Vector3(v , h, 0.0f))))));
			...
			// 3) rb
			this->manualObject->position(p + (o * (so * (sp + (so * Ogre::Vector3(w, h, 0.0f))))));
			this->manualObject->colour(Ogre::ColourValue(tempInnerColor.x, tempInnerColor.y, tempInnerColor.z, 1.0f));
			this->manualObject->normal(0.0f, 0.0f, 1.0f);
			this->manualObject->textureCoord(1, 1);
			...
			// 2) lb
			this->manualObject->position(p + (o * (so * (sp + (so * Ogre::Vector3(v , 0.0f, 0.0f))))));
			this->manualObject->colour(Ogre::ColourValue(tempInnerColor.x, tempInnerColor.y, tempInnerColor.z, 1.0f));
			this->manualObject->normal(0.0f, 0.0f, 1.0f);
			this->manualObject->textureCoord(0, 1);

			// 3) rb
			this->manualObject->position(p + (o * (so * (sp + (so * Ogre::Vector3(w, 0.0f, 0.0f))))));
			this->manualObject->colour(Ogre::ColourValue(tempInnerColor.x, tempInnerColor.y, tempInnerColor.z, 1.0f));
			this->manualObject->normal(0.0f, 0.0f, 1.0f);
			this->manualObject->textureCoord(1, 1);
