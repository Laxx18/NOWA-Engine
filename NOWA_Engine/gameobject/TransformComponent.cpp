...
namespace NOWA
{
    ...
    bool TransformComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
    ...
    void TransformComponent::onRemoveComponent(void)
    {
    ...
    bool TransformComponent::disconnect(void)
    {
            ...
            if (true == this->rotationActivated->getBool())
            {
                ...
                if (true == this->rotationRepeat->getBool() || true == this->internalRotationDirectionChange || 0 == this->rotationRound)
                {
                    if (nullptr == this->physicsComponent)
                    {
                        ...
                        NOWA::GraphicsModule::getInstance()->updateNodeOrientation(this->gameObjectPtr->getSceneNode(),
                            this->gameObjectPtr->getSceneNode()->getOrientation() * Ogre::Quaternion(Ogre::Degree(this->rotationSpeed->getReal() * this->rotationOppositeDir), this->rotationAxis->getVector3()));
                    }
                    else
                    {
            ...
            if (true == this->translationActivated->getBool())
            {
                ...
                if (true == this->translationRepeat->getBool() || true == this->internalTranslationDirectionChange || 0 == this->translationRound)
                {
                    if (nullptr == this->physicsComponent)
                    {
                        ...
                        NOWA::GraphicsModule::getInstance()->updateNodePosition(this->gameObjectPtr->getSceneNode(),
                            this->gameObjectPtr->getSceneNode()->getPosition() + this->translationAxis->getVector3() * (this->translationSpeed->getReal() * this->translationOppositeDir) * dt);
                    }
                    else
                    {
    ...
    void TransformComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
    {
        // 2 = int
        // 6 = real
        // 7 = string
        ...
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->translationSpeed->getReal())));
        propertiesXML->append_node(propertyXML);
        ...
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->scaleSpeed->getReal())));
        propertiesXML->append_node(propertyXML);
