...
namespace NOWA
{
    ...
    void CameraComponent::onRemoveComponent(void)
    {
        ...
        NOWA::GraphicsModule::RenderCommand renderCommand = [this, cameraCopy, dummyItemCopy, sceneManagerCopy, cameraManager, gameObjectCopy, workspaceBaseComponentCopy, active]()
        {
            if (cameraCopy)
            {
                ...
                AppStateManager::getSingletonPtr()->getWorkspaceModule()->removeCamera(cameraCopy);
    ...
    void CameraComponent::setActivated(bool activated)
    {
        ...
        if (true == this->active->getBool())
        {
            ...
            if (nullptr != this->camera)
            {
                ...
                if (nullptr != workspaceBaseCompPtr)
                {
                    ...
                    AppStateManager::getSingletonPtr()->getWorkspaceModule()->setPrimaryWorkspace(this->gameObjectPtr->getSceneManager(), this->camera, workspaceBaseCompPtr.get());

                    // Bug: WorkspaceBaseComponent::connect() only enables/disables compositor effects
                    // ONCE, at simulation start, checking each effect's camera via
                    // CompositorEffectBaseComponent::enableEffect() -> cameraComponent->isActivated().
                    // If that camera gets activated LATER (e.g. from a Lua "connect" script running
                    // after simulation start, as opposed to already being marked Activated in the scene
                    // file), connect() already evaluated and locked in "not active" for its effects -
                    // AND createWorkspace() above rebuilds the node graph from scratch, which resets
                    // every post-process node to disabled/unconnected (see the "Ogre/Postprocess"
                    // iteration in createWorkspace() - setStartEnabled(false), addNodeAlias only, no
                    // connect() calls). Re-sync here, at the point a camera ACTUALLY becomes active,
                    // by reusing WorkspaceBaseComponent::enableEffect() - it already does exactly this
                    // (rebuild every effect's desired state from CompositorEffectBaseComponent::
                    // isActivated(), then reconnectAllNodes()) in one self-contained render command.
                    // Passing an effectName that matches no real effect ("" never equals a real
                    // comp->effectName) makes its override a no-op, so every component simply keeps
                    // its own current isActivated() state - a pure "reapply everything" call, with no
                    // logic duplicated here and no extra beginWorkspaceTransition()/
        ...
        else
        {
            ...
            {
                AppStateManager::getSingletonPtr()->getWorkspaceModule()->removeCamera(this->camera);
    ...
    void CameraComponent::setActivatedFlag(bool activated)
    {
        ...
        if (nullptr != this->camera)
        {
            NOWA::GraphicsModule::RenderCommand renderCommand = [this, activated]()
            {
                ...
                if (false == activated)
                {
                    // if (true == AppStateManager::getSingletonPtr()->getWorkspaceModule()->hasMoreThanOneWorkspace())
                    {
                        auto workspaceBaseCompPtr = NOWA::makeStrongPtr(this->gameObjectPtr->getComponent<WorkspaceBaseComponent>());
                        if (nullptr != workspaceBaseCompPtr)
                        {
