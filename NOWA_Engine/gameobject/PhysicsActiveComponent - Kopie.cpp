...
// Attention: Is this correct
	...
	PhysicsActiveComponent::ContactData PhysicsActiveComponent::getContactAhead(int index, const Ogre::Vector3& offset, Ogre::Real length, bool forceDrawLine, unsigned int categoryIds)
	{
		...
		// Get the direction of the ray (ahead of the player)
		Ogre::Vector3 direction = this->physicsBody->getOrientation() * this->gameObjectPtr->getDefaultDirection();

		// Incorporate gravity direction if it's defined (gravityDirection should be in the same plane as the raycast direction)
		if (!this->gravityDirection.isZeroLength())
		{
			// You can modify direction with gravity if needed, e.g., pulling the ray slightly towards the gravity direction
			// Here we assume a small influence of gravity on the ray's direction, you can adjust the magnitude as needed
			direction += this->gravityDirection * 0.1f;  // Adjust 0.1f as a factor for gravity influence
			direction.normalise();  // Re-normalize the direction after the gravity adjustment
		}
		...
		// Shoot the ray in the direction modified by gravity
		OgreNewt::BasicRaycast ray(this->ogreNewt, fromPosition, toPosition, true);

		// Debug line drawing if enabled
		if (true == this->bShowDebugData || true == forceDrawLine)
		{
			// Get a key from id and user index to match a line, when line drawing is set to on
			Ogre::String key = std::to_string(this->gameObjectPtr->getId()) + std::to_string(index) + "getContactAhead";
		...
		if (info.mBody)
		{
			...
			if (type == finalType)
			{
				...
				try
				{
					// Attempt to retrieve the GameObject from the raycast hit
					Ogre::SceneNode* tempNode = static_cast<Ogre::SceneNode*>(info.mBody->getOgreNode());
					if (tempNode)
					{
						gameObject = Ogre::any_cast<GameObject*>(tempNode->getUserObjectBindings().getUserAny());
				...
				{
					// Catch any exceptions and return default data
					return std::move(PhysicsActiveComponent::ContactData(nullptr, height, normal, slope));
				}
			}
		}
		...
		Ogre::Real from, Ogre::Real to, bool forceDrawLine, unsigned int categoryIds)
	{
		...
		// Calculate the effective direction by combining the given direction and gravity direction
		Ogre::Vector3 effectiveDirection = direction + this->gravityDirection;

		// raycast in direction of the object
		Ogre::Vector3 position = this->physicsBody->getPosition() + (this->physicsBody->getOrientation() * offset);

		// get the position relative to direction and an offset
		Ogre::Vector3 fromPosition = position + (effectiveDirection * from);
		Ogre::Vector3 toPosition = position + (effectiveDirection * to);
	...
	Ogre::Real PhysicsActiveComponent::determineGameObjectHeight(const Ogre::Vector3& positionOffset1, const Ogre::Vector3& positionOffset2, unsigned int categoryIds)
	{
		...
		// Compute the world-space position of the offset point (where the ray starts)
		Ogre::Vector3 charPoint = this->physicsBody->getPosition() + (this->physicsBody->getOrientation() * positionOffset1);
		...
		if (info.mBody)
		{
			...
			if (type == finalType)
			{
				...
				charPoint = this->physicsBody->getPosition() + (this->physicsBody->getOrientation() * positionOffset2);

				// Straht von Anfangsposition in Richtung der Schwerkraft erzeugen
