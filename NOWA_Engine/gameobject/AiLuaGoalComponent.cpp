...
namespace NOWA
{
    ...
    void AiLuaGoalComponent::setActivated(bool activated)
    {
        ...
        // checkLuaStateAvailable, createLuaEnvironmentForStateTable, setRootGoal
        // -> Lua "enter" call) and MUST run on the main/logic thread - Lua itself
    ...
    void AiLuaGoalComponent::createStaticApiForLua(lua_State* lua, class_<GameObject>& gameObjectClass, class_<GameObjectController>& gameObjectControllerClass)
    {
        ...
        // LuaScriptApi::getInstance()->addClassToCollection("AiLuaGoalComponent", "void setGlobalState(Table stateName)", "Sets the global state name, which will be loaded in lua script and executed besides the current state. Important: Do not use
        // state function in 'connect' function, because at this time the AiLuaComponent is not ready!"); LuaScriptApi::getInstance()->addClassToCollection("AiLuaGoalComponent", "void exitGlobalState()", "Exits the global state, if it does exist.");
        // LuaScriptApi::getInstance()->addClassToCollection("AiLuaGoalComponent", "String getCurrentState()", "Gets the current state name.");
        // LuaScriptApi::getInstance()->addClassToCollection("AiLuaGoalComponent", "String getPreviousState()", "Gets the previous state name, that has been loaded bevor the current state name. Important: Do not use state function in 'connect'
        // function, because at this time the AiLuaComponent is not ready!"); LuaScriptApi::getInstance()->addClassToCollection("AiLuaGoalComponent", "String getGlobalState()", "Gets the global state name, if existing, else empty string. Important:
        // Do not use state function in 'connect' function, because at this time the AiLuaComponent is not ready!"); LuaScriptApi::getInstance()->addClassToCollection("AiLuaGoalComponent", "void changeState(Table newStateName)", "Changes the current
        // state name to a new one. Calles 'exit' function on the current state and 'enter' on the new state in lua script. Important: Do not use state function in 'connect' function, because at this time the AiLuaComponent is not ready!");
        // LuaScriptApi::getInstance()->addClassToCollection("AiLuaGoalComponent", "void revertToPreviousState()", "Changes the current state name to the previous one. Calles 'exit' function on the current state and 'enter' on the previous state in
        // lua script. Important: Do not use state function in 'connect' function, because at this time the AiLuaComponent is not ready!");
