...
namespace NOWA
{
	...
	bool CrowdComponent::init(rapidxml::xml_node<>*& propertyElement)
	{
	...
	void CrowdComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
	{
		// 2 = int
		// 6 = real
		// 7 = string
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->controlled->getBool())));
		propertiesXML->append_node(propertyXML);
	}
