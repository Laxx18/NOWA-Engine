...
namespace
{
    // The anchor is authored in the object's LOCAL frame: -0.5 in x means the
    // left edge, +0.5 the right one. Body::getAABB() however returns the axis
    // aligned WORLD bounding box, so as soon as the object is rotated its extents
    // no longer line up with the axes the anchor refers to. For a door rotated by
    // 90 degrees around y, world x holds the door's THICKNESS instead of its
    // width, which pushed the pivot into the middle of the door.
    //
    // Two steps are needed: rotate the world extents back into the local frame,
    // build the offset there, then rotate that offset out into the world again.
    Ogre::Vector3 calculateAnchorWorldPosition(OgreNewt::Body* body, const Ogre::Vector3& anchorPosition)
    {
        if (nullptr == body)
        {
        ...
        Ogre::Matrix3 inverseRotation;
        orientation.Inverse().ToRotationMatrix(inverseRotation);

        // Taking the absolute value of every matrix entry keeps the result a size
        // (all components positive) instead of a signed vector. Exact for axis
        // aligned rotations, slightly conservative for arbitrary angles.
        const Ogre::Vector3 worldSize = body->getAABB().getSize();
        Ogre::Vector3 localSize = Ogre::Vector3::ZERO;

        for (size_t row = 0; row < 3; ++row)
        {
            for (size_t col = 0; col < 3; ++col)
            {
                localSize[row] += Ogre::Math::Abs(inverseRotation[row][col]) * worldSize[col];
            }
        }

        const Ogre::Vector3 localOffset = anchorPosition * localSize;

        return body->getPosition() + (orientation * localOffset);
    }
}
...
namespace NOWA
{
        ...
        for (size_t i = 0; i < this->gameObjectPtr->getComponents()->size(); i++)
        {
            ...
            if (component.get() == this)
            {
                if (false == foundPriorPhysicsComponent)
                {
                    ...
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointComponent] Illegal state: Could not add joint component for game object: " + this->gameObjectPtr->getName() +
                                                                                            " because a physics component must be placed before a joint component! Please swap the components or add a prior physics component!");
    ...
    bool JointComponent::connect(void)
    {
        ...
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointComponent] Error: Final init failed, because the game object: " + this->gameObjectPtr->getName() + " has no kind of physics component!");
            return false;
        }
    }
    ...
    void JointComponent::releaseJoint(bool resetPredecessorAndTarget)
    {
        if (nullptr != this->joint)
        {
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[JointComponent] Joint: " + this->type->getString() + " released for game object: " + gameObjectName);
    ...
    JointHingeComponent::JointHingeComponent() : JointComponent()
    {
        ...
        Ogre::String torqueFrictionDescription =
            "Sets the friction (max. torque) for torque. That is, without a friction the torque will not work. So get a balance between torque and friction (1, 20) e.g. The more friction, the more powerful the motor will become.";
    ...
    bool JointHingeComponent::connect(void)
    {
        ...
        Ogre::Vector3 debugPosition = this->anchorPosition->getVector3();

        if (nullptr != this->body && nullptr != this->gameObjectPtr && nullptr != this->gameObjectPtr->getSceneNode())
        {
            this->jointPosition = calculateAnchorWorldPosition(this->body, this->anchorPosition->getVector3());
            // internalShowDebugData attaches the arrow as a child of the game object
            // node, so it needs the pivot expressed in that node's local space.
            debugPosition = this->gameObjectPtr->getSceneNode()->convertWorldToLocalPosition(this->jointPosition);
        }

        this->internalShowDebugData(true, 0, debugPosition, this->pin->getVector3());
    ...
    bool JointHingeComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        {
            // Same helper as setAnchorPosition and getUpdatedJointPosition, so the
            // three call sites can never drift apart again.
            this->jointPosition = calculateAnchorWorldPosition(this->body, this->anchorPosition->getVector3());
        }
        else
        {
        ...
        if (this->predecessorJointCompPtr)
        {
            ...
            {
                Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL, "[JointHingeComponent] Creating hinge joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + gameObjectName);
            }
        }
        else
        {
    ...
    Ogre::Vector3 JointHingeComponent::getUpdatedJointPosition(void)
    {
        ...
        {
            return calculateAnchorWorldPosition(this->body, this->anchorPosition->getVector3());
        }
        else
        {
            return this->jointPosition;
    ...
    void JointHingeComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            NOWA::GraphicsModule::RenderCommand renderCommand = [this]()
            {
                // The debug node is a CHILD of the game object node, so it expects a
                // position in the parent's local space. Assigning the world pivot
                // directly made the arrow jump away from the actual joint.
                this->debugGeometryNode->setPosition(this->gameObjectPtr->getSceneNode()->convertWorldToLocalPosition(this->jointPosition));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointHingeComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    void JointHingeComponent::setPin(const Ogre::Vector3& pin)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            NOWA::GraphicsModule::RenderCommand renderCommand = [this, pin]()
            {
                this->debugGeometryNode->setDirection(pin);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointHingeComponent::setPin debugGeometryNode");
        }
    }
    ...
    JointHingeActuatorComponent::JointHingeActuatorComponent() : JointComponent(), round(0), internalDirectionChange(false), oppositeDir(1.0f)
    {
        ...
        this->targetAngle
            ->setDescription("Sets the target angle til the joint should be rotated. Note: When using @directionChange, @Repeat, specify the target angle the same as the @MaxAngleLimit, so that the joint will rotate backwards and forwards.");
    ...
    bool JointHingeActuatorComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            ...
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointHingeActuatorComponent] Creating hinge actuator joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    void JointHingeActuatorComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointHingeActuatorComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    void JointHingeActuatorComponent::setPin(const Ogre::Vector3& pin)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointHingeActuatorComponent::setPin debugGeometryNode");
        }
    }
    ...
    JointBallAndSocketComponent::JointBallAndSocketComponent() : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    bool JointBallAndSocketComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointBallAndSocketComponent] Creating ball and socket joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    void JointBallAndSocketComponent::setTwistSpringDamper(bool state)
    {
        ...
        if (nullptr != ballAndSocketJoint)
        {
            ballAndSocketJoint->setTwistSpringDamper(state, this->twistSpringDamperRelaxation->getReal(), this->twistSpringK->getReal(), this->twistSpringD->getReal());
        }
    }
    ...
    JointPointToPointComponent::JointPointToPointComponent() : JointComponent(), jointPosition2(Ogre::Vector3::ZERO)
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    JointPointToPointComponent::~JointPointToPointComponent()
    {
        if (nullptr != this->debugGeometryNode2)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "JointPointToPointComponent::~JointPointToPointComponent");
        }
    }
    ...
    Ogre::Vector3 JointPointToPointComponent::getUpdatedJointPosition(void)
    {
        ...
        if (false == this->hasCustomJointPosition)
        {
            return calculateAnchorWorldPosition(this->body, this->anchorPosition1->getVector3());
        }
        else
        {
            return this->jointPosition;
    ...
    bool JointPointToPointComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (nullptr != this->predecessorJointCompPtr)
        {
            ...
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointPointToPointComponent] Creating point to point joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    void JointPointToPointComponent::setAnchorPosition1(const Ogre::Vector3& anchorPosition1)
    {
        ...
        if (nullptr != this->body)
        {
            this->jointPosition = calculateAnchorWorldPosition(this->body, this->anchorPosition1->getVector3());
            this->jointAlreadyCreated = false;
        }

        if (nullptr != this->debugGeometryNode)
        {
            NOWA::GraphicsModule::RenderCommand renderCommand = [this, anchorPosition1]()
            {
                this->debugGeometryNode->setPosition(anchorPosition1);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointPointToPointComponent::setAnchorPosition1 debugGeometryNode");
        }
    }
    ...
    void JointPointToPointComponent::setAnchorPosition2(const Ogre::Vector3& anchorPosition2)
    {
        ...
        if (nullptr != this->predecessorJointCompPtr)
        {
            ...
            this->jointPosition2 = calculateAnchorWorldPosition(predecessorBody, this->anchorPosition2->getVector3());
            this->jointAlreadyCreated = false;
        }
    }
    ...
    JointPinComponent::JointPinComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    JointPlaneComponent::JointPlaneComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointPlaneComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL, "[JointPlaneComponent] Creating plane joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    JointSpringComponent::JointSpringComponent() : JointComponent(), predecessorBody(nullptr), dragLineNode(nullptr), dragLineObject(nullptr)
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    void JointSpringComponent::releaseSpring(void)
    {
        ...
        if (nullptr != this->dragLineNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "JointSpringComponent::releaseSpring");
        }
    }
    ...
    void JointSpringComponent::createLine(void)
    {
        if (nullptr == this->dragLineNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "JointSpringComponent::createLine");
        }
    }
    ...
    JointAttractorComponent::JointAttractorComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    JointCorkScrewComponent::JointCorkScrewComponent() : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    bool JointCorkScrewComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointCorkScrewComponent] Creating cork screw joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
    ...
    void JointCorkScrewComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            NOWA::GraphicsModule::RenderCommand renderCommand = [this, anchorPosition]()
            {
                this->debugGeometryNode->setPosition(anchorPosition);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointCorkScrewComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    JointPassiveSliderComponent::JointPassiveSliderComponent() : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    bool JointSliderActuatorComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointSliderActuatorComponent] Creating slider actuator joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    void JointSliderActuatorComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointSliderActuatorComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    void JointSliderActuatorComponent::setPin(const Ogre::Vector3& pin)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointSliderActuatorComponent::setPin debugGeometryNode");
        }
    }
    ...
    JointSlidingContactComponent::JointSlidingContactComponent() : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    JointActiveSliderComponent::JointActiveSliderComponent() : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    bool JointActiveSliderComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        // Release joint each time, to create new one with new values
        this->internalReleaseJoint();
        this->joint = new OgreNewt::ActiveSliderJoint(this->body, predecessorBody, this->jointPosition, this->pin->getVector3(), this->minStopDistance->getReal(), this->maxStopDistance->getReal(), this->moveSpeed->getReal(), this->repeat->getBool(),
            this->directionChange->getBool(), this->activated->getBool());
    ...
    JointMathSliderComponent::JointMathSliderComponent() : JointComponent(), oppositeDir(1.0f), progress(0.0f), round(0)
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    void JointMathSliderComponent::parseMathematicalFunction(void)
    {
        ...
        if (resX > 0)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointMathSliderComponent] Mathematical function parse error for mx: " + Ogre::String(this->functionParser[0].ErrorMsg()));
        }

        int resY = this->functionParser[1].Parse(this->getFunctionY(), "t");
        if (resY > 0)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointMathSliderComponent] Mathematical function parse error for my: " + Ogre::String(this->functionParser[1].ErrorMsg()));
        }

        int resZ = this->functionParser[2].Parse(this->getFunctionZ(), "t");
        if (resZ > 0)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointMathSliderComponent] Mathematical function parse error for mz: " + Ogre::String(this->functionParser[2].ErrorMsg()));
        }
    }
    ...
    void JointMathSliderComponent::setFunctionY(const Ogre::String& my)
    {
        ...
        if (resY > 0)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointMathSliderComponent] Mathematical function parse error for mx: " + Ogre::String(this->functionParser[1].ErrorMsg()));
        }
    }
        ...
        if (true == success)
        {
            ...
            else
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[JointKinematicComponent] Error: Final init failed, because the game object: " + this->gameObjectPtr->getName() + " has no sort of physics active component to set gravity!");
                return false;
            }
        }
    ...
    bool JointKinematicComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            ...
            if (nullptr != this->predecessorJointCompPtr->getOwner() && nullptr != this->gameObjectPtr)
            {
                Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL, "[JointKinematicComponent] Creating hinge joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + gameObjectName);
            }
        }
        else
        {
    ...
    JointTargetTransformComponent::JointTargetTransformComponent() : JointComponent(), offsetPosition(Ogre::Vector3::ZERO), offsetOrientation(Ogre::Quaternion::IDENTITY), gravity(Ogre::Vector3::ZERO)
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointTargetTransformComponent::connect(void)
    {
        ...
        if (true == success)
        {
            ...
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[JointTargetTransformComponent] Error: Final init failed, because the game object: " + this->gameObjectPtr->getName() + " has no sort of physics active component to set gravity!");
                return false;
            }
        }
    ...
    bool JointTargetTransformComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        {
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointKinematicComponent] Creating target transform joint for body1 (predecessor) name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    JointPathFollowComponent::JointPathFollowComponent() : JointComponent(), lineNode(nullptr), lineObjects(nullptr), pathProgress(0.0f), currentMoveDirection(Ogre::Vector3::ZERO), oldActivated(true), finishedOnce(false)
    {
    ...
    bool JointPathFollowComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (nullptr != this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointPathFollowComponent] Creating path follow joint for body1 (predecessor) name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    void JointPathFollowComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointPathFollowComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    void JointPathFollowComponent::createLines(void)
    {
        ...
        NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "JointPathFollowComponent::createLines");
    }
    ...
    void JointPathFollowComponent::destroyLines(void)
    {
        if (this->lineNode != nullptr)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "JointPathFollowComponent::destroyLines");
        }
    }

    /*******************************JointDryRollingFrictionComponent*******************************/

    JointDryRollingFrictionComponent::JointDryRollingFrictionComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
        ...
        this->rollingFrictionCoefficient = new Variant(JointDryRollingFrictionComponent::AttrFrictionCoefficient(), 0.5f, this->attributes);
        this->rollingFrictionCoefficient->setDescription("Dimensionless rolling friction coefficient. Higher values increase the rolling resistance torque "
                                                         "generated around the contact point.");

        this->contactTrail = new Variant(JointDryRollingFrictionComponent::AttrContactTrail(), 0.0f, this->attributes);
        this->contactTrail->setDescription("Effective contact trail (lever arm) in world units. This acts like a rolling radius for the friction torque: "
    ...
    JointGearComponent::JointGearComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointGearComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (nullptr != this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointGearComponent] Creating gear joint for body1 (predecessor) name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
        ...
        if (nullptr == this->targetJointCompPtr)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointGearComponent] Error could not create gear for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist.");
            return false;
        }
        ...
        if (nullptr == jointHingeCompPtr1)
        {
            ...
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[JointGearComponent] Error could not create gear for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist or is no hinge.");
                return false;
            }
        }
        ...
        if (nullptr == jointHingeCompPtr2)
        {
            ...
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[JointGearComponent] Error could not create gear for joint: " + this->gameObjectPtr->getName() + ", because the predecessor joint handler does not exist or is no hinge.");
                return false;
            }
        }
        ...
        {
            this->joint = new OgreNewt::Gear(jointHingeCompPtr1->getBody(), jointHingeCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointHingeCompPtr2->getBody()->getOrientation() * jointHingeCompPtr2->getPin(), this->gearRatio->getReal());
        }
        else if (jointHingeActuatorCompPtr1 && jointHingeCompPtr2)
        {
            this->joint = new OgreNewt::Gear(jointHingeActuatorCompPtr1->getBody(), jointHingeCompPtr2->getBody(), jointHingeActuatorCompPtr1->getBody()->getOrientation() * jointHingeActuatorCompPtr1->getPin(),
                jointHingeCompPtr2->getBody()->getOrientation() * jointHingeCompPtr2->getPin(), this->gearRatio->getReal());
        }
        else if (jointHingeCompPtr1 && jointHingeActuatorCompPtr2)
        {
            this->joint = new OgreNewt::Gear(jointHingeCompPtr1->getBody(), jointHingeActuatorCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointHingeActuatorCompPtr2->getBody()->getOrientation() * jointHingeActuatorCompPtr2->getPin(), this->gearRatio->getReal());
        }
    ...
    JointRackAndPinionComponent::JointRackAndPinionComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointRackAndPinionComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                "[JointRackAndPinionComponent] Error could not create rack and pinion for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist.");
            return false;
        }
        JointHingeCompPtr jointHingeCompPtr1 = boost::dynamic_pointer_cast<JointHingeComponent>(this->targetJointCompPtr);
        JointHingeActuatorCompPtr jointHingeActuatorCompPtr1;
        if (nullptr == jointHingeCompPtr1)
        {
            ...
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[JointRackAndPinionComponent] Error could not create rack and pinion for joint: " + this->gameObjectPtr->getName() + ", because the predecessor joint handler does not exist or is no hinge.");
                return false;
            }
        }
        ...
        if (nullptr == jointPassiveSliderCompPtr2)
        {
            ...
            if (nullptr == jointActiveSliderCompPtr2)
            {
                ...
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                        "[JointRackAndPinionComponent] Error could not create rack and pinion for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist or is no slider.");
                    return false;
                }
            }
        }
        ...
        {
            this->joint = new OgreNewt::WormGear(jointHingeCompPtr1->getBody(), jointPassiveSliderCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointPassiveSliderCompPtr2->getBody()->getOrientation() * jointPassiveSliderCompPtr2->getPin(), this->gearRatio->getReal());
        }
        else if (jointHingeCompPtr1 && jointSliderActuatorCompPtr2)
        {
            this->joint = new OgreNewt::WormGear(jointHingeCompPtr1->getBody(), jointSliderActuatorCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointSliderActuatorCompPtr2->getBody()->getOrientation() * jointSliderActuatorCompPtr2->getPin(), this->gearRatio->getReal());
        }
        else if (jointHingeActuatorCompPtr1 && jointPassiveSliderCompPtr2)
        {
            this->joint = new OgreNewt::WormGear(jointHingeActuatorCompPtr1->getBody(), jointPassiveSliderCompPtr2->getBody(), jointHingeActuatorCompPtr1->getBody()->getOrientation() * jointHingeActuatorCompPtr1->getPin(),
                jointPassiveSliderCompPtr2->getBody()->getOrientation() * jointPassiveSliderCompPtr2->getPin(), this->gearRatio->getReal());
        }
        else
        {
            // Predecessor may also be an active slider
            this->joint = new OgreNewt::WormGear(jointHingeCompPtr1->getBody(), jointActiveSliderCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointActiveSliderCompPtr2->getBody()->getOrientation() * jointActiveSliderCompPtr2->getPin(), this->gearRatio->getReal());
        }
    ...
    JointWormGearComponent::JointWormGearComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointWormGearComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (nullptr == this->targetJointCompPtr)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointWormGearComponent] Error could not create worm gear for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist.");
            return false;
        }
        JointHingeCompPtr jointHingeCompPtr1 = boost::dynamic_pointer_cast<JointHingeComponent>(this->targetJointCompPtr);
        JointHingeActuatorCompPtr jointHingeActuatorCompPtr1;
        if (nullptr == jointHingeCompPtr1)
        {
            ...
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[JointWormGearComponent] Error could not create worm gear for joint: " + this->gameObjectPtr->getName() + ", because the predecessor joint handler does not exist or is no hinge.");
                return false;
            }
        }
        ...
        if (nullptr == jointPassiveSliderCompPtr2)
        {
            ...
            if (nullptr == jointActiveSliderCompPtr2)
            {
                ...
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                        "[JointWormGearComponent] Error could not create worm gear for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist or is no slider.");
                    return false;
                }
            }
        }
        ...
        if (jointHingeCompPtr1 && jointPassiveSliderCompPtr2)
        {
            this->joint = new OgreNewt::CustomGearAndSlide(jointHingeCompPtr1->getBody(), jointPassiveSliderCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointPassiveSliderCompPtr2->getBody()->getOrientation() * jointPassiveSliderCompPtr2->getPin(), this->gearRatio->getReal(), this->slideRatio->getReal());
        }
        else if (jointHingeCompPtr1 && jointSliderActuatorCompPtr2)
        {
            this->joint = new OgreNewt::CustomGearAndSlide(jointHingeCompPtr1->getBody(), jointSliderActuatorCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointSliderActuatorCompPtr2->getBody()->getOrientation() * jointSliderActuatorCompPtr2->getPin(), this->gearRatio->getReal(), this->slideRatio->getReal());
        }
        else if (jointHingeActuatorCompPtr1 && jointPassiveSliderCompPtr2)
        {
            this->joint = new OgreNewt::CustomGearAndSlide(jointHingeActuatorCompPtr1->getBody(), jointPassiveSliderCompPtr2->getBody(), jointHingeActuatorCompPtr1->getBody()->getOrientation() * jointHingeActuatorCompPtr1->getPin(),
                jointPassiveSliderCompPtr2->getBody()->getOrientation() * jointPassiveSliderCompPtr2->getPin(), this->gearRatio->getReal(), this->slideRatio->getReal());
        }
        else
        {
            // Predecessor may also be an active slider
            this->joint = new OgreNewt::CustomGearAndSlide(jointHingeCompPtr1->getBody(), jointActiveSliderCompPtr2->getBody(), jointHingeCompPtr1->getBody()->getOrientation() * jointHingeCompPtr1->getPin(),
                jointPassiveSliderCompPtr2->getBody()->getOrientation() * jointActiveSliderCompPtr2->getPin(), this->gearRatio->getReal(), this->slideRatio->getReal());
        }
    ...
    JointPulleyComponent::JointPulleyComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointPulleyComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (nullptr == this->targetJointCompPtr)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[JointPulleyComponent] Error could not create pulley for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist.");
            return false;
        }
        ...
        if (nullptr == jointPassiveSliderCompPtr1)
        {
            ...
            if (nullptr == jointActiveSliderCompPtr1)
            {
                ...
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                        "[JointPulleyComponent] Error could not create pulley for joint: " + this->gameObjectPtr->getName() + ", because the target joint handler does not exist or is no slider.");
                    return false;
                }
            }
        }
        ...
        if (nullptr == jointPassiveSliderCompPtr2)
        {
            ...
            if (nullptr == jointActiveSliderCompPtr2)
            {
                ...
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                        "[JointPulleyComponent] Error could not create pulley for joint: " + this->gameObjectPtr->getName() + ", because the predecessor joint handler does not exist or is no slider.");
                    return false;
                }
            }
        }
        ...
        {
            this->joint = new OgreNewt::Pulley(jointPassiveSliderCompPtr1->getBody(), jointPassiveSliderCompPtr2->getBody(), jointPassiveSliderCompPtr1->getBody()->getOrientation() * jointPassiveSliderCompPtr1->getPin(),
        ...
        {
            this->joint = new OgreNewt::Pulley(jointActiveSliderCompPtr1->getBody(), jointPassiveSliderCompPtr2->getBody(), jointActiveSliderCompPtr1->getBody()->getOrientation() * jointActiveSliderCompPtr1->getPin(),
                jointPassiveSliderCompPtr2->getBody()->getOrientation() * jointPassiveSliderCompPtr2->getPin(), this->pulleyRatio->getReal());
        }
        else if (jointPassiveSliderCompPtr1 && jointActiveSliderCompPtr2)
        {
            this->joint = new OgreNewt::Pulley(jointPassiveSliderCompPtr1->getBody(), jointActiveSliderCompPtr2->getBody(), jointPassiveSliderCompPtr1->getBody()->getOrientation() * jointPassiveSliderCompPtr1->getPin(),
        ...
        {
            this->joint = new OgreNewt::Pulley(jointPassiveSliderCompPtr1->getBody(), jointSliderActuatorCompPtr2->getBody(), jointPassiveSliderCompPtr1->getBody()->getOrientation() * jointPassiveSliderCompPtr1->getPin(),
        ...
        {
            this->joint = new OgreNewt::Pulley(jointSliderActuatorCompPtr1->getBody(), jointPassiveSliderCompPtr2->getBody(), jointActiveSliderCompPtr1->getBody()->getOrientation() * jointSliderActuatorCompPtr1->getPin(),
                jointSliderActuatorCompPtr1->getBody()->getOrientation() * jointPassiveSliderCompPtr2->getPin(), this->pulleyRatio->getReal());
        }
        else
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                "[JointPulleyComponent] Error could not create pulley for joint: " + this->gameObjectPtr->getName() + ", because both predecessor and target joint handler are two active slider! But only one of them can be one.");
            return false;
        }
    ...
    JointUniversalComponent::JointUniversalComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    void JointUniversalComponent::setPin(const Ogre::Vector3& pin)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointUniversalComponent::setPin debugGeometryNode");
        }
    }
    ...
    bool JointUniversalActuatorComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointUniversalActuatorComponent] Creating universal actuator joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
        ...
        // Release joint each time, to create new one with new values
        this->internalReleaseJoint();
        this->joint = new OgreNewt::UniversalActuator(this->body, predecessorBody, this->jointPosition, Ogre::Degree(this->angleRate0->getReal()), Ogre::Degree(this->minAngleLimit0->getReal()), Ogre::Degree(this->maxAngleLimit0->getReal()),
    ...
    void JointUniversalActuatorComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointUniversalComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    Joint6DofComponent::Joint6DofComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool Joint6DofComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (Ogre::Vector3::ZERO == customJointPosition)
        {
            this->jointPosition = calculateAnchorWorldPosition(this->body, this->anchorPosition0->getVector3());
        }
        else
        {
            this->hasCustomJointPosition = true;
        ...
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            jointPosition2 = calculateAnchorWorldPosition(predecessorBody, this->anchorPosition1->getVector3());

            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL, "[Joint6DofComponent] Creating 6 Dof joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    Ogre::Vector3 Joint6DofComponent::getUpdatedJointPosition(void)
    {
        ...
        if (false == this->hasCustomJointPosition)
        {
            return calculateAnchorWorldPosition(this->body, this->anchorPosition0->getVector3());
        }
        else
        {
            return this->jointPosition;
    ...
    JointMotorComponent::JointMotorComponent(void) : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    bool JointMotorComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointMotorComponent] Creating motor joint for body name: " + this->gameObjectPtr->getName() + " predecessor body name: " + this->predecessorJointCompPtr->getOwner()->getName());
        }
    ...
    JointWheelComponent::JointWheelComponent(void) : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
    ...
    bool JointWheelComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL, "[JointWheelComponent] Creating wheel joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    void JointWheelComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointWheelComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    void JointWheelComponent::setPin(const Ogre::Vector3& pin)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointWheelComponent::setPin debugGeometryNode");
        }
    }
    ...
    JointFlexyPipeHandleComponent::JointFlexyPipeHandleComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointFlexyPipeHandleComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointFlexyPipeHandleComponent] Creating flexy pipe handle joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
    ...
    {
        module(lua)[class_<JointFlexyPipeHandleComponent, GameObjectComponent>("JointFlexyPipeHandleComponent")
        ...
        LuaScriptApi::getInstance()->addClassToCollection("GameObject", "JointFlexyPipeHandleComponent getJointFlexyPipeHandleComponent()",
            "Gets the joint flexy pipe handle component. This can be used if the game object just has one joint flexy pipe handle component.");
        ...
        LuaScriptApi::getInstance()->addClassToCollection("GameObjectController", "JointFlexyPipeHandleComponent castJointFlexyPipeHandleComponent(JointFlexyPipeHandleComponent other)",
            "Casts an incoming type from function for lua auto completion.");
    }

    /*******************************JointFlexyPipeSpinnerComponent*******************************/

    JointFlexyPipeSpinnerComponent::JointFlexyPipeSpinnerComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
    ...
    bool JointFlexyPipeSpinnerComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            predecessorBody = this->predecessorJointCompPtr->getBody();
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                "[JointFlexyPipeSpinnerComponent] Creating hinge joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + this->gameObjectPtr->getName());
        }
        else
        {
    ...
    JointVehicleTireComponent::JointVehicleTireComponent() : JointComponent()
    {
        // Note that also in JointComponent internalBaseInit() is called, which sets the values for JointComponent, so this is called for already existing values
        // But luckely in Variant, no new attributes are added, but the values changed via interalAdd(...) in Variant
        this->type->setReadOnly(false);
        ...
        /*
            In vehicle dynamics, longitudinal and lateral friction are key factors that influence a car's behavior during acceleration, braking, and cornering. Let's explore the concepts of longitudinal and lateral friction and how they can be
           applied in a simulation context.
    ...
    bool JointVehicleTireComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        ...
        if (this->predecessorJointCompPtr)
        {
            ...
            if (nullptr != this->predecessorJointCompPtr->getOwner() && nullptr != this->gameObjectPtr)
            {
                Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL, "[JointVehicleTireComponent] Creating tire vehicle tire joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + gameObjectName);
            }
    ...
    void JointVehicleTireComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointVehicleTireComponent::setAnchorPosition debugGeometryNode");
        }
    }
    ...
    void JointVehicleTireComponent::setPin(const Ogre::Vector3& pin)
    {
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointVehicleTireComponent::setPin debugGeometryNode");
        }
    }
    ...
    JointComplexVehicleTireComponent::JointComplexVehicleTireComponent() : JointComponent()
    {
        this->type->setReadOnly(false);
        this->type->setValue(this->getClassName());
        this->type->setReadOnly(true);
        this->type->setDescription("Tire joint for ND4 ComplexVehicle chassis.");

        this->anchorPosition = new Variant(JointComplexVehicleTireComponent::AttrAnchorPosition(), Ogre::Vector3::ZERO, this->attributes);
        this->pin = new Variant(JointComplexVehicleTireComponent::AttrPin(), Ogre::Vector3(0.0f, 0.0f, 1.0f), this->attributes);
        this->tireSide = new Variant(JointComplexVehicleTireComponent::AttrTireSide(), Ogre::StringVector({"Tire Side Right", "Tire Side Left"}), this->attributes);
        this->tireSteer = new Variant(JointComplexVehicleTireComponent::AttrTireSteer(), Ogre::StringVector({"No Steer", "Steer"}), this->attributes);
        this->steerSide = new Variant(JointComplexVehicleTireComponent::AttrSteerSide(), Ogre::StringVector({"Steer Side Right", "Steer Side Left"}), this->attributes);
        this->tireAccel = new Variant(JointComplexVehicleTireComponent::AttrTireAccel(), Ogre::StringVector({"No Accel", "Accel"}), this->attributes);
        this->brakeMode = new Variant(JointComplexVehicleTireComponent::AttrBrakeMode(), Ogre::StringVector({"No Brake", "Brake"}), this->attributes);
        this->lateralFriction = new Variant(JointComplexVehicleTireComponent::AttrLateralFriction(), 2.0f, this->attributes);
        this->longitudinalFriction = new Variant(JointComplexVehicleTireComponent::AttrLongitudinalFriction(), 4.0f, this->attributes);
        this->springLength = new Variant(JointComplexVehicleTireComponent::AttrSpringLength(), 0.2f, this->attributes);
        this->springConst = new Variant(JointComplexVehicleTireComponent::AttrSpringConst(), 200.0f, this->attributes);
        this->springDamp = new Variant(JointComplexVehicleTireComponent::AttrSpringDamp(), 12.0f, this->attributes);

        this->springLength->setDescription("Important value, if too low, no raycast will hit the object (like floor) below and vehicle tires will not rotate. Adjust this value until everything works.");

        this->lateralFriction->setDescription("It acts to counter the centrifugal force acting on the tire in lateral direction.");
        this->longitudinalFriction->setDescription("It acts in the rolling direction of the tires of the vehicle.");

        this->targetId->setVisible(false);
    }

    JointComplexVehicleTireComponent::~JointComplexVehicleTireComponent()
    {
    }

    bool JointComplexVehicleTireComponent::init(xml_node<>*& propertyElement)
    {
        JointComponent::init(propertyElement);
    ...
    bool JointComplexVehicleTireComponent::postInit(void)
    {
        JointComponent::postInit();

        this->gameObjectPtr->setDynamic(true);
        this->gameObjectPtr->getAttribute(GameObject::AttrDynamic())->setVisible(false);
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[JointComplexVehicleTireComponent] Init joint complex vehicle tire component for game object: " + this->gameObjectPtr->getName());

        return true;
    }

    void JointComplexVehicleTireComponent::onRemoveComponent(void)
    {
        JointComponent::onRemoveComponent();
    ...
    bool JointComplexVehicleTireComponent::connect(void)
    {
        bool success = JointComponent::connect();
    ...
    bool JointComplexVehicleTireComponent::disconnect(void)
    {
        bool success = JointComponent::disconnect();
    ...
    Ogre::String JointComplexVehicleTireComponent::getClassName(void) const
    {
        return "JointComplexVehicleTireComponent";
    }

    Ogre::String JointComplexVehicleTireComponent::getParentClassName(void) const
    {
        return "JointComponent";
    }

    void JointComplexVehicleTireComponent::update(Ogre::Real dt, bool notSimulating)
    {
        if (nullptr != this->joint)
        {
            // nothing special here – ND4 ComplexVehicle handles tire simulation
        }
    }

    GameObjectCompPtr JointComplexVehicleTireComponent::clone(GameObjectPtr clonedGameObjectPtr)
    {
        JointComplexVehicleTireCompPtr clonedJointCompPtr(boost::make_shared<JointComplexVehicleTireComponent>());
    ...
    bool JointComplexVehicleTireComponent::createJoint(const Ogre::Vector3& customJointPosition)
    {
        if (false == JointComponent::createJoint(customJointPosition))
        {
            return true;
        ...
        if (Ogre::Vector3::ZERO == this->pin->getVector3())
        {
            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL, "[JointComplexVehicleTireComponent] Cannot create joint for: " + gameObjectName + " because the pin is zero.");
            return false;
        }
        ...
        if (this->predecessorJointCompPtr)
        {
            ...
            {
                Ogre::LogManager::getSingleton().logMessage(Ogre::LML_TRIVIAL,
                    "[JointComplexVehicleTireComponent] Creating complex vehicle tire joint for body1 name: " + this->predecessorJointCompPtr->getOwner()->getName() + " body2 name: " + gameObjectName);
            }
        ...
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[JointComplexVehicleTireComponent] Cannot create joint for: " + gameObjectName + " because there is no predecessor.");
            return false;
        }

        this->internalReleaseJoint();

        auto physicsActiveComplexVehicleCompPtr = NOWA::makeStrongPtr(this->getOwner()->getComponent<PhysicsActiveComplexVehicleComponent>());
        if (nullptr == physicsActiveComplexVehicleCompPtr)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[JointComplexVehicleTireComponent] Cannot create joint for GameObject " + this->getOwner()->getName() + " because there is no complex vehicle predecessor.");
            return false;
        }

        // Decide axle and side for this tire based on attributes
        OgreNewt::ComplexVehicleTire::Axle axle = OgreNewt::ComplexVehicleTire::axleRear;
        if (this->tireSteer->getListSelectedValue() == "Steer")
        {
            axle = OgreNewt::ComplexVehicleTire::axleFront;
        }

        OgreNewt::ComplexVehicleTire::Side side = OgreNewt::ComplexVehicleTire::sideRight;
        if (this->tireSide->getListSelectedValue() == "Tire Side Left")
        {
            side = OgreNewt::ComplexVehicleTire::sideLeft;
        }

        Ogre::Real radius = this->getOwner()->getSize().y / 2.0f;

        // Create ComplexVehicleTire joint instead of VehicleTire
        this->joint = new OgreNewt::ComplexVehicleTire(this->body, predecessorBody, this->jointPosition, this->pin->getVector3(), physicsActiveComplexVehicleCompPtr->getComplexVehicle(), axle, side, radius);
        ...
        if (complexTire)
        {
            // give spring mass some weight
            complexTire->setSmass(this->body->getMass());
        ...
        if (nullptr != this->gameObjectPtr)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[JointComplexVehicleTireComponent] Created joint: " + gameObjectName);
        }
    ...
    void JointComplexVehicleTireComponent::forceShowDebugData(bool activate)
    {
        this->internalShowDebugData(activate, 0, this->anchorPosition->getVector3(), this->pin->getVector3());
    }

    void JointComplexVehicleTireComponent::actualizeValue(Variant* attribute)
    {
        JointComponent::actualizeValue(attribute);

        if (JointComplexVehicleTireComponent::AttrAnchorPosition() == attribute->getName())
        {
            this->setAnchorPosition(attribute->getVector3());
        }
        else if (JointComplexVehicleTireComponent::AttrPin() == attribute->getName())
        {
            this->setPin(attribute->getVector3());
        }
        else if (JointComplexVehicleTireComponent::AttrTireSide() == attribute->getName())
        {
            this->setVehicleTireSide(attribute->getListSelectedValue());
        }
        else if (JointComplexVehicleTireComponent::AttrTireSteer() == attribute->getName())
        {
            this->setVehicleTireSteer(attribute->getListSelectedValue());
        }
        else if (JointComplexVehicleTireComponent::AttrSteerSide() == attribute->getName())
        {
            this->setVehicleSteerSide(attribute->getListSelectedValue());
        }
        else if (JointComplexVehicleTireComponent::AttrTireAccel() == attribute->getName())
        {
            this->setVehicleTireAccel(attribute->getListSelectedValue());
        }
        else if (JointComplexVehicleTireComponent::AttrBrakeMode() == attribute->getName())
        {
            this->setVehicleTireBrake(attribute->getListSelectedValue());
        }
        else if (JointComplexVehicleTireComponent::AttrLateralFriction() == attribute->getName())
        {
            this->setLateralFriction(attribute->getReal());
        }
        else if (JointComplexVehicleTireComponent::AttrLongitudinalFriction() == attribute->getName())
        {
            this->setLongitudinalFriction(attribute->getReal());
        }
        else if (JointComplexVehicleTireComponent::AttrSpringLength() == attribute->getName())
        {
            this->setSpringLength(attribute->getReal());
        }
        else if (JointComplexVehicleTireComponent::AttrSpringConst() == attribute->getName())
        {
            this->setSpringConst(attribute->getReal());
        }
        else if (JointComplexVehicleTireComponent::AttrSpringDamp() == attribute->getName())
        {
            this->setSpringDamp(attribute->getReal());
        }
    }

    void JointComplexVehicleTireComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
    {
        JointComponent::writeXML(propertiesXML, doc);
    ...
    Ogre::Vector3 JointComplexVehicleTireComponent::getUpdatedJointPosition(void)
    {
        if (nullptr == this->body)
        {
            return Ogre::Vector3::ZERO;
    ...
    void JointComplexVehicleTireComponent::setAnchorPosition(const Ogre::Vector3& anchorPosition)
    {
        this->anchorPosition->setValue(anchorPosition);
        ...
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointComplexVehicleTireComponent::setAnchorPosition debugGeometryNode");
        }
    }

    Ogre::Vector3 JointComplexVehicleTireComponent::getAnchorPosition(void) const
    {
        return this->anchorPosition->getVector3();
    }

    void JointComplexVehicleTireComponent::setPin(const Ogre::Vector3& pin)
    {
        this->pin->setValue(pin);
        if (nullptr != this->debugGeometryNode)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "JointComplexVehicleTireComponent::setPin debugGeometryNode");
        }
    }

    Ogre::Vector3 JointComplexVehicleTireComponent::getPin(void) const
    {
        return this->pin->getVector3();
    }

    void JointComplexVehicleTireComponent::setVehicleTireSide(const Ogre::String& tireSide)
    {
        OgreNewt::VehicleTireSide vehicleTireSide;
        if (tireSide == "Tire Side Right")
        {
    ...
    Ogre::String JointComplexVehicleTireComponent::getVehicleTireSide(void)
    {
        return this->tireSide->getListSelectedValue();
    }

    void JointComplexVehicleTireComponent::setVehicleTireSteer(const Ogre::String& tireSteer)
    {
        OgreNewt::VehicleTireSteer vehicleTireSteer;
        if (tireSteer == "No Steer")
        {
    ...
    Ogre::String JointComplexVehicleTireComponent::getVehicleTireSteer(void)
    {
        return this->tireSteer->getListSelectedValue();
    }

    void JointComplexVehicleTireComponent::setVehicleSteerSide(const Ogre::String& steerSide)
    {
        OgreNewt::VehicleSteerSide vehicleSteerSide;
        if (steerSide == "Steer Side Right")
        {
    ...
    Ogre::String JointComplexVehicleTireComponent::getVehicleSteerSide(void)
    {
        return this->steerSide->getListSelectedValue();
    }

    void JointComplexVehicleTireComponent::setVehicleTireAccel(const Ogre::String& tireAccel)
    {
        OgreNewt::VehicleTireAccel vehicleTireAccel;
        if (tireAccel == "No Accel")
        {
    ...
    Ogre::String JointComplexVehicleTireComponent::getVehicleTireAccel(void)
    {
        return this->tireAccel->getListSelectedValue();
    }

    void JointComplexVehicleTireComponent::setVehicleTireBrake(const Ogre::String& brakeMode)
    {
        OgreNewt::VehicleTireBrake vehicleTireBrake;
        if (brakeMode == "No Brake")
        {
    ...
    Ogre::String JointComplexVehicleTireComponent::getVehicleTireBrake(void)
    {
        return this->brakeMode->getListSelectedValue();
    }

    void JointComplexVehicleTireComponent::setLateralFriction(Ogre::Real lateralFriction)
    {
        this->lateralFriction->setValue(lateralFriction);
    ...
    Ogre::Real JointComplexVehicleTireComponent::getLateralFriction(void) const
    {
        return this->lateralFriction->getReal();
    }

    void JointComplexVehicleTireComponent::setLongitudinalFriction(Ogre::Real longitudinalFriction)
    {
        this->longitudinalFriction->setValue(longitudinalFriction);
    ...
    Ogre::Real JointComplexVehicleTireComponent::getLongitudinalFriction(void) const
    {
        return this->longitudinalFriction->getReal();
    }

    void JointComplexVehicleTireComponent::setSpringLength(Ogre::Real springLength)
    {
        this->springLength->setValue(springLength);
    ...
    Ogre::Real JointComplexVehicleTireComponent::getSpringLength(void) const
    {
        return this->springLength->getReal();
    }

    void JointComplexVehicleTireComponent::setSpringConst(Ogre::Real springConst)
    {
        this->springConst->setValue(springConst);
    ...
    Ogre::Real JointComplexVehicleTireComponent::getSpringConst(void) const
    {
        return this->springConst->getReal();
    }

    void JointComplexVehicleTireComponent::setSpringDamp(Ogre::Real springDamp)
    {
        this->springDamp->setValue(springDamp);
    ...
    Ogre::Real JointComplexVehicleTireComponent::getSpringDamp(void) const
    {
        return this->springDamp->getReal();
    }
