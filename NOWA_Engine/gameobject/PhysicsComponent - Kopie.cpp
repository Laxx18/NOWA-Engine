...
namespace NOWA
{
	...
	PhysicsComponent::PhysicsComponent()
		...
		volume(0.0f),
		position(new Variant(PhysicsComponent::AttrPosition(), Ogre::Vector3::ZERO, this->attributes)),
		scale(new Variant(PhysicsComponent::AttrScale(), Ogre::Vector3::UNIT_SCALE, this->attributes)),
		orientation(new Variant(PhysicsComponent::AttrOrientation(), Ogre::Vector3::ZERO, this->attributes)),
	...
	{
		// Transform control via physics component instead of game object, because physics has highest prio
	...
	void PhysicsComponent::translate(const Ogre::Vector3& relativePosition)
	{
		...
		this->position->setValue(this->physicsBody->getPosition() + relativePosition);
		if (nullptr != this->physicsBody)
	...
	{
		this->orientation->setValue(MathHelper::getInstance()->quatToDegrees(this->getOrientation() * relativeRotation));
