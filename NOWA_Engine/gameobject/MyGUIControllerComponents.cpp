...
namespace NOWA
{
    ...
    void MyGUIControllerComponent::onActivatedChanged(bool activated)
    {
        // Base no-op: subclasses override to start/stop their specific MyGUI controller item
    }
    ...
    void MyGUIControllerComponent::activateController(bool bActivate)
    {
        if (nullptr != this->sourceWidget)
        {
            ...
            {
                MyGUI::IntCoord coord(static_cast<int>(this->oldCoordinate.x), static_cast<int>(this->oldCoordinate.y), static_cast<int>(this->oldCoordinate.z), static_cast<int>(this->oldCoordinate.w));
                ...
                if (this->sourceWidget->getCoord() != coord)
                {
                    ENQUEUE_RENDER_COMMAND("MyGUIControllerComponent::activateController", {
                        ...
                        MyGUI::FloatCoord oldRelativeCoord =
                            MyGUI::CoordConverter::convertToRelative(MyGUI::IntCoord(this->oldCoordinate.x, this->oldCoordinate.y, this->oldCoordinate.z, this->oldCoordinate.w), MyGUI::RenderManager::getInstance().getViewSize());
    ...
    {
        MyGUIControllerComponent::connect();
        ...
        // activated=false at connect time: sourceWidget resolved by base, Lua triggers later
        if (true == this->activated->getBool() && nullptr != this->sourceWidget)
        {
            this->onActivatedChanged(true);
        }
    ...
    bool MyGUIFadeAlphaControllerComponent::connect(void)
    {
        ...
        if (nullptr == this->controllerItem)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(cmd), "MyGUIFadeAlphaControllerComponent::connect");
        }
        ...
        if (nullptr != this->sourceWidget)
        {
            ...
            {
                // activated=false at connect: source widget must start fully invisible.
                // Store original alpha so disconnect can restore it, then set alpha=0.
                NOWA::GraphicsModule::RenderCommand cmd = [this]()
                {
                    this->oldAlpha = this->sourceWidget->getAlpha();
                    this->sourceWidget->setAlpha(0.0f);
                };
                NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(cmd), "MyGUIFadeAlphaControllerComponent::connect_hide");
            }
        }
    ...
    void MyGUIFadeAlphaControllerComponent::controllerFinished(MyGUI::Widget* sender, MyGUI::ControllerItem* controller)
    {
        ...
        // Always reset so Lua can re-trigger: setAlpha(X); setActivated(true)
        this->activated->setValue(false);

        // If we faded OUT (target alpha == 0): also deactivate the source widget
        // so it does not receive mouse events while invisible.
        // If we faded IN (target alpha > 0): leave the source widget active.
        if (nullptr != this->sourceMyGUIComponent && this->alpha->getReal() < 0.01f)
        {
            this->sourceMyGUIComponent->setActivated(false);
        }
    }

    void MyGUIFadeAlphaControllerComponent::onActivatedChanged(bool activated)
    {
        if (nullptr == this->sourceWidget)
        {
            return;
        ...
        if (true == activated)
        {
            ...
            NOWA::GraphicsModule::RenderCommand cmd = [this]()
            {
                ...
                this->controllerItem->castType<MyGUI::ControllerFadeAlpha>()->setAlpha(targetAlpha);
                this->controllerItem->castType<MyGUI::ControllerFadeAlpha>()->setCoef(this->coefficient->getReal());

                MyGUI::ControllerManager::getInstance().addItem(this->sourceWidget, this->controllerItem);
            };
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(cmd), "MyGUIFadeAlphaControllerComponent::onActivatedChanged_start");
        }
        else
        {
    ...
    bool MyGUIScrollingMessageControllerComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
        ...
        for (size_t i = 0; i < this->messages.size(); i++)
        {
            if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == "Message" + Ogre::StringConverter::toString(i))
            {
                ...
                {
                    this->messages[i] = new Variant(MyGUIScrollingMessageControllerComponent::AttrMessage() + Ogre::StringConverter::toString(i), XMLConverter::getAttrib(propertyElement, "data"), this->attributes);
                }
                else
                {
    ...
    void MyGUIScrollingMessageControllerComponent::onActivatedChanged(bool activated)
    {
        if (nullptr == this->sourceWidget)
        {
            return;
        ...
        if (true == activated)
        {
            ...
            ENQUEUE_RENDER_COMMAND("MyGUIScrollingMessageControllerComponent::onActivatedChanged_start", {
                for (size_t i = 0; i < this->messages.size(); i++)
                {
                    ...
                    this->editBoxes[i] =
                        MyGUI::Gui::getInstancePtr()->createWidgetReal<MyGUI::EditBox>("EditBoxEmpty", coord.left, coord.top + 0.1f * i, coord.width, coord.height, this->sourceWidget->getAlign(), this->sourceWidget->getLayer()->getName());

                    this->editBoxes[i]->setColour(MyGUI::Colour(this->sourceMyGUIComponent->getColor().x, this->sourceMyGUIComponent->getColor().y, this->sourceMyGUIComponent->getColor().z, this->sourceMyGUIComponent->getColor().w));
            ...
            if (true == this->justAdded)
            {
                auto closureFunction = [this](Ogre::Real renderDt)
                {
                    ...
                    if (this->timeToGo < 1e-3f)
                    {
                        this->timeToGo = 0.0f;
                        this->justAdded = false;
                        // Reset so Lua can re-trigger via setActivated(true)
                        this->activated->setValue(false);
                    }
                    ...
                    for (unsigned int i = 0; i < this->messageCount->getUInt(); i++)
                    {
                        ...
                        this->editBoxes[i]->setColour(MyGUI::Colour(this->editBoxes[i]->getTextColour().red, this->editBoxes[i]->getTextColour().green, this->editBoxes[i]->getTextColour().blue, 1.0f - progress /* / 4*/));
    ...
    void MyGUIEdgeHideControllerComponent::onActivatedChanged(bool activated)
    {
        if (nullptr == this->sourceWidget)
        {
            return;
    ...
    {
        // RepeatClick::eventPostAction fires on EVERY repeat click (it is a continuous controller).
        // Do NOT deactivate here — the controller keeps running until setActivated(false) is called.
    }

    void MyGUIRepeatClickControllerComponent::onActivatedChanged(bool activated)
    {
        if (nullptr == this->sourceWidget)
        {
            return;
