	...
	*/
	...
	void PhysicsRagDollComponent::update(Ogre::Real dt, bool notSimulating)
	{
		if (true == activated->getBool() && false == notSimulating)
		{
			...
			else */if (this->rdState == PhysicsRagDollComponent::PARTIAL_RAGDOLLING || this->rdState == PhysicsRagDollComponent::RAGDOLLING)
			{
	...
	bool PhysicsRagDollComponent::createRagDollFromXML(rapidxml::xml_node<>* rootXmlElement, const Ogre::String& boneName)
	{
		...
		while (nullptr != jointXmlElement)
		{
			...
			else if (strJointType == JointUniversalActuatorComponent::getStaticClassName())
			{
				...
				{
					Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[PhysicsRagDollComponent] Error: Pin for universal actuator joint is missing in "
						+ this->boneConfigFile->getString() + " for bone: '" + childRagBone->getName()
						+ "' for game object " + this->gameObjectPtr->getName());
					return false;
				}
		...
		const Ogre::Degree& maxTwistAngle2, Ogre::Real friction, bool useSpring, const Ogre::Vector3& offset)
	{
		...
		switch(type)
		{
			...
			{
				JointUniversalActuatorCompPtr tempChildCompPtr = boost::dynamic_pointer_cast<JointUniversalActuatorComponent>(
					childRagBone->createJointComponent(JointUniversalActuatorComponent::getStaticClassName()));
		...
		if (nullptr != childJointCompPtr)
		{
			if (nullptr != parentRagBone)
			{
				Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[PhysicsRagDollComponent]: Name: " + this->gameObjectPtr->getName()
					...
					+ " between child: " + childRagBone->getName() 
					+ " and parent: " + parentRagBone->getName()
					+ ", joint pos: " + Ogre::StringConverter::toString(parentJointCompPtr->getBody()->getPosition()) 
					+ ", joint ori: " + Ogre::StringConverter::toString(parentJointCompPtr->getBody()->getOrientation())
