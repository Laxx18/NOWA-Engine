...
namespace NOWA
{
	...
	public:
		...
		virtual void execute(size_t threadId, size_t numThreads) override
		{
			...
			// Make sure all threads finish before marking as completed
#pragma omp barrier
	...
	void SpawnComponent::onRemoveComponent(void)
	{
		...
		while (false == this->pendingTasks.empty())
		{
			delete this->pendingTasks.front();
			...
			if (this->currentCount <= this->count->getUInt())
			{
				...
				if (this->spawnTimer >= this->interval->getUInt())
				{
					...
					if (this->currentCount <= this->count->getUInt())
					{
						...
						SpawnCloneTask* task = new SpawnCloneTask(this->spawnTargetGameObject, position, orientation, this->cloneDatablock->getBool());
						...
						this->gameObjectPtr->getSceneManager()->executeUserScalableTask(task, true);
					}
				}

				{
					...
					std::queue<SpawnCloneTask*> remainingTasks;
					while (false == this->pendingTasks.empty())
					{
						SpawnCloneTask* task = this->pendingTasks.front();
						this->pendingTasks.pop();

						if (task->isCompleted())
						{
							GameObjectPtr clonedGameObjectPtr = task->getClonedGameObject();

							if (nullptr != clonedGameObjectPtr)
							{
								// Add to completed clones for processing in the main thread
								this->completedClones.push(clonedGameObjectPtr);
							}
							else
							{
								Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
									"[SpawnComponent] The game object: " + this->gameObjectPtr->getName() +
									" could not be spawned because the clone operation delivered null!");
							}
						...
						{
							// Task is not completed, keep it for next frame
							remainingTasks.push(task);
						}
					}

					// Update the pending tasks queue
					this->pendingTasks = std::move(remainingTasks);

					// Process completed clones
					while (false == this->completedClones.empty())
					{
						GameObjectPtr clonedGameObjectPtr = this->completedClones.front();
						this->completedClones.pop();
