...
namespace NOWA
{
	...
	bool SimpleSoundComponent::createSound(void)
	{
		...
		ENQUEUE_RENDER_COMMAND_MULTI("BackgroundScrollComponent::createSound", _1(loop),
		{
	...
	void SimpleSoundComponent::destroySound(void)
	{
		if (nullptr != this->sound)
		{
			ENQUEUE_RENDER_COMMAND("BackgroundScrollComponent::createSound",
			{
		...
		if (true == this->bIsInSimulation && true == this->playWhenInMotion->getBool())
		{
			...
			if (true == canPlay)
			{
				...
				else
				{
					this->createSound();
					// Wait?
					ENQUEUE_RENDER_COMMAND("BackgroundScrollComponent::update",
					{
						this->sound->play();
					});
			...
			else
			{
				if (nullptr != this->sound)
				{
					ENQUEUE_RENDER_COMMAND("BackgroundScrollComponent::update2",
					{
	...
	void SimpleSoundComponent::setActivated(bool activated)
	{
		...
		if (false == activated)
		{
			if (nullptr != this->sound)
			{
				ENQUEUE_RENDER_COMMAND("BackgroundScrollComponent::setActivated",
				{
					this->sound->stop();
				});
			}
	...
	void SimpleSoundComponent::setRelativeToListener(bool relativeToListener)
	{
		...
		if (nullptr != this->sound)
		{
			ENQUEUE_RENDER_COMMAND_MULTI("BackgroundScrollComponent::setRelativeToListener", _1(relativeToListener),
			{
		...
		OgreAL::AudioProcessor::SpectrumPreparationType spectrumPreparationType, Ogre::Real smoothFactor)
	{
		if (nullptr != this->sound)
		{
			ENQUEUE_RENDER_COMMAND_MULTI("BackgroundScrollComponent::setRelativeToListener", _6(enable, processingSize, numberOfBands, windowType, spectrumPreparationType, smoothFactor),
			{
	...
	void SimpleSoundComponent::setupSound(void)
	{
		...
		ENQUEUE_RENDER_COMMAND("BackgroundScrollComponent::setupSound",
		{
