...
namespace NOWA
{
        ...
        cachedBrake(0.0f)
    {
        ...
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[PhysicsActiveVehicleComponent] Physics active vehicle cannot be controlled, because this game object: " + owner->getName() + " has no lua script component.");
        }
    }
    ...
    Ogre::Real PhysicsActiveVehicleComponent::PhysicsVehicleCallback::callDrivingFunction(const Ogre::String& functionName, std::atomic<Ogre::Real>& cachedResult, Ogre::Real dt, Ogre::Real (VehicleDrivingManipulation::*resultGetter)(void) const)
    {
        ...
        // Only plain values cross the thread boundary, no pointer into this callback.
        LuaScript* const capturedLuaScript = this->luaScript;
        const Ogre::String capturedFunctionName = functionName;
        const Ogre::Real capturedDt = dt;
        std::atomic<Ogre::Real>* const capturedCache = &cachedResult;

        NOWA::AppStateManager::LogicCommand logicCommand = [capturedLuaScript, capturedFunctionName, capturedDt, capturedCache, resultGetter]()
        {
            ...
            // every call. Lua receives a raw pointer as usual and must not hold on to it
            // past the call.
            VehicleDrivingManipulation drivingManipulation;
            ...
            capturedLuaScript->callTableFunction(capturedFunctionName, &drivingManipulation, capturedDt);
    ...
    {
        return this->callDrivingFunction(this->onSteerAngleChangedFunctionName, this->cachedSteerAngle, dt, &VehicleDrivingManipulation::getSteerAngle);
    }
    ...
    {
        return this->callDrivingFunction(this->onMotorForceChangedFunctionName, this->cachedMotorForce, dt, &VehicleDrivingManipulation::getMotorForce);
    }
    ...
    {
        return this->callDrivingFunction(this->onHandBrakeChangedFunctionName, this->cachedHandBrake, dt, &VehicleDrivingManipulation::getHandBrake);
    }
    ...
    {
        return this->callDrivingFunction(this->onBrakeChangedFunctionName, this->cachedBrake, dt, &VehicleDrivingManipulation::getBrake);
    }

    void PhysicsActiveVehicleComponent::PhysicsVehicleCallback::onTireContact(const OgreNewt::RayCastTire* tire, const Ogre::String& tireName, OgreNewt::Body* hitBody, const Ogre::Vector3& contactPosition, const Ogre::Vector3& contactNormal,
        Ogre::Real penetration)
    {
        // hitBody was dereferenced without a null check.
        if (nullptr == hitBody)
        {
            return;
        }
        ...
        if (nullptr == this->luaScript || false == this->luaScript->isCompiled() || true == this->onTireContactFunctionName.empty())
        {
            return;
        }

        // Deferred like the four driving callbacks: this runs on a newton worker and
        // lua_State must only be touched from the logic thread. No return value is needed
        // here, so there is nothing to cache.
        LuaScript* const capturedLuaScript = this->luaScript;
        const Ogre::String capturedFunctionName = this->onTireContactFunctionName;
        const Ogre::String capturedTireName = tireName;
        const Ogre::Vector3 capturedContactPosition = contactPosition;
        const Ogre::Vector3 capturedContactNormal = contactNormal;
        const Ogre::Real capturedPenetration = penetration;

        // Kept alive as a shared pointer until the call has happened - the hit game object
        // may well be destroyed between the contact and the command running. Lua still gets
        // a raw pointer.
        GameObjectPtr hitGameObjectPtr = hitPhysicsComponent->getOwner();
        if (nullptr == hitGameObjectPtr)
        {
            return;
        }

        NOWA::AppStateManager::LogicCommand logicCommand = [capturedLuaScript, capturedFunctionName, capturedTireName, hitGameObjectPtr, capturedContactPosition, capturedContactNormal, capturedPenetration]()
        {
            if (nullptr == capturedLuaScript || false == capturedLuaScript->isCompiled())
            {
                return;
            ...
            auto hitPhysicsCompPtr = NOWA::makeStrongPtr(hitGameObjectPtr->getComponent<PhysicsComponent>());
            if (nullptr == hitPhysicsCompPtr)
            {
                return;
            }

            capturedLuaScript->callTableFunction(capturedFunctionName, capturedTireName, hitPhysicsCompPtr.get(), capturedContactPosition, capturedContactNormal, capturedPenetration);
        };
        NOWA::AppStateManager::getSingletonPtr()->enqueue(std::move(logicCommand));
    }
    ...
    bool PhysicsActiveVehicleComponent::postInit(void)
    {
        ...
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[PhysicsActiveVehicleComponent] Init physics active vehicle component for game object: " + this->gameObjectPtr->getName());
    ...
    bool PhysicsActiveVehicleComponent::createDynamicBody(void)
    {
        ...
        this->physicsBody = new OgreNewt::Vehicle(this->ogreNewt, this->gameObjectPtr->getSceneManager(), this->gameObjectPtr->getDefaultDirection(), collisionPtr, weightedMass, this->collisionPosition->getVector3(), this->massOrigin->getVector3(),
            this->gravity->getVector3(),
            new PhysicsVehicleCallback(this->gameObjectPtr.get(), this->gameObjectPtr->getLuaScript(), this->ogreNewt, this->onSteerAngleChangedFunctionName->getString(), this->onMotorForceChangedFunctionName->getString(),
