...
namespace NOWA
{
	...
	bool MeshDecalComponent::init(rapidxml::xml_node<>*& propertyElement)
	{
...
// Attention: Is that correct?
	...
	void MeshDecalComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
	{
		// 2 = int
		// 6 = real
		// 7 = string
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->updateFrequency->getReal())));
		propertiesXML->append_node(propertyXML);
	}
