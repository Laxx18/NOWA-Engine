...
namespace NOWA
{
    ...
    DatablockPbsComponent::DatablockPbsComponent() :
        ...
        brdf(new Variant(DatablockPbsComponent::AttrBrdf(),
            {"Default", "CookTorrance", "BlinnPhong", "DefaultUncorrelated", "DefaultSeparateDiffuseFresnel", "CookTorranceSeparateDiffuseFresnel", "BlinnPhongSeparateDiffuseFresnel", "BlinnPhongLegacyMath", "BlinnPhongFullLegacy"},
            this->attributes)),
        ...
        alphaTest(new Variant(DatablockPbsComponent::AttrAlphaTest(),
            {
                "CMPF_ALWAYS_FAIL",
                "CMPF_ALWAYS_PASS",
                "CMPF_LESS",
        ...
        blendMode0(new Variant(DatablockPbsComponent::AttrBlendMode0(),
            {"PBSM_BLEND_NORMAL_NON_PREMUL", "PBSM_BLEND_NORMAL_PREMUL", "PBSM_BLEND_ADD", "PBSM_BLEND_SUBTRACT", "PBSM_BLEND_MULTIPLY", "PBSM_BLEND_MULTIPLY2X", "PBSM_BLEND_SCREEN", "PBSM_BLEND_OVERLAY", "PBSM_BLEND_LIGHTEN", "PBSM_BLEND_DARKEN",
    ...
    {
        this->subItemIndex->setDescription("Specifies the sub item index, for which the datablock should be shown.");
    ...
    Ogre::PbsBrdf::PbsBrdf DatablockPbsComponent::mapStringToBrdf(const Ogre::String& strBrdf)
    {
        ...
        else if ("DefaultSeparateDiffuseFresnel" == strBrdf)
        {
            ...
            brdf |= Ogre::PbsBrdf::FLAG_HAS_DIFFUSE_FRESNEL | Ogre::PbsBrdf::FLAG_SPERATE_DIFFUSE_FRESNEL;
        }
    ...
    bool DatablockPbsComponent::readDatablockItem(Ogre::Item* item)
    {
        ...
        if (false == this->alreadyCloned && false == this->isCloned && nullptr != this->originalDatablock)
        {
            ...
            Ogre::HlmsDatablock* clonedDatablock = NOWA::AppStateManager::getSingletonPtr()->getGameObjectController()->cloneDatablockUnique(this->originalDatablock, originalDatablockName, this->gameObjectPtr->getId(), this->oldSubIndex);
        ...
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[DatablockPbsComponent] Datablock reading failed, because there is no data block for game object: " + this->gameObjectPtr->getName());
            return false;
        }
    ...
    void DatablockPbsComponent::postReadDatablock(void)
    {
        ...
        */
        ...
        this->backgroundColor->setValue(Ogre::Vector4(this->datablock->getBackgroundDiffuse().r, this->datablock->getBackgroundDiffuse().g, this->datablock->getBackgroundDiffuse().b, this->datablock->getBackgroundDiffuse().a));
        ...
        NOWA::GraphicsModule::RenderCommand renderCommand = [this, pbsTextureType, textureType, attribute, textureName]()
        {
            ...
            if (Ogre::PBSM_REFLECTION != pbsTextureType && false == Ogre::ResourceGroupManager::getSingleton().resourceExistsInAnyGroup(oldTextureName))
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[DatablockPbsComponent] Cannot set texture: '" + oldTextureName + "', because it does not exist in any resource group! for game object: " + this->gameObjectPtr->getName());
    ...
    {
        NOWA::GraphicsModule::RenderCommand renderCommand = [this, pbsTextureType, texture]()
        {
            if (nullptr == this->datablock)
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[DatablockPbsComponent] setTextureDirectly: datablock is null for game object: " + this->gameObjectPtr->getName());
                return;
            }

            this->datablock->setTexture(pbsTextureType, texture);

            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
                "[DatablockPbsComponent] setTextureDirectly: slot " + Ogre::StringConverter::toString(static_cast<int>(pbsTextureType)) + " set directly for game object: " + this->gameObjectPtr->getName());
        };
    ...
    void DatablockPbsComponent::setFresnel(const Ogre::Vector3& fresnel, bool separateFresnel)
    {
        ...
        if (nullptr != this->datablock)
        {
            ...
            {
                this->fresnel->setValue(Ogre::Vector4(this->datablock->getFresnel().x, this->datablock->getFresnel().y, this->datablock->getFresnel().z, static_cast<Ogre::Real>(this->datablock->hasSeparateFresnel())));
            }
        }
    }
        ...
        if (nullptr != this->datablock)
        {
            if (false == newlyCreated || true == this->isCloned)
            {
                ...
                */
                ...
                22:09:33: OGRE EXCEPTION(1:InvalidStateException): Renderable needs at least 2 coordinates in UV set #1. Either change the mesh, or change the UV source settings in HlmsPbs::calculateHashForPreCreate at
                D:\Ogre\GameEngineDevelopment\external\Ogre2.1SDK\Components\Hlms\Pbs\src\OgreHlmsPbs.cpp (line 649) 22:09:33: OGRE EXCEPTION(1:InvalidStateException): Renderable needs at least 2 coordinates in UV set #1. Either change the mesh, or
                change the UV source settings in HlmsPbs::calculateHashForPreCreate at D:\Ogre\GameEngineDevelopment\external\Ogre2.1SDK\Components\Hlms\Pbs\src\OgreHlmsPbs.cpp (line 649) 22:09:33: Couldn't apply change to datablock '' for this
                renderable. Using default one. Check previous log messages to see if there's more information.
                */
            }
