...
namespace NOWA
{
	...
	public:
		TagPointListener(Ogre::Node* realNode, const Ogre::Vector3& offsetPosition, const Ogre::Quaternion& offsetOrientation, 
			PhysicsActiveComponent* sourcePhysicsActiveComponent, GameObject* sourceGameObject, GameObject* targetGameObject)
			: realNode(realNode),
			...
			jointKinematicComponent(nullptr)
		{
			if (nullptr != sourcePhysicsActiveComponent)
			{
				...
				if (nullptr == existingJointKinematicCompPtr)
				{
					...
					this->sourcePhysicsActiveComponent->getOwner()->addComponent(jointKinematicCompPtr);
					...
					jointKinematicCompPtr->setPickingMode(2);
					jointKinematicCompPtr->setMaxLinearAngleFriction(100.0f, 0.1f);
		...
		virtual void OldNodeUpdated(const Ogre::v1::OldNode* updatedNode) override
		{
			...
			else
			{
				...
				Ogre::Node* parentNode = this->targetGameObject->getMovableObject()->getParentNode();
				Ogre::SceneNode* sceneNode = this->targetGameObject->getMovableObject()->getParentSceneNode();
				...
				// Ogre::Quaternion newOrientation = worldOrientation * this->offsetOrientation.Inverse();
				// Ogre::Vector3 newPosition = worldPosition - (worldOrientation * this->offsetPosition);

				Ogre::Quaternion newOrientation = this->sourceGameObject->getOrientation() * this->offsetOrientation.Inverse();
				Ogre::Vector3 newPosition = worldPosition - (worldOrientation * this->offsetPosition);
	...
	bool TagPointComponent::connect(void)
	{
		if (false == alreadyConnected)
		{
			...
			if (nullptr != oldSkeletonInstance)
			{
				...
				this->positionOffsetSnapshot = sourceGameObjectPtr->getPosition() - worldPosition;
				this->orientationOffsetSnapshot = sourceGameObjectPtr->getOrientation() * worldOrientation.Inverse();
				...
				this->tagPoint->setListener(new TagPointListener(this->tagPointNode, this->positionOffsetSnapshot, 
					this->orientationOffsetSnapshot, this->sourcePhysicsActiveComponent, this->gameObjectPtr.get()));
	...
	void TagPointComponent::showDebugData(void)
	{
		...
		if (true == this->bShowDebugData)
		{
			if (nullptr == this->debugGeometryArrowNode && nullptr != this->tagPoint)
			{
				...
				this->debugGeometryArrowNode->setPosition(worldPosition/* + this->gameObjectPtr->getSceneNode()->getPosition()*/);
				this->debugGeometryArrowNode->setOrientation(worldOrientation/* * this->gameObjectPtr->getSceneNode()->getOrientation()*/);
				this->debugGeometryArrowNode->setScale(0.1f, 0.1f, 0.05f);
				...
				this->debugGeometrySphereNode->setPosition(worldPosition/* + this->gameObjectPtr->getSceneNode()->getPosition()*/);
