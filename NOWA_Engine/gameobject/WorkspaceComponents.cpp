...
namespace NOWA
{
    ...
    {
    public:
        void propertiesMergedPreGenerationStep(const Ogre::String& shaderProfile, const Ogre::HlmsCache& passCache, const Ogre::HlmsPropertyVec& renderableCacheProperties, const Ogre::PiecesMap renderableCachePieces[Ogre::NumShaderTypes],
            const Ogre::HlmsPropertyVec& properties, const Ogre::QueuedRenderable& queuedRenderable)
        {
            std::stringstream ss;
            ss << "\n";
    ...
    {
    public:
        PlanarReflectionsWorkspaceListener(Ogre::PlanarReflections* planarReflections) : planarReflections(planarReflections)
        {
        }
        ...
        workspacePccProbes(nullptr)
    {
        ...
        this->useReflection
            ->setDescription("Activates reflection, which is rendered by a dynamic cubemap. This component works in conjunction with a ReflectionCameraComponent. Attention: Activating this mode decreases the application's performance considerably!");
    ...
    bool WorkspaceBaseComponent::postInit(void)
    {
        ...
        else
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                "[WorkspaceBaseComponent] Could not get prior CameraComponent. This component must be set under the CameraComponent! Affected game object: " + this->gameObjectPtr->getName());
            return false;
        }
            ...
            if (true == this->useReflection->getBool())
            {
                ...
                if (nullptr != reflectionCameraGameObjectPtr)
                {
                    ...
                    if (nullptr != reflectionCameraCompPtr)
                    {
                        ...
                        this->cubemapTexture =
                            textureManager->createOrRetrieveTexture("cubemap", Ogre::GpuPageOutStrategy::Discard, Ogre::TextureFlags::RenderToTexture | Ogre::TextureFlags::AllowAutomipmaps | iblSpecularFlag, Ogre::TextureTypes::TypeCube);
                        ...
                        this->workspaceCubemap = this->compositorManager->addWorkspace(this->gameObjectPtr->getSceneManager(), cubemapExternalChannels, reflectionCameraCompPtr->getCamera(), workspaceCubemapName, true);
                    ...
                    else
                    {
                        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
                            "[WorkspaceBaseComponent] Warning reflections will not work, because the given game object id: " + Ogre::StringConverter::toString(this->reflectionCameraGameObjectId->getULong()) + " has no " +
                                CameraComponent::getStaticClassName() + ". Affected game object : " + this->gameObjectPtr->getName());
                    }
                }
                else
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[WorkspaceBaseComponent] Warning reflections will not work, because the given game object id: " +
                                                                                           Ogre::StringConverter::toString(this->reflectionCameraGameObjectId->getULong()) + " does not exist. Affected game object : " + this->gameObjectPtr->getName());
                }
            }
...
// TODO: Check if -> and condition this->involvedInSplitScreen is necessary!
    ...
    void WorkspaceBaseComponent::removeWorkspace(void)
    {
        ...
        GraphicsModule::RenderCommand renderCommand = [this]()
        {
            ...
            if (nullptr != this->workspace)
            {
                ...
                // In ogre sample its not destroyed at all! And if i destroy it, then suddently its no more possible to load another scene, it will hang in render thread yield forever, what the fuck is going on?
                /* if (true == this->useSSAO->getBool())
                {
    ...
    void WorkspaceBaseComponent::createFinalRenderNode(void)
    {
        ...
        if (false == this->compositorManager->hasNodeDefinition(this->finalRenderingNodeName))
        {
            ...
            if (true == this->useSSAO->getBool())
            {
                ...
                depthCopyDef->format = Ogre::PFG_D32_FLOAT;
                ...
                ssaoDef->format = Ogre::PFG_R32_FLOAT;
                ...
                blurHDef->format = Ogre::PFG_R32_FLOAT;
                ...
                blurVDef->format = Ogre::PFG_R32_FLOAT;
            ...
            if (true == this->useSSAO->getBool())
            {
                ...
                // NO colourAttachments — this is a depth target!
                rtv->depthAttachment.textureName = "depthTextureCopy";
                rtv->depthBufferId = Ogre::DepthBuffer::NO_POOL_EXPLICIT_RTV;
            ...
            if (true == this->useSSAO->getBool())
            {
                // 1. Depth downsampling
                {
                    ...
                    passQuad->mMaterialName = (this->msaaLevel > 1) ? "Ogre/Depth/DownscaleMax_Subsample0" : "Ogre/Depth/DownscaleMax";
    ...
    void WorkspaceBaseComponent::initializeHdr(Ogre::uint8 fsaa)
    {
        ...
        NOWA::GraphicsModule::RenderCommand renderCommand = [this, fsaa]()
        {
            ...
            Ogre::MaterialPtr material = Ogre::MaterialManager::getSingleton().load("HDR/Resolve_4xFP32_HDR_Box", Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME).staticCast<Ogre::Material>();
    ...
    void WorkspaceBaseComponent::initializeSmaa(PresetQuality quality, EdgeDetectionMode edgeDetectionMode)
    {
        ...
        Ogre::String materialNames[3] = {"SMAA/EdgeDetection", "SMAA/BlendingWeightCalculation", "SMAA/NeighborhoodBlending"};
        ...
        case EdgeDetectionDepth:
            OGRE_EXCEPT(Ogre::Exception::ERR_NOT_IMPLEMENTED, "EdgeDetectionDepth not implemented.", "SmaaUtils::initialize");
            break;
    ...
    void WorkspaceBaseComponent::setPlanarMaxReflections(unsigned long gameObjectId, bool useAccurateLighting, unsigned int width, unsigned int height, bool withMipmaps, bool useMipmapMethodCompute, const Ogre::Vector3& position,
        const Ogre::Quaternion& orientation, const Ogre::Vector2& mirrorSize)
    {
    ...
    void WorkspaceBaseComponent::addPlanarReflectionsActor(unsigned long gameObjectId, bool useAccurateLighting, unsigned int width, unsigned int height, bool withMipmaps, bool useMipmapMethodCompute, const Ogre::Vector3& position,
        const Ogre::Quaternion& orientation, const Ogre::Vector2& mirrorSize)
    {
        if (nullptr != this->planarReflections && false == this->planarReflectionReflectiveWorkspaceName.empty())
        {
    ...
    void WorkspaceBaseComponent::createSSAONoiseTexture(void)
    {
        NOWA::GraphicsModule::RenderCommand renderCommand = [this]()
        {
            ...
            psParams->setNamedConstant("noiseScale", Ogre::Vector2((Core::getSingletonPtr()->getOgreRenderWindow()->getWidth() * 0.5f) / 2.0f, (Core::getSingletonPtr()->getOgreRenderWindow()->getHeight() * 0.5f) / 2.0f));
    ...
    void WorkspaceBaseComponent::createLocalCubemapProbeRendererNode(void)
    {
        if (false == this->compositorManager->hasNodeDefinition("LocalCubemapProbeRendererNode"))
        {
            ...
            const Ogre::uint8 cubemapFaces[6] = {Ogre::CubemapSide::PX, Ogre::CubemapSide::NX, Ogre::CubemapSide::PY, Ogre::CubemapSide::NY, Ogre::CubemapSide::PZ, Ogre::CubemapSide::NZ};
    ...
    WorkspacePbsComponent::WorkspacePbsComponent() : WorkspaceBaseComponent()
    {
    }
    ...
    void WorkspaceBackgroundComponent::changeBackground(unsigned short index, const Ogre::String& backgroundTextureName)
    {
        if (index >= 9)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[WorkspaceBackgroundComponent] Invalid layer index: " + Ogre::StringConverter::toString(index));
            return;
        }
    ...
    void WorkspaceBackgroundComponent::setBackgroundScrollSpeedX(unsigned short index, Ogre::Real speedX)
    {
        if (index >= 9)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[WorkspaceBackgroundComponent] Invalid layer index for X speed: " + Ogre::StringConverter::toString(index));
            return;
        }
    ...
    void WorkspaceBackgroundComponent::setBackgroundScrollSpeedY(unsigned short index, Ogre::Real speedY)
    {
        if (index >= 9)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[WorkspaceBackgroundComponent] Invalid layer index for Y speed: " + Ogre::StringConverter::toString(index));
            return;
        }
    ...
    WorkspaceCustomComponent::WorkspaceCustomComponent() : WorkspaceBaseComponent(), customWorkspaceName(new Variant(WorkspaceCustomComponent::AttrCustomWorkspaceName(), Ogre::String("PbsCustomWorkspace"), this->attributes))
    {
    }
