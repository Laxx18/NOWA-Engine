...
namespace NOWA
{
    ...
    public:
         ...
         */
        template <class Type> static boost::shared_ptr<Type> makeStrongPtr(boost::weak_ptr<Type> weakPtr)
        {
            if (!weakPtr.expired())
            {
    ...
    public:
        ...
        // Custom data strings
        static const Ogre::String AttrCustomDataNewCreation(void)
        {
            return "NewCreation";
        }
    ...
    public:
        /**
         * @brief		Opens a render command batch.
         * @note		While a batch is open, every enqueueRenderCommand() call is only collected
         *				instead of being dispatched. endRenderCommandBatch() then runs all of them
         *				inside ONE enqueueAndWait.
         *				Loading a component that sets many texture slots costs one blocking round
         *				trip to the render thread PER SETTER otherwise - DatablockPbsComponent
         *				alone routes 16 setters through internalSetTextureName().
         * @note		Deliberately NOT virtual. This is a mechanism, not a customisation point:
         *				a component must not be able to change HOW batching works, and an override
         *				that forgets to close the batch would silently swallow every later command.
         */
        void beginRenderCommandBatch(void);

        /**
         * @brief		Closes the batch and executes everything it collected in a single round trip.
         */
        void endRenderCommandBatch(void);

    protected:
        /**
         * @brief		Dispatches a render command, or collects it when a batch is open.
         * @param[in]	renderCommand	The command to run on the render thread.
         * @param[in]	commandName		Name for logging, used only on the direct path.
         * @note		Attention: MUST NOT be used by code that reads a result back on the calling
         *				thread (e.g. a lambda capturing a local by reference and the caller
         *				evaluating it right afterwards). A batched command has not run yet when
         *				this returns. Such call sites have to keep using enqueueAndWait directly -
         *				see DatablockPbsComponent::preReadDatablock().
         */
        ...
        template <class T> T* getPriorComponentByType(void) const
        {
        ...
        template <class T> T* getNextComponentByType(void) const
        {
            T* resultComponent = nullptr;
