...
namespace NOWA
{
    ...
    CustomPickingMask::CustomPickingMask() : width(0), height(0)
    {
    }
    ...
    void MyGUIComponent::setRealPosition(const Ogre::Vector2& position)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            GraphicsModule::RenderCommand renderCommand = [this, position]()
            {
                this->widget->setRealPosition(position.x, position.y);
            };
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComponent::setRealPosition");
    ...
    void MyGUIComponent::refreshTransform(void)
    {
        ...
        if (0 == this->parentId->getULong())
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComponent::refreshTransform");
        }
    }
    ...
    void MyGUIComponent::setAlign(const Ogre::String& align)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, align]()
            {
                this->widget->setAlign(this->mapStringToAlign(align));
                // this->widget->_setAlign(this->widget->getSize(), this->widget->getSize());
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComponent::setAlign");
        }
    }
    ...
    void MyGUIComponent::setColor(const Ogre::Vector4& color)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, color]()
            {
                this->widget->setColour(MyGUI::Colour(color.x, color.y, color.z, color.w));
                this->widget->setAlpha(color.w);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComponent::setColor");
        }
    }
    ...
    void MyGUIComponent::setEnabled(bool enabled)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, enabled]()
            {
                this->widget->setEnabled(enabled);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComponent::setEnabled");
        }
    }
    ...
    void MyGUIComponent::onRemoveComponent(void)
    {
        ...
        else
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComponent::onRemoveComponent");
        }
    }
    ...
    void MyGUIComponent::setParentId(unsigned long parentId)
    {
        ...
        if (this->id->getULong() != parentId /* && 0 != parentId*/)
        {
            ...
            GraphicsModule::RenderCommand renderCommand = [this, parentId]()
            {
                if (0 == parentId)
                {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComponent::setParentId");
        }
    ...
    MyGUIWindowComponent::MyGUIWindowComponent() : MyGUIComponent(), movable(new Variant(MyGUIWindowComponent::AttrMovable(), false, this->attributes)), windowCaption(new Variant(MyGUIWindowComponent::AttrWindowCaption(), "", this->attributes))
    {
        this->skin = new Variant(MyGUIComponent::AttrSkin(), {"WindowCSX", "Window", "WindowC", "WindowCX", "WindowCS", "WindowCX2_Dark", "WindowCXS2_Dark", "Back1Skin_Dark", "PanelSkin", "WoodPanel", "WoodWindow"}, this->attributes);
    }
    ...
    void MyGUIWindowComponent::setWindowCaption(const Ogre::String& windowCaption)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, windowCaption]()
            {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIWindowComponent::setWindowCaption");
        }
    }
    ...
    void MyGUITextComponent::onKeyButtonPressed(MyGUI::Widget* sender, MyGUI::KeyCode code, MyGUI::Char c)
    {
        ...
        {
            if (true == InputDeviceCore::getSingletonPtr()->getKeyboard()->isKeyDown(OIS::KC_LSHIFT) || true == InputDeviceCore::getSingletonPtr()->getKeyboard()->isKeyDown(OIS::KC_RSHIFT))
            {
    ...
    void MyGUITextComponent::setTextAlign(const Ogre::String& align)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, align]()
            {
                this->widget->castType<MyGUI::EditBox>()->setTextAlign(this->mapStringToAlign(align));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setTextAlign");
        }
    }
    ...
    void MyGUITextComponent::setTextOffset(const Ogre::Vector2& textOffset)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, textOffset]()
            {
                this->widget->castType<MyGUI::EditBox>()->setTextOffset(MyGUI::IntPoint(textOffset.x, textOffset.y));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setTextOffset");
        }
    }
    ...
    void MyGUITextComponent::setTextColor(const Ogre::Vector4& textColor)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, textColor]()
            {
                this->widget->castType<MyGUI::EditBox>()->setTextColour(MyGUI::Colour(textColor.x, textColor.y, textColor.z, textColor.w));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setTextColor");
        }
    }
    ...
    void MyGUITextComponent::setReadOnly(bool readOnly)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, readOnly]()
            {
                // setEditStatic: ReadOnly and text cannot be selected
                this->widget->castType<MyGUI::EditBox>()->setEditStatic(readOnly);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setReadOnly");
        }
    }
    ...
    void MyGUITextComponent::setMultiLine(bool multiLine)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, multiLine]()
            {
                this->widget->castType<MyGUI::EditBox>()->setEditMultiLine(multiLine);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setMultiLine");
    ...
    void MyGUITextComponent::setWordWrap(bool wordWrap)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, wordWrap]()
            {
                this->widget->castType<MyGUI::EditBox>()->setEditWordWrap(wordWrap);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setWordWrap");
        }
    }
    ...
    bool MyGUIButtonComponent::postInit(void)
    {
        ...
        if (true == this->createWidgetInParent)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIButtonComponent::postInit");
        }
    ...
    void MyGUIButtonComponent::setCaption(const Ogre::String& caption)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            {
                this->widget->castType<MyGUI::Button>()->setCaptionWithReplacing(caption);
            };
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIButtonComponent::setCaption");
        }
    }
    ...
    void MyGUIButtonComponent::setFontHeight(unsigned int fontHeight)
    {
        ...
        GraphicsModule::RenderCommand renderCommand = [this, fontHeight]()
        {
    ...
    void MyGUIButtonComponent::setTextAlign(const Ogre::String& align)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, align]()
            {
                this->widget->castType<MyGUI::TextBox>()->setTextAlign(this->mapStringToAlign(align));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setTextAlign");
        }
    }
    ...
    void MyGUIButtonComponent::setTextOffset(const Ogre::Vector2& textOffset)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, textOffset]()
            {
                this->widget->castType<MyGUI::TextBox>()->setTextOffset(MyGUI::IntPoint(textOffset.x, textOffset.y));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setTextOffset");
        }
    }
    ...
    void MyGUIButtonComponent::setTextColor(const Ogre::Vector4& textColor)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, textColor]()
            {
                this->widget->castType<MyGUI::TextBox>()->setTextColour(MyGUI::Colour(textColor.x, textColor.y, textColor.z, textColor.w));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUITextComponent::setTextColor");
        }
    }
    ...
    bool MyGUICheckBoxComponent::postInit(void)
    {
        ...
        if (true == this->createWidgetInParent)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUICheckBoxComponent::postInit");
        }
    ...
    void MyGUICheckBoxComponent::setCaption(const Ogre::String& caption)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUICheckBoxComponent::setCaption");
        }
    }
    ...
    void MyGUICheckBoxComponent::setChecked(bool checked)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, checked]()
            {
                this->widget->castType<MyGUI::Button>()->setStateCheck(checked);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUICheckBoxComponent::setChecked");
        }
    }
    ...
    void MyGUICheckBoxComponent::setFontHeight(unsigned int fontHeight)
    {
        ...
        NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUICheckBoxComponent::setFontHeight");
    }
    ...
    void MyGUICheckBoxComponent::setTextAlign(const Ogre::String& align)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUICheckBoxComponent::setTextAlign");
        }
    }
    ...
    void MyGUICheckBoxComponent::setTextOffset(const Ogre::Vector2& textOffset)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUICheckBoxComponent::setTextOffset");
        }
    }
    ...
    void MyGUICheckBoxComponent::setTextColor(const Ogre::Vector4& textColor)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUICheckBoxComponent::setTextColor");
        }
    }
    ...
    void MyGUIImageBoxComponent::setCenter(const Ogre::Vector2& center)
    {
        ...
        if (nullptr != this->rotatingSkin)
        {
            GraphicsModule::RenderCommand renderCommand = [this, center]()
            {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIImageBoxComponent::setCenter");
        }
    }
    ...
    void MyGUIImageBoxComponent::setAngle(Ogre::Real angle)
    {
        ...
        if (nullptr != this->rotatingSkin)
        {
            GraphicsModule::RenderCommand renderCommand = [this, angle]()
            {
                // Attention: Here better this->rotatingSkin->getWidth()  * center.x etc. ?
                this->rotatingSkin->setAngle(MathHelper::getInstance()->degreeAngleToRadian(angle));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIImageBoxComponent::setAngle");
        }
    }
    ...
    bool MyGUIProgressBarComponent::postInit(void)
    {
        ...
        if (true == this->createWidgetInParent)
        {
            ...
            {
                this->widget =
                    MyGUI::Gui::getInstancePtr()->createWidgetReal<MyGUI::ProgressBar>(this->skin->getListSelectedValue(), this->position->getVector2().x, this->position->getVector2().y, this->size->getVector2().x, this->size->getVector2().y,
                        this->mapStringToAlign(this->align->getListSelectedValue()), this->layer->getListSelectedValue(), this->getClassName() + "_" + this->gameObjectPtr->getName() + Ogre::StringConverter::toString(this->index));
            };
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIProgressBarComponent::postInit");
        }
    ...
    void MyGUIProgressBarComponent::setValue(unsigned int value)
    {
        if (value > this->range->getUInt())
        {
            if (nullptr != this->gameObjectPtr)
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[MyGUIProgressBarComponent] Warning value: '" + Ogre::StringConverter::toString(value) + "' is higher as range: '" +
                                                                                       Ogre::StringConverter::toString(this->range->getUInt()) + "' for game object: " + this->gameObjectPtr->getName());
            }
        }
    ...
    void MyGUIProgressBarComponent::setRange(unsigned int range)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, range]()
            {
                this->widget->castType<MyGUI::ProgressBar>()->setProgressRange(range);
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIProgressBarComponent::setRange");
        }
    }
    ...
    void MyGUIProgressBarComponent::setFlowDirection(const Ogre::String& flowDirection)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, flowDirection]()
            {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIProgressBarComponent::setFlowDirection");
        }
    }
    ...
    bool MyGUIListBoxComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
        ...
        for (size_t i = 0; i < this->items.size(); i++)
        {
            if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == "Item" + Ogre::StringConverter::toString(i))
            {
                ...
                {
                    this->items[i] = new Variant(MyGUIListBoxComponent::AttrItem() + Ogre::StringConverter::toString(i), XMLConverter::getAttrib(propertyElement, "data"), this->attributes);
                }
                else
                {
    ...
    bool MyGUIListBoxComponent::postInit(void)
    {
        ...
        NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIListBoxComponent::postInit");
    ...
    void MyGUIListBoxComponent::setItemHeight(unsigned int itemHeight)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            if (itemHeight != 21)
            {
                GraphicsModule::RenderCommand renderCommand = [this, itemHeight]()
                {
                ...
                NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIListBoxComponent::setItemHeight");
    ...
    void MyGUIListBoxComponent::setFontHeight(unsigned int fontHeight)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIListBoxComponent::setFontHeight");
        }
    }
    ...
    void MyGUIListBoxComponent::setTextAlign(const Ogre::String& align)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIListBoxComponent::setTextAlign");
        }
    }
    ...
    void MyGUIListBoxComponent::setTextOffset(const Ogre::Vector2& textOffset)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIListBoxComponent::setTextOffset");
        }
    }
    ...
    void MyGUIListBoxComponent::setTextColor(const Ogre::Vector4& textColor)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIListBoxComponent::setTextColor");
        }
    }
    ...
    void MyGUIListBoxComponent::setItemCount(unsigned int itemCount)
    {
        ...
        else if (itemCount < oldSize)
        {
            ...
            if (nullptr != this->widget)
            {
                GraphicsModule::RenderCommand renderCommand = [this, itemCount, oldSize]()
                {
                ...
                NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIListBoxComponent::setItemCount");
            }
        }
    }
    ...
    void MyGUIListBoxComponent::setItemText(unsigned int index, const Ogre::String& itemText)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, index, itemText]()
            {
                this->widget->castType<MyGUI::ListBox>()->setItemNameAt(index, MyGUI::LanguageManager::getInstancePtr()->replaceTags(itemText));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIListBoxComponent::setItemText");
        }
    }
    ...
    void MyGUIListBoxComponent::addItem(const Ogre::String& itemText)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, itemText]()
            {
                this->widget->castType<MyGUI::ListBox>()->addItem(itemText);
            };
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIListBoxComponent::addItem");
    ...
    void MyGUIListBoxComponent::insertItem(int index, const Ogre::String& itemText)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIListBoxComponent::insertItem");
    ...
    void MyGUIListBoxComponent::removeItem(unsigned int index)
    {
        if (index > 0 && index < this->itemCount->getUInt())
        {
            ...
            if (nullptr != this->widget)
            {
                GraphicsModule::RenderCommand renderCommand = [this, index]()
                {
    ...
    bool MyGUIComboBoxComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
        ...
        for (size_t i = 0; i < this->items.size(); i++)
        {
            if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == "Item" + Ogre::StringConverter::toString(i))
            {
                this->items[i] = new Variant(MyGUIComboBoxComponent::AttrItem() + Ogre::StringConverter::toString(i), XMLConverter::getAttrib(propertyElement, "data"), this->attributes);
                propertyElement = propertyElement->next_sibling("property");
                this->items[i]->addUserData(GameObject::AttrActionSeparator());
            }
        }
    ...
    bool MyGUIComboBoxComponent::postInit(void)
    {
        ...
        NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::postInit");
    ...
    void MyGUIComboBoxComponent::setItemHeight(unsigned int itemHeight)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            if (itemHeight != 21)
            {
                ...
                NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::setItemHeight");
    ...
    void MyGUIComboBoxComponent::setFontHeight(unsigned int fontHeight)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, &fontHeight, oldFontHeight]()
            {
                MyGUI::ComboBox* comboBox = widget->castType<MyGUI::ComboBox>();
                for (size_t i = 0; i < this->items.size(); i++)
                {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::setFontHeight");
    ...
    void MyGUIComboBoxComponent::setTextAlign(const Ogre::String& align)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComboBoxComponent::setTextAlign");
        }
    }
    ...
    void MyGUIComboBoxComponent::setTextOffset(const Ogre::Vector2& textOffset)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComboBoxComponent::setTextOffset");
        }
    }
    ...
    void MyGUIComboBoxComponent::setTextColor(const Ogre::Vector4& textColor)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComboBoxComponent::setTextColor");
        }
    }
    ...
    void MyGUIComboBoxComponent::setFlowDirection(const Ogre::String& flowDirection)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComboBoxComponent::setFlowDirection");
        }
    }
    ...
    void MyGUIComboBoxComponent::setItemCount(unsigned int itemCount)
    {
        ...
        else if (itemCount < oldSize)
        {
            ...
            if (nullptr != this->widget)
            {
                ...
                NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::setItemCount");
            }
        }
    }
    ...
    void MyGUIComboBoxComponent::setItemText(unsigned int index, const Ogre::String& itemText)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, index, itemText]()
            {
                this->widget->castType<MyGUI::ComboBox>()->setItemNameAt(index, MyGUI::LanguageManager::getInstancePtr()->replaceTags(itemText));
            };
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "MyGUIComboBoxComponent::setItemText");
        }
    }
    ...
    void MyGUIComboBoxComponent::addItem(const Ogre::String& itemText)
    {
        ...
        if (nullptr != this->widget)
        {
            ...
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::addItem");
        }
    }
    ...
    void MyGUIComboBoxComponent::insertItem(int index, const Ogre::String& itemText)
    {
        ...
        if (nullptr != this->widget)
        {
            GraphicsModule::RenderCommand renderCommand = [this, index, itemText]()
            {
                this->widget->castType<MyGUI::ComboBox>()->insertItemAt(index, itemText);
            };
            NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::insertItem");
    ...
    void MyGUIComboBoxComponent::removeItem(unsigned int index)
    {
        if (index > 0 && index < this->itemCount->getUInt())
        {
            ...
            if (nullptr != this->widget)
            {
                ...
                NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::removeItem");
            }
        }
    }
    ...
    MyGUIMessageBoxComponent::MyGUIMessageBoxComponent() : GameObjectComponent(), messageBox(nullptr), isSimulating(false)
    {
    ...
    void MyGUIMessageBoxComponent::setActivated(bool activated)
    {
        ...
        if (true == this->isSimulating)
        {
            if (true == activated)
            {
                ...
                GraphicsModule::RenderCommand renderCommand = [this, tempStyle]()
                {
                    this->messageBox =
                        MyGUI::Message::createMessageBox(MyGUI::LanguageManager::getInstancePtr()->replaceTags(this->title->getString()), MyGUI::LanguageManager::getInstancePtr()->replaceTags(this->message->getString()), tempStyle, "Popup", true);

                    this->messageBox->eventMessageBoxResult += MyGUI::newDelegate(this, &MyGUIMessageBoxComponent::notifyMessageBoxEnd);
                };
                NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::setActivated");
            }
        }
        ...
        if (false == activated)
        {
            if (nullptr != this->messageBox)
            {
                ...
                NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "MyGUIComboBoxComponent::setActivated2");
            }
        }
    }
