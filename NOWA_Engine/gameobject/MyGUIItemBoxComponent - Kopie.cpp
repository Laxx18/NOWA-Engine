...
namespace NOWA
{
	...
	void MyGUIItemBoxComponent::notifyEndDrop(wraps::BaseLayout* _sender, wraps::DDItemInfo _info, bool _result)
	{
		if (_result && false == this->dropFinished)
		{
			...
			unsigned int chosenQuantity = 1;
			if (senderData->getQuantity() > 1)
			{
				this->showBuyAmountDialog(senderData->getQuantity());
				chosenQuantity = Ogre::StringConverter::parseUnsignedInt(MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::EditBox>("amountEdit")->getOnlyText());
			}
			...
			if (receiverData->getInventoryOwnerId() != this->gameObjectPtr->getId())
			{
				...
				if (nullptr != myGuiItemBoxCompPtr)
				{
					...
					myGuiItemBoxCompPtr->increaseQuantity(_info.receiver_index, chosenQuantity);
			...
			senderData->removeQuantity(chosenQuantity);
	...
	void MyGUIItemBoxComponent::showBuyAmountDialog(unsigned int amount)
	{
		if (0 == this->buyAmountWidgets.size())
		{
			...
			MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::Button>("okButton")->eventMouseButtonClick += MyGUI::newDelegate(this, &MyGUIItemBoxComponent::buttonHit);
		}
