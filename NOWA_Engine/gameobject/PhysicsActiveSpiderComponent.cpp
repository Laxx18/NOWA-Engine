...
/*
* How the Spider Works in NOWA-EngineThe Core IdeaThere are no keyframe animations at all. The legs move because physics forces them to. 
The IK effector (ndIkSwivelPositionEffector) is a spring-damper constraint that pulls the heel body toward a target position every physics step. 
The gait generator just moves those target positions in a believable walking pattern. Physics does the rest.The 3 Mesh FieldsYou set one mesh name per segment type 
— one thigh mesh, one calf mesh, one heel mesh. The component creates 4 instances of each (12 SceneNodes total, 3 per leg). All 4 thighs share the same mesh and datablocks. 
Same for calves and heels.The SceneNodes are children of the torso node initially, 
but every frame PostTransformUpdate captures the world transforms of the actual physics bodies (thigh/calf/heel rigid bodies), 
and the render closure pushes those world positions and orientations to the SceneNodes. 
So the visual meshes follow the physics bodies exactly.For bones.mesh: it will work as long as the mesh origin sits at the joint pivot — 
hip pivot for thigh, knee pivot for calf, ankle pivot for heel. The mesh should extend downward along Y (or whichever axis you set as LegBoneAxis). 
If your bones.mesh origin is at the top of the bone and it extends down the Y axis, that is already correct.ScalingNot automatic. 
You have two controls:LegMeshScale — uniform scale applied to all 12 SceneNodes visually. This makes the meshes look bigger or smaller but does not affect the physics capsule sizes or the IK reach.
ThighBoneLength / CalfBoneLength — these control the physics capsule lengths and the IK workspace. If you set them to 0, the component auto-computes them as half the hip-to-foot distance. 
These are the values that actually determine how far the legs reach and where joints sit.So the workflow is:

Set ThighBoneLength and CalfBoneLength to match your actual intended leg proportions.
Set LegMeshScale so that bones.mesh visually fits those lengths.
Set LegHipPos and LegFootRestPos to match the scaled proportions.
*/
...
namespace NOWA
{
    ...
    bool PhysicsActiveSpiderComponent::connect(void)
    {
        ...
        if (!this->createDynamicBody()) // SpiderBody constructed, m_body = nullptr
        {
            return false;
        }

        if (!this->createLegMeshes()) // visual nodes created
        {
        ...
        this->registerLegChains(); // addLegChain × N -> finalizeModel() -> m_body valid

        this->applyDynamicBodyProperties(); // all body-property calls, now safe
    ...
    bool PhysicsActiveSpiderComponent::createDynamicBody(void)
    {
        ...
        // Construct SpiderBody — m_body is NULL until finalizeModel()
        this->physicsBody = new OgreNewt::SpiderBody(this->ogreNewt, this->gameObjectPtr->getSceneManager(), collisionPtr, this->mass->getReal(), calculatedMassOrigin, this->gravity->getVector3(), this->gameObjectPtr->getDefaultDirection(),
            this->initialPosition, this->initialOrientation,
            new PhysicsSpiderCallback(this->gameObjectPtr.get(), luaScript, this->ogreNewt, m_onMovementChangedFunctionName->getString()));

        // Safe to set on SpiderBody before finalizeModel() — these touch only members,
        // not m_body itself:
    ...
    void PhysicsActiveSpiderComponent::registerLegChains(void)
    {
...

        {
            if (activeLegCount > 0)
            {
                ...
                // finalizeModel() creates the ndModelArticulation notify and adds the
                // model (and all leg bodies + joints) to the Newton world.
                spider->finalizeModel();
            }
        }
    }
    ...
    void PhysicsActiveSpiderComponent::updateLegNodes(Ogre::Real dt)
    {
        ...
        auto closureFunction = [this, legCount, snap, torsoNode, boneCorrection, originAtEnd, natLen](Ogre::Real)
        {
            ...
            for (int i = 0; i < legCount && i < MAX_LEGS; ++i)
            {
                const OgreNewt::LegTransformCache& tc = snap[i];
                if (!tc.valid)
                {
                    continue;
                }
    ...
    void PhysicsActiveSpiderComponent::applyDynamicBodyProperties(void)
    {
        // finalizeModel() has run — m_body is valid, safe to call all body methods.
    ...
    void PhysicsActiveSpiderComponent::setGhost(bool ghost)
    {
        if (nullptr == this->physicsBody)
        {
            return;
        ...
        if (true == ghost)
        {
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[PhysicsActiveSpiderComponent] Ghost ON for: " + this->gameObjectPtr->getName());
        }
        else
        {
            ...
            this->physicsBody->unFreeze();
            this->physicsBody->setCustomForceAndTorqueCallback<PhysicsActiveSpiderComponent>(&PhysicsActiveSpiderComponent::moveCallback, this);

            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[PhysicsActiveSpiderComponent] Ghost OFF for: " + this->gameObjectPtr->getName());
        }
    }
