...
namespace NOWA
{
	...
	bool PhysicsCompoundConnectionComponent::init(xml_node<>*& propertyElement, const Ogre::String& filename)
	{
	...
	void PhysicsCompoundConnectionComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc, const Ogre::String& filePath)
	{
		// 2 = int
		// 6 = real
		// 7 = string
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->rootId->getULong())));
