...
namespace NOWA
{
    ...
    Core::~Core()
    {
        ...
        if (nullptr != this->root)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_NORMAL, "[Core] Manually destroying all SceneManagers");

            Ogre::SceneManagerEnumerator::SceneManagerIterator it = this->root->getSceneManagerIterator();
            ...
            for (Ogre::SceneManager* sm : managers)
            {
                if (sm)
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[Core] Destroying SceneManager: " + sm->getName());
    ...
    bool Core::initialize(const CoreConfiguration& coreConfiguration)
    {
        ...
        while (false == resourceCheck)
        {
            ...
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[Core] RAM is to low to execute this application. A physical RAM with more than " + Ogre::StringConverter::toString(this->requiredRAMPhysicalMB) +
                                                                                        " MB is required and a virtual RAM with more than " + Ogre::StringConverter::toString(this->requiredRAMVirtualMB) + " MB is required.");
                throw Ogre::Exception(Ogre::Exception::ERR_INVALID_STATE,
                    "[Core] RAM is to low to execute this application. A physical RAM with more than " + Ogre::StringConverter::toString(this->requiredRAMPhysicalMB) + " MB is required and a virtual RAM with more than " +
                        Ogre::StringConverter::toString(this->requiredRAMVirtualMB) + " MB is required.\n",
                    "NOWA");
                return false;
            }
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                "[Core]: Available Physical RAM size: " + Ogre::StringConverter::toString(static_cast<DWORD>(thisRAMPhysical)) + " MB, which is enough for this application (> " + Ogre::StringConverter::toString(this->requiredRAMPhysicalMB) +
                    ") and available virtual RAM size: " + Ogre::StringConverter::toString(static_cast<DWORD>(thisRAMVirtual)) + " MB, which is enough for this application (> " + Ogre::StringConverter::toString(this->requiredRAMVirtualMB) + ")");

            if (!this->checkStorage(this->requiredDiscSpaceMB, thisDiscSpace))
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[Core] Disc space is to low to execute this application. A disc space with more than " + Ogre::StringConverter::toString(this->requiredDiscSpaceMB) + " MB is required");
                throw Ogre::Exception(Ogre::Exception::ERR_INVALID_STATE, "[Core] Disc space is to low to execute this application. A disc space with more than " + Ogre::StringConverter::toString(this->requiredDiscSpaceMB) + " MB is required\n",
                    "NOWA");
                return false;
            }
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                "[Core]: Disc space: " + Ogre::StringConverter::toString(static_cast<DWORD>(thisDiscSpace)) + " MB (" + Ogre::StringConverter::toString(static_cast<DWORD>(thisDiscSpace / 1024)) + " GB, " +
                    Ogre::StringConverter::toString(static_cast<float>((thisDiscSpace / (1024.0f * 1024.0f)))) + " TB), which is enough for this application (> " + Ogre::StringConverter::toString(this->requiredDiscSpaceMB) + ")");

            DWORD thisCPU = this->readCPUSpeed();
            if (static_cast<int>(thisCPU) < this->requiredCPUSpeedMhz)
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[Core] CPU is too slow for to execute this application. A processor with more than " + Ogre::StringConverter::toString(this->requiredCPUSpeedMhz / 1000.0f) + " Ghz is required!");
                throw Ogre::Exception(Ogre::Exception::ERR_INVALID_STATE,
                    "[Core] CPU is too slow for to execute this application. A processor with more than " + Ogre::StringConverter::toString(this->requiredCPUSpeedMhz / 1000.0f) + " Ghz is required!\n", "NOWA");
                return false;
            }
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                "[Core]: Cpu speed: " + Ogre::StringConverter::toString(thisCPU / 1000.0f) + " Ghz, which is enough for this application (> " + Ogre::StringConverter::toString(requiredCPUSpeedMhz / 1000.0f) + ")");
            resourceCheck = true;
        }
        ...
        while (seci.hasMoreElements())
        {
            ...
            if (resourceGroupName != "Essential" && resourceGroupName != "General" && resourceGroupName != "PostProcessing" && resourceGroupName != "Hlms" && resourceGroupName != "Internal" && resourceGroupName != "AutoDetect" &&
                resourceGroupName != "Lua" && resourceGroupName != "Audio" && resourceGroupName != "TerrainTextures" && resourceGroupName != "Projects" && resourceGroupName != "NOWA_Design" && false == resourceGroupName.empty())
            {
    ...
    void Core::loadHlmsDiskCache(void)
    {
        ...
        try
        {
            ...
            {
                Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[INFO] Texture cache not found at " + this->writeAccessFolder + "/" + filename);
            }
        }
        catch (Ogre::Exception& e)
        {
    ...
    {
        NOWA::GraphicsModule::RenderCommand renderCommand = [this, config]()
        {
        ...
        GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "Core::preLoadTextures");
    }
    ...
    void Core::setupHlms(void)
    {
        ...
        // HlmsPbs
        {
            ...
            // Always add Terra shadows piece files
            archivePbsLibraryFolders.push_back(archiveManager.load(dataFolder + "Hlms/Terra/" + shaderSyntax + "/PbsTerraShadows", "FileSystem", true));

            // Always add Ocean piece files
            archivePbsLibraryFolders.push_back(archiveManager.load(dataFolder + "Hlms/Ocean/" + shaderSyntax, "FileSystem", true));
        ...
        // HlmsOcean
        {
            Ogre::String oceanMainFolderPath;
            Ogre::StringVector oceanLibraryFolders;

            Ogre::HlmsOcean::getDefaultPaths(oceanMainFolderPath, oceanLibraryFolders);
            Ogre::Archive* archiveOcean = archiveManager.load(dataFolder + oceanMainFolderPath, "FileSystem", true);

            Ogre::ArchiveVec libraries;
            for (const auto& path : oceanLibraryFolders)
            {
        ...
        // HlmsWind
        {
            ...
            {
                archive.push_back(archiveManager.load(dataFolder + str, "FileSystem", true));
            }

            Ogre::Archive* archiveWind = archiveManager.load(dataFolder + mainFolderPath, "FileSystem", true);
            hlmsWind = OGRE_NEW HlmsWind(archiveWind, &archive);
    ...
    {
        NOWA::GraphicsModule::RenderCommand renderCommand = [this, mode]()
        {
            ...
            if (nullptr != hlmsPbs)
            {
                ...
                while (itor != end)
                {
                    ...
                    const Ogre::HlmsMacroblock* finalMacroblock = this->root->getHlmsManager()->getMacroblock(macroblock);
        ...
        GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "Core::setPolygonMode");
    }
    ...
    {
        NOWA::GraphicsModule::RenderCommand renderCommand = [visible]()
        {
            if (MyGUI::PointerManager::getInstancePtr())
            {
        ...
        GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "Core::setMyGuiPointerVisible");
    }
    ...
    bool Core::isAbsolutePath(const Ogre::String& path)
    {
        ...
        if (path.size() > 2)
        {
            ...
            if (((c0 >= 'A' && c0 <= 'Z') || (c0 >= 'a' && c0 <= 'z')) && path[1] == ':' && (path[2] == '\\' || path[2] == '/'))
            {
                return true;
            }
        }
    ...
    Ogre::String Core::getModuleDirectoryFromAddress(const void* address)
    {
        ...
        if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, reinterpret_cast<LPCSTR>(address), &hModule))
        {
            return Ogre::String();
        }
    ...
    void Core::displayError(LPCTSTR errorDesc, DWORD errorCode)
    {
        ...
        FormatMessage(flags, NULL, errorCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), errorMessage, sizeof(errorMessage) / sizeof(TCHAR), NULL);
    ...
    std::vector<Ogre::String> Core::getSceneFoldersInProject(const Ogre::String& projectFilePathName)
    {
        ...
        if (hFind != INVALID_HANDLE_VALUE)
        {
            do
            {
                ...
                if ((fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && Ogre::String(fd.cFileName) != "." && Ogre::String(fd.cFileName) != "..")
                {
    ...
    void Core::dumpNodes(Ogre::Node* pNode, Ogre::String padding)
    {
        ...
        Ogre::LogManager::getSingletonPtr()->logMessage(padding + "SceneNode: " + pNode->getName() + " derived position: " + Ogre::StringConverter::toString(pNode->_getDerivedPosition()) +
                                                        " local position: " + Ogre::StringConverter::toString(pNode->getPosition()) + " derived orientation: " + Ogre::StringConverter::toString(pNode->_getDerivedOrientation()) +
                                                        " local orientation: " + Ogre::StringConverter::toString(pNode->getOrientation()) + " derived scale: " + Ogre::StringConverter::toString(pNode->_getDerivedScale()) +
        ...
        while (objectIt.hasMoreElements())
        {
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(padding + "Type: " + movableObject->getMovableType() + ": " + movableObject->getName() + ": query flags: " + Ogre::StringConverter::toString(movableObject->getQueryFlags()) +
    ...
    void Core::loadCustomConfiguration(const char* strFilename)
    {
        ...
        if (pSubElement)
        {
            if (pSubElement->first_attribute("Level"))
            {
                ...
                if (this->optionLogLevel < 1)
                {
                    this->optionLogLevel = 1;
                }
                if (this->optionLogLevel > 3)
                {
                    this->optionLogLevel = 3;
                }
...
#if OGRE_PLATFORM != OGRE_PLATFORM_APPLE_IOS && OGRE_PLATFORM != OGRE_PLATFORM_ANDROID && OGRE_ARCH_TYPE != OGRE_ARCHITECTURE_32
    ...
    Ogre::String Core::dumpGraphicsMemory(void)
    {
        ...
        text.clear();
        text.a("\n\nGPU buffer pools (meshes, const, texture, indirect & uav buffers): ", (Ogre::uint32)((capacityBytes - freeBytes) / bytesToMb), "/", (Ogre::uint32)(capacityBytes / bytesToMb), " MB");
        outText += text.c_str();

        Ogre::TextureGpuManager* textureGpuManager = renderSystem->getTextureGpuManager();
        size_t textureBytesCpu, textureBytesGpu, usedStagingTextureBytes, availableStagingTextureBytes;
        textureGpuManager->getMemoryStats(textureBytesCpu, textureBytesGpu, usedStagingTextureBytes, availableStagingTextureBytes);
        ...
        text.clear();
        text.a("\nGPU StagingTextures. In use: ", (Ogre::uint32)(usedStagingTextureBytes / bytesToMb), " MB. Available: ", (Ogre::uint32)(availableStagingTextureBytes / bytesToMb),
            " MB. Total:", (Ogre::uint32)((usedStagingTextureBytes + availableStagingTextureBytes) / bytesToMb));
        outText += text.c_str();

        text.clear();
        text.a("\nGPU Textures:\t", (Ogre::uint32)(textureBytesGpu / bytesToMb), " MB");
        text.a("\nCPU Textures:\t", (Ogre::uint32)(textureBytesCpu / bytesToMb), " MB");
        text.a("\nTotal GPU:\t", (Ogre::uint32)((capacityBytes + textureBytesGpu + usedStagingTextureBytes + availableStagingTextureBytes) / bytesToMb), " MB");
        outText += text.c_str();
    ...
    void Core::unloadUnusedTextures(void)
    {
        ...
        while (itor != end)
        {
            ...
            if (entry.texture->getTextureType() != Ogre::TextureTypes::Type2D || !entry.texture->hasAutomaticBatching() || !entry.texture->isTexture() || entry.texture->isRenderToTexture() || entry.texture->isUav())
            {
