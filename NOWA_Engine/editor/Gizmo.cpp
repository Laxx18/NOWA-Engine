	...
	// Attention: Is this correct?
			if (this->arrowEntityX != nullptr && this->sceneManager->hasMovableObject(this->arrowEntityX))
			{
				if (this->arrowEntityX->getMesh()->getName() != "Ring.mesh")
				{
					...
					this->arrowNodeX->setScale(distance * this->thickness * fovy, distance * this->thickness * fovy, distance * fovy);
					this->arrowNodeY->setScale(distance * this->thickness * fovy, distance * this->thickness * fovy, distance * fovy);
					this->arrowNodeZ->setScale(distance * this->thickness * fovy, distance * this->thickness * fovy, distance * fovy);
				}
				else
				{
					distance = camDistance * 0.03f;
...
// Attention: Is this correct?
	...
	void Gizmo::setOrientation(const Ogre::Quaternion& orientation)
	{
		...
		/*if (true == this->debugPlane)
		{
			this->firstPlaneNode->setOrientation(orientation);
			this->secondPlaneNode->setOrientation(orientation);
		}*/
	}
	...
	void Gizmo::translate(const Ogre::Vector3& translateVector)
	{
		...
		this->selectNode->translate(internalTranslateVector, Ogre::Node::TransformSpace::TS_WORLD);

		/*if (true == this->debugPlane)
		{
			this->firstPlaneNode->translate(translateVector);
			this->secondPlaneNode->translate(translateVector);
		}*/
	}

	void Gizmo::rotate(const Ogre::Quaternion& rotateQuaternion)
	{
		this->selectNode->rotate(rotateQuaternion);
		/*if (true == this->debugPlane)
		{
			this->firstPlaneNode->rotate(rotateQuaternion);
			this->secondPlaneNode->rotate(rotateQuaternion);
		}*/
	}
