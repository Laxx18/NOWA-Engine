...
namespace NOWA
{
        ...
        virtual void undo(void) override
        {
            ...
            for (auto it = this->gameObjectIds.begin(); it != this->gameObjectIds.end(); ++it)
            {
                ...
                else
                {
                    ...
                    gameObjectPtr->setAttributePosition(restorePos);
                    gameObjectPtr->setAttributeOrientation(restoreOrient);
                    gameObjectPtr->setAttributeScale(restoreScale);
                }
                i++;
            }
        }
    ...
    {
    public:
        AttributeGameObjectUndoCommand(std::vector<EditorManager::GameObjectData> oldGameObjectDataList) : oldGameObjectDataList(std::move(oldGameObjectDataList))
        {
        }
    ...
    {
    public:
        AddComponentUndoCommand(std::vector<unsigned long> gameObjectIds, const Ogre::String& componentClassName, bool newComponent) : gameObjectIds(gameObjectIds), componentClassName(componentClassName), newComponent(newComponent)
        {
    ...
    {
    public:
        DeleteComponentUndoCommand(std::vector<unsigned long> gameObjectIds, unsigned int index) : gameObjectIds(gameObjectIds), index(index)
        {
                        ...
                        if (componentPtr)
                        {
                            ...
                            if (!componentPtr->postInit())
                            {
                                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                                    "[DeleteComponentUndoCommand]: Error: Could not post init component during undo: " + componentPtr->getClassName() + " for game object: " + gameObjectPtr->getName());
                            }
                        }
                    }
                }
    ...
    public:
        ...
        void applyData(const std::vector<unsigned char>& data)
        {
            // Look up the component fresh each time
            GameObjectPtr gameObjectPtr = AppStateManager::getSingletonPtr()->getGameObjectController()->getGameObjectFromId(this->gameObjectId);
            ...
            auto meshEditComponentBase = NOWA::makeStrongPtr(gameObjectPtr->getComponent<NOWA::MeshEditComponentBase>());

            if (nullptr == meshEditComponentBase)
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[MeshEditModifyUndoCommand] redo: MeshEditComponentBase not found");
                return;
            }

            meshEditComponentBase->setMeshData(data);
        }
    ...
    public:
        ...
        void applyData(const std::vector<unsigned char>& data)
        {
            ...
            if (nullptr == gameObjectPtr)
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[PlanetTerraModifyUndoCommand] applyData: GameObject not found (ID: " + Ogre::StringConverter::toString(this->gameObjectId) + ")");
                return;
            }

            auto planetTerraComponentBase = NOWA::makeStrongPtr(gameObjectPtr->getComponent<NOWA::PlanetTerraComponentBase>());

            if (nullptr == planetTerraComponentBase)
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[PlanetTerraModifyUndoCommand] applyData: PlanetTerraComponentBase not found on GameObject (ID: " + Ogre::StringConverter::toString(this->gameObjectId) + ")");
                return;
            }

            planetTerraComponentBase->setPlanetData(data);
        }
    ...
    bool EditorManager::handleKeyPress(const OIS::KeyEvent& keyEventRef)
    {
        ...
        else if (OIS::KC_ESCAPE == keyEventRef.key)
        {
            ...
            else if (EDITOR_TRANSLATE_MODE == this->manipulationMode || EDITOR_ROTATE_MODE1 == this->manipulationMode || EDITOR_ROTATE_MODE2 == this->manipulationMode || EDITOR_SCALE_MODE == this->manipulationMode)
            {
                handled = true;
                this->manipulationMode = EDITOR_SELECT_MODE;
    ...
    void EditorManager::handleMouseMove(const OIS::MouseEvent& evt)
    {
        ...
        {
            if (true == this->mouseIdPressed && EDITOR_SELECT_MODE <= this->manipulationMode && EDITOR_ROTATE_MODE2 >= this->manipulationMode && Gizmo::GIZMO_NONE == this->gizmo->getState() && false == this->isGizmoMoving &&
                EDITOR_PICKER_MODE != this->manipulationMode && false == this->isInSimulation)
            {
                this->selectionManager->handleMouseMove(evt);
            }

            // Select the gizmo arrow depending which the mouse hit
            if (EDITOR_SELECT_MODE < this->manipulationMode && EDITOR_ROTATE_MODE2 >= this->manipulationMode && false == this->selectionManager->getIsSelectingRectangle() && true == this->gizmo->isEnabled())
            {
                    ...
                    // if (nullptr != physicsComponent && physicsComponent->getBody() != nullptr)
                    {
                        ...
                        if (GetAsyncKeyState(VK_LCONTROL))
                        {
                            NOWA::PhysicsComponent* component = this->movePicker2->grab(AppStateManager::getSingletonPtr()->getOgreNewtModule()->getOgreNewt(),
                                Ogre::Vector2(static_cast<Ogre::Real>(evt.state.X.abs), static_cast<Ogre::Real>(evt.state.Y.abs)), Core::getSingletonPtr()->getOgreRenderWindow(), this->pickForce);
                        }
                        else
                        {
                            NOWA::PhysicsComponent* component = this->movePicker->grab(AppStateManager::getSingletonPtr()->getOgreNewtModule()->getOgreNewt(),
                                Ogre::Vector2(static_cast<Ogre::Real>(evt.state.X.abs), static_cast<Ogre::Real>(evt.state.Y.abs)), Core::getSingletonPtr()->getOgreRenderWindow());
                        }
                    }
                }
            }
            else if (true == this->mouseIdPressed && Gizmo::GIZMO_NONE < this->gizmo->getState() && false == this->selectionManager->getIsSelectingRectangle() && EDITOR_SELECT_MODE < this->manipulationMode &&
                     EDITOR_ROTATE_MODE2 >= this->manipulationMode)
            {
    ...
    void EditorManager::handleMousePress(const OIS::MouseEvent& evt, OIS::MouseButtonID id)
    {
        ...
        else if (id == this->mouseButtonId && Gizmo::GIZMO_NONE == this->gizmo->getState() && this->manipulationMode >= EDITOR_SELECT_MODE && this->manipulationMode <= EDITOR_ROTATE_MODE2 && this->manipulationMode != EDITOR_PLACE_MODE)
        {
            if (false == this->isInSimulation)
            {
            ...
            if (this->manipulationMode >= EDITOR_TRANSLATE_MODE && false == this->selectionManager->getSelectedGameObjects().empty() && EDITOR_PICKER_MODE != this->manipulationMode)
            {
    ...
    void EditorManager::focusCameraGameObject(GameObject* gameObject)
    {
        if (nullptr != gameObject)
        {
            ...
            NOWA::GraphicsModule::getInstance()->updateCameraPosition(this->camera, gameObject->getMovableObject()->getWorldAabbUpdated().mCenter + (this->camera->getOrientation() * Ogre::Vector3(0, 0, adjustedDistance)));
    ...
    void EditorManager::applyPlaceMovableTransform(void)
    {
        ...
        NOWA::GraphicsModule::getInstance()->setNodeTransform(this->tempPlaceMovableNode,
            this->placeNode->_getDerivedPositionUpdated() + (this->placeNode->getOrientation() * MathHelper::getInstance()->getBottomCenterOfMesh(this->tempPlaceMovableNode, this->tempPlaceMovableObject)),
            this->placeNode->_getDerivedOrientationUpdated());
    }
    ...
    void EditorManager::applyGroupTransform(void)
    {
        ...
        for (size_t i = 0; i < this->groupGameObjectIds.size(); i++)
        {
            ...
            else
            {
                ...
                // NOWA::GraphicsModule::getInstance()->setNodeTransform(gameObjectPtr->getSceneNode(), targetPosition, this->placeNode->_getDerivedOrientationUpdated() * std::get<2>(this->groupGameObjectIds[i]) /**
                // gameObjectPtr->getSceneNode()->getOrientation()*/);
            }
        }
    }
            ...
            for (const auto& selectedGameObject : selectedGameObjects)
            {
                ...
                if (nullptr != physicsComponent)
                {
                    if (EDITOR_ROTATE_MODE1 == this->manipulationMode)
                    {
                        ...
                        physicsComponent->setKinematicPositionOrientation(physicsComponent->getBody()->getPosition(), localRotation);
                    }
                    else if (EDITOR_ROTATE_MODE2 == this->manipulationMode)
                    {
                        ...
                        physicsComponent->setKinematicPositionOrientation(newPosition, localRotation);
            ...
            for (const auto& selectedGameObject : selectedGameObjects)
            {
                ...
                if (nullptr != physicsComponent && physicsComponent->getBody() != nullptr)
                {
                    ...
                    else if (EDITOR_ROTATE_MODE2 == this->manipulationMode)
                    {
                        ...
                        physicsComponent->setKinematicPositionOrientation(newPosition, newOrientation);
                    }
                }
                else
                {
    ...
    void EditorManager::setCameraView(EditorManager::eCameraView cameraView)
    {
        ...
        {
            NOWA::GraphicsModule::getInstance()->setCameraOrientation(this->camera, Ogre::Quaternion(Ogre::Degree(0.0f), Ogre::Vector3::UNIT_Y));
            break;
        }
        ...
        {
            NOWA::GraphicsModule::getInstance()->setCameraOrientation(this->camera, Ogre::Quaternion(Ogre::Degree(-180.0f), Ogre::Vector3::UNIT_Y));
            break;
        }
        case EDITOR_CAMERA_VIEW_BOTTOM:
        {
            NOWA::GraphicsModule::getInstance()->setCameraOrientation(this->camera, Ogre::Quaternion(Ogre::Degree(90.0f), Ogre::Vector3::UNIT_X));
            break;
        }
        case EDITOR_CAMERA_VIEW_LEFT:
        {
            NOWA::GraphicsModule::getInstance()->setCameraOrientation(this->camera, Ogre::Quaternion(Ogre::Degree(-90.0f), Ogre::Vector3::UNIT_Y));
            break;
        }
    ...
    void EditorManager::handleMeshEditModifyEnd(EventDataPtr eventData)
    {
        boost::shared_ptr<EventDataMeshEditModifyEnd> castEventData = boost::static_pointer_cast<NOWA::EventDataMeshEditModifyEnd>(eventData);
        ...
        if (nullptr != meshEditComponentBase)
        {
            this->sceneManipulationCommandModule.pushCommand(std::make_shared<MeshEditUndoCommand>(castEventData->getOldData(), castEventData->getNewData(), castEventData->getGameObjectId()));
        }
    }

    void EditorManager::handlePlanetTerraModifyEnd(EventDataPtr eventData)
    {
        boost::shared_ptr<EventDataPlanetTerraModifyEnd> castEventData = boost::static_pointer_cast<NOWA::EventDataPlanetTerraModifyEnd>(eventData);

        GameObjectPtr gameObjectPtr = AppStateManager::getSingletonPtr()->getGameObjectController()->getGameObjectFromId(castEventData->getGameObjectId());

        if (nullptr != gameObjectPtr)
        {
            auto planetTerraComponent = NOWA::makeStrongPtr(gameObjectPtr->getComponent<NOWA::PlanetTerraComponentBase>());
            if (nullptr != planetTerraComponent)
            {
                this->sceneManipulationCommandModule.pushCommand(std::make_shared<PlanetTerraModifyUndoCommand>(castEventData->getOldPlanetData(), castEventData->getNewPlanetData(), castEventData->getGameObjectId()));
            }
        }
    }
    ...
    {
        boost::shared_ptr<EventDataCommandTransactionBegin> castEventData = boost::static_pointer_cast<EventDataCommandTransactionBegin>(eventData);
