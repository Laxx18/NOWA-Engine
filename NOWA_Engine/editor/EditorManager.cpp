			...
			for (auto it = this->gameObjectIds.begin(); it != this->gameObjectIds.end(); ++it)
			{
				...
				else
				{
					...
					NOWA::GraphicsModule::getInstance()->updateNodeTransform(gameObjectPtr->getSceneNode(), this->oldGameObjectDataList[i].oldPosition,
					 	this->oldGameObjectDataList[i].oldOrientation, this->oldGameObjectDataList[i].oldScale, false, true);
			...
			for (auto& it = this->gameObjectIds.begin(); it != this->gameObjectIds.end(); ++it)
			{
				...
				else
				{
					...
					NOWA::GraphicsModule::getInstance()->updateNodeTransform(gameObjectPtr->getSceneNode(), this->oldGameObjectDataList[i].newPosition,
					 	this->oldGameObjectDataList[i].newOrientation, this->oldGameObjectDataList[i].newScale, false, true);
	...
	void EditorManager::handleMousePress(const OIS::MouseEvent& evt, OIS::MouseButtonID id)
	{
		...
		if (id == this->mouseButtonId && EDITOR_PLACE_MODE == this->manipulationMode)
		{
			...
			if (nullptr != this->tempPlaceMovableObject && true == this->tempPlaceMovableObject->isAttached())
			{
				...
				ENQUEUE_RENDER_COMMAND("placenode reset orientation1",
				{
		...
		ENQUEUE_RENDER_COMMAND_MULTI("EditorManager::movePlaceNode", _1(hitRay),
		{
			...
			switch (this->placeMode)
			{
				...
				case EDITOR_PLACE_MODE_STACK_ORIENTATED:
				{
					...
					{
						ENQUEUE_RENDER_COMMAND("placenode reset orientation2",
						{
							this->placeNode->resetOrientation();
						});	
					}
