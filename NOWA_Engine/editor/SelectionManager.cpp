...
namespace NOWA
{
    ...
    public:
        ...
        {
            // Capture new selection now (before we overwrite it)
        ...
        {
            this->selectionManager->clearSelection();
            for (unsigned long id : this->newSelectedGameObjectIds)
            {
                GameObjectPtr go = AppStateManager::getSingletonPtr()->getGameObjectController()->getGameObjectFromId(id);
                if (nullptr != go)
                {
    ...
    void SelectionManager::init(Ogre::SceneManager* sceneManager, Ogre::Camera* camera, const Ogre::String& categories, OIS::MouseButtonID mouseButtonId, SelectionManager::ISelectionObserver* selectionObserver, const Ogre::String& materialName)
    {
        ...
        if (nullptr == this->selectionObserver)
        {
            ...
            throw Ogre::Exception(Ogre::Exception::ERR_INVALID_STATE, "[SelectionManager] Cannot init selection manager, because there is no selection observer implemented. Implement the ISelectionInterface first.", "NOWA");
        }
    ...
    void SelectionManager::queueSelectionEvent(unsigned long id, bool bSelected, bool bPartOfMultiselection)
    {
        boost::shared_ptr<NOWA::EventDataGameObjectSelected> evt(new NOWA::EventDataGameObjectSelected(id, bSelected, bPartOfMultiselection));
    ...
    void SelectionManager::applySelectInternal(GameObject* gameObject, bool bSelect, bool bPartOfMultiselection)
    {
        ...
        if (true == bSelect)
        {
            ...
            this->queueSelectionEvent(gameObject->getId(), true, bPartOfMultiselection);
        ...
        else
        {
            ...
            if (it != this->selectedGameObjects.end())
            {
                ...
                this->queueSelectionEvent(it->second.gameObject->getId(), false, false);
    ...
    void SelectionManager::internalSnapshotGameObjectSelection(const std::vector<unsigned long>& oldIds)
    {
        if (oldIds.size() > 0)
        {
            this->selectionCommandModule.pushCommand(std::make_shared<GameObjectsSelectUndoCommand>(this, oldIds));
        }
    }
    ...
    void SelectionManager::handleMousePress(const OIS::MouseEvent& evt, OIS::MouseButtonID id)
    {
        ...
        {
            // Capture OLD selection before any changes
            std::vector<unsigned long> oldIds;
            if (false == Core::getSingletonPtr()->getIsGame())
            {
                oldIds.reserve(this->selectedGameObjects.size());
                for (auto& kv : this->selectedGameObjects)
                {
                    oldIds.push_back(kv.second.gameObject->getId());
                }
            }
            ...
            if (0 != pickedId)
            {
                ...
                if (nullptr != gameObjectPtr)
                {
                    ...
                    if (it != this->selectedGameObjects.end())
                    {
                        this->applySelectInternal(gameObjectPtr.get(), false, false);
                    }
                    else
                    {
                        this->applySelectInternal(gameObjectPtr.get(), true, false);
                    }
                }
            }
            else
            ...
            // Snapshot with the OLD ids captured before any changes
            if (false == Core::getSingletonPtr()->getIsGame())
            {
                this->internalSnapshotGameObjectSelection(oldIds);
            }
        }
    ...
    void SelectionManager::selectGameObjects(const Ogre::Vector2& leftTopCorner, const Ogre::Vector2& bottomRightCorner)
    {
        ...
        for (auto* movable : qryresult.movables)
        {
            ...
            for (size_t i = 0; i < indexCount; ++i)
            {
                ...
                if (screenSpacePos.x > ndcLeft && screenSpacePos.x < ndcRight && screenSpacePos.y < ndcTop && screenSpacePos.y > ndcBottom)
                {
                    hitfound = true;
        ...
        for (unsigned long id : *hitIds)
        {
            ...
            if (goPtr)
            {
                this->applySelectInternal(goPtr.get(), true, true);
            }
        }
    }
    ...
    void SelectionManager::clearSelection(void)
    {
        for (auto it = this->selectedGameObjects.begin(); it != this->selectedGameObjects.end();)
        {
            ...
            this->applySelectInternal(gameObject, false, true);
        }
