...
namespace NOWA
{
    namespace KI
    {
        template <class Owner> class EXPORTED LuaGoalComposite : public LuaGoal<Owner>
        {
        public:
            LuaGoalComposite(Owner* owner, const luabind::object& thisLuaGoal) : LuaGoal<Owner>(owner, thisLuaGoal)
            {
            }
            ...
            virtual void activate(void)
            {
                ...
                catch (luabind::error& error)
                {
                    ...
                    Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[LuaGoalComposite] Caught error in 'activate' Error: " + Ogre::String(error.what()) + " details: " + msg.str());
                }
            }
            ...
            //-----------------------------------------------------------------------------
            // NOTE: The public no-arg version has been removed. Use process(dt) instead.
            ...
            virtual void terminate(void)
            {
                ...
                catch (luabind::error& error)
                {
                    ...
                    Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[LuaGoalComposite] Caught error in 'terminate' Error: " + Ogre::String(error.what()) + " details: " + msg.str());
                }
            }

            //! @brief Adds a subgoal to the back of the subgoal list (FIFO execution order)
            //! @param luaGoalTable  A Lua table defining activate / process / terminate.
            template <class Owner> void addSubGoal(luabind::object luaGoalTable)
            {
                // object.hpp exposes type checking as the FREE FUNCTION luabind::type(obj),
                // NOT as a member method .type(). Using activateFunc.type() does not compile.
                luabind::object activateFunc = luaGoalTable["activate"];
                if (!activateFunc.is_valid() || luabind::type(activateFunc) == LUA_TNIL)
                {
                    Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[LuaGoalComposite] addSubGoal: LuaGoal table must define an 'activate' function.");
                    return;
                }

                // push_back keeps insertion order so goals execute in the order they were added.
                this->subGoals.push_back(new LuaGoal<Owner>(this->owner, luaGoalTable));
            }
            ...
            template <class Owner> void removeAllSubGoals(void)
            {
        ...
        private:
            ...
            //  subgoal list. It then processes the next goal in the list (if there is one)
            //-----------------------------------------------------------------------------
            template <class Owner> boost::shared_ptr<GoalResult> processSubGoals(Ogre::Real dt)
            {
            ...
            template <class Owner> int getType(void) const
            {
                return this->type;
            }
