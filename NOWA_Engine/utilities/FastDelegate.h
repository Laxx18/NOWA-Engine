...
public:
	...
	inline void CopyFrom (DerivedClass *parent, const DelegateMemento &x) {
		SetMementoFrom(x);
		if (m_pStaticFunction!=0) {
			// transform self references...
			m_pthis=reinterpret_cast<GenericClass *>(parent);
		}
	}
	...
	inline void bindstaticfunc(DerivedClass *parent, ParentInvokerSig static_function_invoker, 
				StaticFuncPtr function_to_bind ) {
		if (function_to_bind==0) { // cope with assignment to 0
			m_pFunction=0;
		} else { 
			bindmemfunc(parent, static_function_invoker);
        }
...
// support static_cast between void * and function pointers.
	...
	template< class DerivedClass >
	inline void CopyFrom (DerivedClass *parent, const DelegateMemento &right) {
		SetMementoFrom(right);
	}
	// For static functions, the 'static_function_invoker' class in the parent 
	// will be called. The parent then needs to call GetStaticFunction() to find out 
