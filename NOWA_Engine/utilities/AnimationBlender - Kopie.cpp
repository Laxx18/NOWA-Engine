	...
	void AnimationBlender::addTime(Ogre::Real time)
	{
		...
		if (this->source != nullptr)
		{
			...
			if (this->source->getTimePosition() + 0.02f >= this->source->getLength())
			{
				...
				if (this->previousSource != nullptr && this->complete)
				{
					Ogre::Real blendFactor = std::min(1.0f, time / this->duration);  // Gradual weight increase
					this->previousSource->setWeight(this->source->getWeight() + blendFactor);
				}
				...
				if (nullptr != this->animationBlenderObserver)
				{
					...
					// If observer should react only once, delete it after reacting
