...
namespace NOWA
{
    ...
    public:
        ...
        /**
         * @brief		Sets whether a loading indicator is shown while the scene is parsed.
         * @param[in]	show	True to show one.
         * @note		If no indicator was set via setLoadingIndicator(), a DotsTextLoadingIndicator
         *				is created internally and destroyed again afterwards.
         * @note		Attention: the indicator is only visible for as long as loading actually
         *				takes. A scene that parses in 300 ms will barely flash it. There is
         *				deliberately NO minimum display time - an artificial delay would work
         *				against the loading performance.
         */
        void setShowLoadingIndicator(bool show);

        /**
         * @brief		Sets which indicator to show.
         * @param[in]	indicator	The indicator, or nullptr to fall back to the internal default.
         * @note		Attention: ownership stays with the caller. The object must stay alive for
         *				as long as it can be used, i.e. across every parse call.
         */
    ...
    protected:
        ...
        /**
         * @brief		Shows the loading indicator and makes sure something can be rendered at all.
         * @note		Attention: runs ON THE RENDER THREAD. If no workspace exists - which is the
         *				normal case, because the scene is destroyed before parsing - a temporary
         *				camera plus dummy workspace is created here, otherwise renderOneFrame would
         *				have nothing to draw into.
         */
        void beginLoadingIndicator(void);

        /**
         * @brief		Hides the loading indicator and removes the temporary camera again.
         * @note		Attention: runs ON THE RENDER THREAD, and must run even when parsing threw.
         */
        void endLoadingIndicator(void);
    ...
    protected:
        ...
        // Attention: only set when we created the default indicator ourselves and therefore have
        // to delete it again. An indicator handed in via setLoadingIndicator() is never deleted.
        ILoadingIndicator* ownedLoadingIndicator;
        Ogre::Camera* temporaryLoadingCamera;
    };
