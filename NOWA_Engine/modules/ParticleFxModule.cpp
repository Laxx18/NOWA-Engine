...
// Attention: TEMPORARY diagnostic for the double free in ParticleSystemDef::_destroy during
// scene teardown. Comment this out once the cause is identified.
// #define NOWA_PARTICLE_FX_DIAGNOSTIC

#include "RenderQueueEnums.h"

namespace NOWA
{
    ParticleFxModule::ParticleFxModule(const Ogre::String& appStateName) : appStateName(appStateName), sceneManager(nullptr)
    {
    ...
    void ParticleFxModule::destroyContent(void)
    {
        ...
        // Attention: NO destroyAllParticleSystems() / destroyAllParticleSystems2() here.
        //
        // Both end up in ParticleSystemDef::_destroyAllParticleSystems(), which after deleting the
        // instances does:
        //
        ...
        //
        // The manager variant is additionally wrong because it deletes the ParticleSystem2 instances
        // WITHOUT removing them from the SceneManager's MovableObjectCollection, where
        // createParticleSystem2() had registered them via createMovableObject().
        //
        // None of it is necessary: Core::destroyScene -> clearScene(true) destroys the instances
        // through ParticleSystem2Factory::destroyInstance() -> _destroyParticleSystem(), which is the
        // correct path, and ~ParticleSystemManager2 then calls _destroy() on every def to free the
        // buffers. _destroy() does NOT contain that write. Ogre cleans up correctly on its own.
        if (nullptr != this->particleManager)
        {
    ...
    void ParticleFxModule::createParticleSystem(const Ogre::String& name, const Ogre::String& templateName, Ogre::Real playTimeMS, const Ogre::Quaternion& orientation, const Ogre::Vector3& position, const Ogre::Vector2& scale, bool repeat,
        Ogre::Real playSpeed, ParticleBlendingMethod::ParticleBlendingMethod blendingMethod, bool fadeIn, Ogre::Real fadeInTimeMS, bool fadeOut, Ogre::Real fadeOutTimeMS)
    {
        GraphicsModule::RenderCommand renderCommand = [this, name, templateName, playTimeMS, orientation, position, scale, repeat, playSpeed, blendingMethod, fadeIn, fadeInTimeMS, fadeOut, fadeOutTimeMS]()
        {
...
#ifdef NOWA_PARTICLE_FX_DIAGNOSTIC
                // Reusing an existing clone. Attention: clonedDefName is derived from the TEMPLATE name
                // only, so every ParticleFxComponent using the same template lands on the SAME def here.
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[ParticleFxDiag] REUSING existing clone '" + cloneName + "' ptr=" + Ogre::StringConverter::toString(reinterpret_cast<size_t>(particleSystemDefInstance)) +
                                                                                        " initialized=" + Ogre::StringConverter::toString(nullptr != particleSystemDefInstance ? particleSystemDefInstance->isInitialized() : false));
#endif
...
#ifdef NOWA_PARTICLE_FX_DIAGNOSTIC
                const bool baseWasInitializedBeforeClone = (nullptr != baseDef) ? baseDef->isInitialized() : false;
#endif
...
#ifdef NOWA_PARTICLE_FX_DIAGNOSTIC
                // Attention: TEMPORARY diagnostic. If the freshly cloned def already reports
                // isInitialized(), then clone() copied the initialized state INCLUDING the
                // mParticleCpuData pointers. Base and clone would then share the same SIMD buffers,
                // init() below is skipped, and ParticleSystemManager2's destructor frees those buffers
                // once per def - which is the crash in ParticleSystemDef::_destroy / OGRE_FREE_SIMD.
                //
                // basePtr == clonePtr would mean clone() did not clone at all but handed back the base.
                if (nullptr != particleSystemDefInstance && nullptr != baseDef)
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                        "[ParticleFxDiag] cloned '" + cloneName + "' from '" + templateName + "' basePtr=" + Ogre::StringConverter::toString(reinterpret_cast<size_t>(baseDef)) +
                            " clonePtr=" + Ogre::StringConverter::toString(reinterpret_cast<size_t>(particleSystemDefInstance)) + " baseInitializedBeforeClone=" + Ogre::StringConverter::toString(baseWasInitializedBeforeClone) +
                            " baseInitializedNow=" + Ogre::StringConverter::toString(baseDef->isInitialized()) + " cloneInitialized=" + Ogre::StringConverter::toString(particleSystemDefInstance->isInitialized()) +
                            " samePointer=" + Ogre::StringConverter::toString(baseDef == particleSystemDefInstance));
                }
                else
                {
                    Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[ParticleFxDiag] clone FAILED for '" + cloneName + "' from '" + templateName + "' (baseDef=" + Ogre::String(nullptr != baseDef ? "valid" : "NULL") +
                                                                                            ", clone=" + Ogre::String(nullptr != particleSystemDefInstance ? "valid" : "NULL") + ")");
                }
#endif
...
#ifdef NOWA_PARTICLE_FX_DIAGNOSTIC
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[ParticleFxDiag] init() called on '" + cloneName + "' - it allocated its OWN buffers.");
#endif
            }
#ifdef NOWA_PARTICLE_FX_DIAGNOSTIC
            else if (nullptr != particleSystemDefInstance)
            {
                // Attention: this is the dangerous case. init() is skipped because the def already
                // claims to be initialized. For a REUSED clone that is correct and expected. For a
                // FRESHLY cloned def it means it inherited the base's buffers - see the note above.
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[ParticleFxDiag] init() SKIPPED on '" + cloneName + "' - it already reports isInitialized().");
            }
#endif
    ...
    void ParticleFxModule::destroyEverything(ParticleFxData& particleData)
    {
        ...
        // Attention: The clone ParticleSystemDef is deliberately NOT touched here anymore.
        //
        // The former code called clone->_destroyAllParticleSystems(). That is an INTERNAL Ogre method
        // (leading underscore) and it was redundant on top: destroyParticleEffect() above already
        // destroyed the instance through the public sceneManager->destroyParticleSystem2(), which
        // goes via ParticleSystem2Factory::destroyInstance() -> _destroyParticleSystem() and correctly
        // removes it from BOTH the def's mParticleSystems and the SceneManager's collection.
        //
        // _destroyAllParticleSystems() does neither: it deletes the instances behind the
        // SceneManager's back and then writes into mParticleCpuData.mTimeToLive without checking
        // whether the def is initialized - see the longer note in destroyContent().
        //
        // The def itself belongs to ParticleSystemManager2 and is destroyed with the SceneManager.
    ...
    ParticleBlendingMethod::ParticleBlendingMethod ParticleFxModule::detectBlendingModeFromMaterial(const Ogre::String& materialName)
    {
        ...
        try
        {
            ...
            if (!scriptNames || scriptNames->empty())
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ParticleFxModule] No material files found, using default blending mode");
                return defaultMode;
            }
    ...
    ParticleBlendingMethod::ParticleBlendingMethod ParticleFxModule::parseMaterialFileForBlendingMode(const Ogre::String& fileContent, const Ogre::String& materialName)
    {
        ...
        if (bmPos != Ogre::String::npos)
        {
            ...
            if (modeEnd != Ogre::String::npos)
            {
                ...
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ParticleFxModule] Found blending mode annotation: '" + modeStr + "' for material: " + materialName);
            }
        }
        else
        {
            // No annotation found
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ParticleFxModule] No blending mode annotation found for material: " + materialName);
        }
    ...
    void ParticleFxModule::precacheAllMaterialAnnotations(void)
    {
        try
        {
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ParticleFxModule] Cached annotations for " + Ogre::StringConverter::toString(this->materialBlendingModeCache.size()) + " materials (" +
                                                                                   Ogre::StringConverter::toString(this->materialBlendingModeCache.size()) + " with texture atlas)");
        }
        catch (Ogre::Exception& e)
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[ParticleFxModule] Error caching material annotations: " + e.getFullDescription());
        }
    }
    ...
    void ParticleFxModule::parseAndCacheAllAnnotationsInFile(const Ogre::String& fileContent)
    {
        ...
        while (pos < fileContent.length())
        {
            ...
            if (hlmsPos != Ogre::String::npos && (matPos == Ogre::String::npos || hlmsPos < matPos))
            {
                materialPos = hlmsPos;
