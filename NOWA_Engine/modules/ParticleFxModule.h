...
namespace NOWA
{
    ...
    struct EXPORTED ParticleFxData
    {
        ParticleFxData() :
            ...
            particleGlobalPosition(Ogre::Vector3::ZERO),
            particleGlobalOrientation(Ogre::Quaternion::IDENTITY),
        ...
        // World transform of the particle node.
        // Attention: this is NOT the same as particleOffsetPosition / particleOffsetOrientation.
        // Those two hold the LOCAL offset the designer configured on the component and are only
        // used to compute the world transform on the component side.
        // The node itself is created lazily in createParticleEffect() on the first play, which
        // happens long after the owning component knows where the effect belongs. Storing the
        // world transform here keeps it alive across that gap, so createParticleEffect() can
        // place the node correctly instead of leaving it at the world origin.
        Ogre::Vector3 particleGlobalPosition;
        Ogre::Quaternion particleGlobalOrientation;
        bool hasGlobalTransform;
    };
    ...
    private:
        struct TextureAtlasInfo
        {
            ...
            TextureAtlasInfo() : hasAtlas(false), rows(1), columns(1), fps(16.0f), currentTime(0.0f), currentFrame(0)
            {
            }
            TextureAtlasInfo(int r, int c, Ogre::Real f) : hasAtlas(true), rows(r), columns(c), fps(f), currentTime(0.0f), currentFrame(0)
            {
            }
        };
    ...
    public:
         ...
         */
        void createParticleSystem(const Ogre::String& name, const Ogre::String& templateName, Ogre::Real playTimeMS, const Ogre::Quaternion& offsetOrientation = Ogre::Quaternion::IDENTITY, const Ogre::Vector3& offsetPosition = Ogre::Vector3::ZERO,
            const Ogre::Vector2& scale = Ogre::Vector2::UNIT_SCALE, bool repeat = false, Ogre::Real playSpeed = 1.0f, ParticleBlendingMethod::ParticleBlendingMethod blendingMethod = ParticleBlendingMethod::AlphaBlending, bool fadeIn = false,
            Ogre::Real fadeInTimeMS = 1000.0f, bool fadeOut = false, Ogre::Real fadeOutTimeMS = 1000.0f);
