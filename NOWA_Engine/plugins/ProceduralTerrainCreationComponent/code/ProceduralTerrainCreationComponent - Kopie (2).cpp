...
namespace NOWA
{
	...
	ProceduralTerrainCreationComponent::ProceduralTerrainCreationComponent()
		...
		hillFrequency(new Variant(ProceduralTerrainCreationComponent::AttrHillFrequency(), 0.0002f, this->attributes)),
	...
	std::vector<ProceduralTerrainCreationComponent::RoadPoint>
		ProceduralTerrainCreationComponent::generateRoadPath(const Ogre::Vector2& start, const Ogre::Vector2& end, int numSegments)
	{
		...
		int numWaypoints = static_cast<int>(2 + curviness * 4); // 2-6 waypoints depending on curviness

		std::mt19937 rng(this->seed->getUInt());

		// Calculate perpendicular direction for random offsets
		Ogre::Vector2 dir = (end - start);
		float roadLength = dir.length();
		dir.normalise();
		Ogre::Vector2 perpendicular(-dir.y, dir.x);

		// Max offset is proportional to road length and curviness
		float maxOffset = (roadLength * 0.2f) * curviness;
		...
		while (controlPoints.size() < 4)
		{
			Ogre::Vector2 mid = (controlPoints[0] + controlPoints.back()) * 0.5f;
			controlPoints.insert(controlPoints.begin() + controlPoints.size() / 2, mid);
		}
	...
	// NEW: Properly carve roads into the heightmap with flattening
		...
		Ogre::uint32 height)
	{
		...
		// Check influence from all roads
		for (const auto& road : roads)
		{
			// Find closest point on this road
			float minDist = std::numeric_limits<float>::max();
			float closestRoadHeight = 0.0f;
			...
			for (const auto& rp : road)
			{
				...
				{
					minDist = dist;
					closestRoadHeight = rp.targetHeight;
				}
			}

			// Calculate influence based on distance from road center
			float influenceRadius = roadWidthVal * smoothness;
			if (minDist < influenceRadius)
			{
				// Smooth falloff curve
				float influence = 1.0f - (minDist / influenceRadius);
				influence = std::pow(influence, 1.5f); // Power curve for smooth blending

				// CRITICAL: Flatten the road - set height to road's target height
				// This is what makes roads visible!
				float targetHeight = closestRoadHeight - depthVal;

				// If within road width, FORCE flatten
				if (minDist < roadWidthVal * 0.5f)
				{
					// Inside road - fully flatten
					influence = 1.0f;
				}

				// Blend toward flattened road height
				finalHeight = finalHeight * (1.0f - influence) + targetHeight * influence;
	...
	void ProceduralTerrainCreationComponent::generateProceduralTerrain(void)
	{
		...
		NOWA::GraphicsModule::RenderCommand renderCommand = [this, terra]()
		{
			...
			// PASS 1: Generate base terrain WITHOUT roads
			std::vector<float> baseTerrainHeights(width * height);
			...
			for (Ogre::uint32 y = 0; y < height; ++y)
			{
				for (Ogre::uint32 x = 0; x < width; ++x)
				{
					...
					baseTerrainHeights[y * width + x] = terrainHeight;
				}
			}

			// PASS 2: Generate roads (if enabled) and carve them into terrain
			std::vector<std::vector<RoadPoint>> roads;
			...
			if (this->enableRoads->getBool())
			{
				...
				for (Ogre::uint32 i = 0; i < numRoads; ++i)
				{
					...
					// Generate more segments for smoother roads
					roads.push_back(generateRoadPath(start, end, 200));
				}

				// Calculate road heights based on the BASE terrain (before carving)
				calculateRoadHeights(roads, baseTerrainHeights, width, height);

				Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_NORMAL, "[ProceduralTerrainCreationComponent] Generated " + Ogre::StringConverter::toString(numRoads) + " road paths");
			}

			// PASS 3: Combine base terrain with carved roads
			std::vector<float> finalHeightData(width * height);
			...
			for (Ogre::uint32 y = 0; y < height; ++y)
			{
				...
				{
					int index = y * width + x;
					float terrainHeight = baseTerrainHeights[index];

					// Apply road carving if enabled
					if (this->enableRoads->getBool() && !roads.empty())
					{
						Ogre::Vector2 pos(static_cast<float>(x), static_cast<float>(y));
						terrainHeight = applyRoadCarving(terrainHeight, pos, roads, width, height);
					}

					finalHeightData[index] = terrainHeight;
				}
			}

			// PASS 4: Convert to Ogre::Image2 and apply to Terra
			Ogre::Image2 image;
			...
			for (Ogre::uint32 i = 0; i < width * height; ++i)
			{
				...
				float normalized = Ogre::Math::Clamp(finalHeightData[i] / maxHeight, 0.0f, 1.0f);
			...
			terra->update(Ogre::Vector3::ZERO);
