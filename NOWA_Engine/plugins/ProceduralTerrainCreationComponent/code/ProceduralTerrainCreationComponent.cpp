...
namespace NOWA
{
    ...
    ProceduralTerrainCreationComponent::ProceduralTerrainCreationComponent() :
        ...
        roadDepth(new Variant(ProceduralTerrainCreationComponent::AttrRoadDepth(), 0.05f, this->attributes)),
        ...
        cachedHeightRange(1.0f),
        cachedSeaLevelNorm(0.5f)
    {
        ...
        this->regenerate->addUserData(GameObject::AttrActionNeedRefresh());
    ...
    bool ProceduralTerrainCreationComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
        ...
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrResolution())
        {
            this->resolution->setValue(XMLConverter::getAttribUnsignedInt(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrBaseHeight())
        {
        ...
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrRoadsClosed())
        {
        ...
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrRiverFlowThreshold())
        {
            this->riverFlowThreshold->setValue(XMLConverter::getAttribReal(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrRiverWidthScale())
        {
            this->riverWidthScale->setValue(XMLConverter::getAttribReal(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrRiverErosionFactor())
        {
            this->riverErosionFactor->setValue(XMLConverter::getAttribReal(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrMaxRiverDepth())
        {
        ...
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrCanyonMinWidth())
        {
            this->canyonMinWidth->setValue(XMLConverter::getAttribReal(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrCanyonMaxWidth())
        {
        ...
        // Erosion
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrEnableErosion())
        {
            this->enableErosion->setValue(XMLConverter::getAttribBool(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrErosionIterations())
        {
            this->erosionIterations->setValue(XMLConverter::getAttribUnsignedInt(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrErosionStrength())
        {
            this->erosionStrength->setValue(XMLConverter::getAttribReal(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == AttrSedimentCapacity())
        {
    ...
    void ProceduralTerrainCreationComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
    {
        ...
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrResolution())));
        ...
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrBaseHeight())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->baseHeight->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrHillAmplitude())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->hillAmplitude->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrHillFrequency())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->hillFrequency->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrOctaves())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->octaves->getUInt())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrPersistence())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->persistence->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrLacunarity())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->lacunarity->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrSeed())));
        ...
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrEnableRoads())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->enableRoads->getBool())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRoadCount())));
        ...
        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRoadDepth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->roadDepth->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRoadSmoothness())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->roadSmoothness->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRoadCurviness())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->roadCurviness->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "12"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRoadsClosed())));
        ...
        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "12"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrEnableRivers())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->enableRivers->getBool())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRiverCount())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->riverCount->getUInt())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRiverWidth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->riverWidth->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRiverDepth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->riverDepth->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRiverMeandering())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->riverMeandering->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRiverFlowThreshold())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->riverFlowThreshold->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRiverWidthScale())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->riverWidthScale->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrRiverErosionFactor())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->riverErosionFactor->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrMaxRiverDepth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->maxRiverDepth->getReal())));
        propertiesXML->append_node(propertyXML);

        // Canyons
        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "12"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrEnableCanyons())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->enableCanyons->getBool())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrCanyonCount())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->canyonCount->getUInt())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrCanyonWidth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->canyonWidth->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrCanyonDepth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->canyonDepth->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrCanyonSteepness())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->canyonSteepness->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrCanyonMinWidth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->canyonMinWidth->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrCanyonMaxWidth())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->canyonMaxWidth->getReal())));
        propertiesXML->append_node(propertyXML);

        // Island
        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "12"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrEnableIsland())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->enableIsland->getBool())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrIslandFalloff())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->islandFalloff->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrIslandSize())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->islandSize->getReal())));
        propertiesXML->append_node(propertyXML);

        // Erosion
        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "12"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrEnableErosion())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->enableErosion->getBool())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrErosionIterations())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->erosionIterations->getUInt())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrErosionStrength())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->erosionStrength->getReal())));
        propertiesXML->append_node(propertyXML);

        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
        propertyXML->append_attribute(doc.allocate_attribute("name", XMLConverter::ConvertString(doc, AttrSedimentCapacity())));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->sedimentCapacity->getReal())));
        propertiesXML->append_node(propertyXML);
    }
    ...
    static const int permutation[512] = {151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117, 35, 11, 32,
        57, 177, 33, 88, 237, 149, 56, 87, 174, 20, 125, 136, 171, 168, 68, 175, 74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83, 111, 229, 122, 60, 211, 133, 230, 220, 105, 92, 41, 55, 46, 245, 40, 244, 102, 143, 54, 65, 25, 63, 161, 1,
        216, 80, 73, 209, 76, 132, 187, 208, 89, 18, 169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126, 255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17, 182,
        189, 28, 42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101, 155, 167, 43, 172, 9, 129, 22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104, 218, 246, 97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191,
        179, 162, 241, 81, 51, 145, 235, 249, 14, 239, 107, 49, 192, 214, 31, 181, 199, 106, 157, 184, 84, 204, 176, 115, 121, 50, 45, 127, 4, 150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180,
        // Repeat the array to avoid overflow
        151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117, 35, 11, 32, 57, 177, 33, 88, 237, 149, 56,
        87, 174, 20, 125, 136, 171, 168, 68, 175, 74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83, 111, 229, 122, 60, 211, 133, 230, 220, 105, 92, 41, 55, 46, 245, 40, 244, 102, 143, 54, 65, 25, 63, 161, 1, 216, 80, 73, 209, 76, 132, 187,
        208, 89, 18, 169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126, 255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17, 182, 189, 28, 42, 223, 183, 170, 213,
        119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101, 155, 167, 43, 172, 9, 129, 22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104, 218, 246, 97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191, 179, 162, 241, 81, 51, 145, 235,
        249, 14, 239, 107, 49, 192, 214, 31, 181, 199, 106, 157, 184, 84, 204, 176, 115, 121, 50, 45, 127, 4, 150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180};
    ...
    std::vector<ProceduralTerrainCreationComponent::RoadPoint> ProceduralTerrainCreationComponent::generateRoadPath(const Ogre::Vector2& startPixels, const Ogre::Vector2& endPixels, int numSegments)
    {
        ...
        if (isClosed)
        {
            ...
            std::uniform_real_distribution<float> radiusVar(0.55f, 1.45f); // ±45% variation
            std::uniform_real_distribution<float> angleJitter(-0.35f, 0.35f);

            for (int i = 0; i < numCorners; ++i)
            {
                float baseAngle = (i * 2.0f * Ogre::Math::PI) / numCorners;
                float angle = baseAngle + angleJitter(rng) * curviness;
                float r = baseRadius * (1.0f + (radiusVar(rng) - 1.0f) * curviness);

                Ogre::Vector2 point(center.x + std::cos(angle) * r, center.y + std::sin(angle) * r);
    ...
    void ProceduralTerrainCreationComponent::calculateRoadHeights(std::vector<std::vector<RoadPoint>>& roads, const std::vector<float>& baseTerrainHeights, Ogre::uint32 width, Ogre::uint32 height)
    {
        for (auto& road : roads)
        {
            for (auto& point : road)
    ...
    void ProceduralTerrainCreationComponent::smoothRoadHeights(std::vector<RoadPoint>& road)
    {
        ...
        // Grade constraint: max normalized height change per road-point step.
        // With 300 road points over 200m: each step ≈ 0.67m.
        // 0.002 per step ≈ 0.3% grade — gentle enough for a road, steep enough to
        // still cut visibly through hills.
        const float maxGradePerStep = 0.002f;

        // --- Forward pass: limit uphill grade ---
        for (size_t i = 1; i < road.size(); ++i)
        {
            float delta = road[i].targetHeight - road[i - 1].targetHeight;
            if (delta > maxGradePerStep)
            {
                road[i].targetHeight = road[i - 1].targetHeight + maxGradePerStep;
            }
        }

        // --- Backward pass: limit downhill grade (same constraint, reversed) ---
        for (int i = static_cast<int>(road.size()) - 2; i >= 0; --i)
        {
            float delta = road[i].targetHeight - road[i + 1].targetHeight;
            if (delta > maxGradePerStep)
            {
        ...
        // --- Final cosmetic smoothing passes to remove any grade-clamping kinks ---
        for (int pass = 0; pass < 8; ++pass)
        {
            for (size_t i = 1; i < road.size() - 1; ++i)
            {
                road[i].targetHeight = road[i - 1].targetHeight * 0.25f + road[i].targetHeight * 0.50f + road[i + 1].targetHeight * 0.25f;
            }
        }

        // For closed roads: blend the junction so start height == end height.
        // Check if this looks like a closed road (first and last points near each other).
        if (road.size() > 10)
        {
            float startH = road.front().targetHeight;
            float endH = road.back().targetHeight;
            float diff = endH - startH;

            // Linearly ramp the height mismatch out over the last 20% of the road
            size_t blendLen = road.size() / 5;
            for (size_t i = 0; i < blendLen; ++i)
            {
                float t = static_cast<float>(i) / static_cast<float>(blendLen);
                // Adjust end side toward start
                size_t idx = road.size() - 1 - i;
                road[idx].targetHeight -= diff * (1.0f - t);
            }
        }
    }

    float ProceduralTerrainCreationComponent::applyRoadCarving(float baseHeight, const Ogre::Vector2& pixelPosition, const std::vector<std::vector<RoadPoint>>& roads, Ogre::uint32 width, Ogre::uint32 height)
    {
        if (roads.empty())
        {
            return baseHeight;
        ...
        // Use the cached value — correct for ANY Terra size, not just 100m hardcoded
        float roadWidthPixels = this->roadWidth->getReal() * this->cachedPixelsPerMeter;
        float roadDepthNorm = this->roadDepth->getReal();
        float smoothness = this->roadSmoothness->getReal();

        // halfWidth  = the flat road surface zone
        // blendWidth = outer edge of the shoulder blend (road + both shoulders)
        float halfWidth = roadWidthPixels * 0.5f;
        float blendWidth = halfWidth + roadWidthPixels * smoothness;

        float minDist = std::numeric_limits<float>::max();
        float closestHeight = baseHeight;

        for (const auto& road : roads)
        {
            for (size_t i = 1; i < road.size(); ++i)
            {
                const Ogre::Vector2& a = road[i - 1].position;
                const Ogre::Vector2& b = road[i].position;

                // Project pixel position onto segment AB, get parameter t in [0,1]
                Ogre::Vector2 ab = b - a;
                float abLen2 = ab.dotProduct(ab);
                float t = 0.0f;
                if (abLen2 > 1e-6f)
                {
                    t = Ogre::Math::Clamp((pixelPosition - a).dotProduct(ab) / abLen2, 0.0f, 1.0f);
                }

                Ogre::Vector2 proj = a + ab * t;
                float dist = (pixelPosition - proj).length();

                if (dist < minDist)
                {
                    minDist = dist;
                    // Interpolated road height at exact projection point along segment
                    closestHeight = road[i - 1].targetHeight * (1.0f - t) + road[i].targetHeight * t;
                }
            }
        }
        ...
        // Road surface is the grade-constrained smooth elevation, minus a small drainage cut.
        float roadSurfaceHeight = closestHeight - roadDepthNorm;

        // Cosine falloff: smooth S-curve from road surface to natural terrain
        float influence;
        if (minDist <= halfWidth)
        {
            influence = 1.0f; // Fully on road — set to road grade exactly
        }
        else
        {
            float t = (minDist - halfWidth) / (blendWidth - halfWidth); // 0 at edge of road, 1 at edge of shoulder
            influence = 0.5f + 0.5f * std::cos(t * Ogre::Math::PI);     // smooth cosine, not linear
        }

        // Blend from road surface back to natural terrain in the shoulder zone.
        // No min() clamp here: allow fills (embankments) as well as cuts.
        // Real roads do both — fill a valley, cut through a hill.
        return roadSurfaceHeight + (baseHeight - roadSurfaceHeight) * (1.0f - influence);
    }

    std::vector<ProceduralTerrainCreationComponent::RiverPath> ProceduralTerrainCreationComponent::traceRiverPaths(const std::vector<float>& heights, Ogre::uint32 width, Ogre::uint32 height)
    {
        std::vector<RiverPath> rivers;

        Ogre::uint32 numRivers = this->riverCount->getUInt();
        float meandering = this->riverMeandering->getReal();
        float riverWidthPixels = this->riverWidth->getReal() * this->cachedPixelsPerMeter;
        float depthNorm = this->riverDepth->getReal() / this->cachedHeightRange;

        // Step must be small enough for 8-neighbor steering to work
        const float stepSize = 1.5f;
        const int maxSteps = static_cast<int>(std::max(width, height) * 3);

        // Minimum meaningful path before we accept a river
        const int minPoints = 30;

        std::mt19937 rng(this->seed->getUInt() + 42);
        std::uniform_int_distribution<int> distX(static_cast<int>(width * 0.1f), static_cast<int>(width * 0.9f));
        std::uniform_int_distribution<int> distY(static_cast<int>(height * 0.1f), static_cast<int>(height * 0.9f));
        std::uniform_real_distribution<float> noiseDist(-1.0f, 1.0f);

        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] River params: widthPx=" + Ogre::StringConverter::toString(riverWidthPixels) + "  depthNorm=" + Ogre::StringConverter::toString(depthNorm) +
                                                                               "  seaLevel=" + Ogre::StringConverter::toString(cachedSeaLevelNorm) + "  heightRange=" + Ogre::StringConverter::toString(cachedHeightRange));

        // Sample candidates, pick highest points as river sources
        std::vector<std::pair<float, Ogre::Vector2>> candidates;
        candidates.reserve(400);
        for (int i = 0; i < 400; ++i)
        {
            int x = distX(rng);
            int y = distY(rng);
            candidates.push_back({heights[y * width + x], Ogre::Vector2(static_cast<float>(x), static_cast<float>(y))});
        }
        ...
        for (size_t ci = 0; ci < candidates.size() && static_cast<Ogre::uint32>(rivers.size()) < numRivers; ++ci)
        {
            // Spread sources: skip if too close to an already accepted river source
            bool tooClose = false;
            for (const auto& existing : rivers)
            {
                if (!existing.points.empty() && (existing.points[0] - candidates[ci].second).length() < width * 0.2f)
                {
                    tooClose = true;
                    break;
                }
            ...
            RiverPath river;
            Ogre::Vector2 pos = candidates[ci].second;
            Ogre::Vector2 dir = Ogre::Vector2::ZERO;
            float angleWander = 0.0f;

            for (int step = 0; step < maxSteps; ++step)
            {
                int cx = static_cast<int>(pos.x + 0.5f);
                int cy = static_cast<int>(pos.y + 0.5f);

                // Terrain edge
                if (cx < 2 || cx >= static_cast<int>(width) - 2 || cy < 2 || cy >= static_cast<int>(height) - 2)
                {
                    break;
                }

                // Loop detection: only start after enough distance traveled,
                // and only compare against OLD points (exclude last 20 steps).
                // Threshold: within one step of an OLD point = genuine loop.
                if (step > 50 && river.points.size() > 25)
                {
                    size_t recentCutoff = river.points.size() - 20; // ignore last 20
                    size_t scanFrom = recentCutoff > 30 ? recentCutoff - 30 : 0;
                    float loopThresh = stepSize * stepSize; // within 1 step = looping

                    bool looping = false;
                    for (size_t pi = scanFrom; pi < recentCutoff; ++pi)
                    {
                        if ((pos - river.points[pi]).squaredLength() < loopThresh)
                        {
                            looping = true;
                            break;
                        }
                ...
                river.points.push_back(pos);
                river.widthsPixels.push_back(riverWidthPixels);
                river.depthsNorm.push_back(depthNorm);

                float currentH = heights[cy * width + cx];

                // Reached sea level: river mouth, stop
                if (currentH <= cachedSeaLevelNorm)
                {
                    break;
                }

                // 8-neighbour steepest descent
                float lowestH = currentH;
                Ogre::Vector2 steepestDir = Ogre::Vector2::ZERO;
                ...
                for (int dy = -1; dy <= 1; ++dy)
                {
                    for (int dx = -1; dx <= 1; ++dx)
                    {
                        ...
                        if (heights[ny * width + nx] < lowestH)
                        {
                            lowestH = heights[ny * width + nx];
                            steepestDir = Ogre::Vector2(static_cast<float>(dx), static_cast<float>(dy));
                            foundLower = true;
                        }
                    }
                }

                // Local flat basin: river stops
                if (!foundLower)
                {
                    break;
                }

                steepestDir.normalise();
                ...
                // Terrain dominates direction — high inertia resists sharp turns
                float inertia = 0.5f;
                dir = dir * inertia + steepestDir * (1.0f - inertia);

                // Meandering only on gentle slopes, kept very small to avoid circles
                float slope = currentH - lowestH;
                float slopeFactor = Ogre::Math::Clamp(1.0f - slope * 50.0f, 0.0f, 1.0f);
                angleWander += noiseDist(rng) * meandering * 0.04f * slopeFactor;
                angleWander *= 0.80f; // strong damping

                float angle = std::atan2(dir.y, dir.x) + angleWander;
                dir = Ogre::Vector2(std::cos(angle), std::sin(angle));
                dir.normalise();
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
                "[ProceduralTerrain] Candidate " + Ogre::StringConverter::toString(ci) + " h=" + Ogre::StringConverter::toString(candidates[ci].first) + " traced " + Ogre::StringConverter::toString(river.points.size()) + " pts");

            if (static_cast<int>(river.points.size()) >= minPoints)
            {
    ...
    void ProceduralTerrainCreationComponent::carveRiversStamp(std::vector<float>& heights, const std::vector<RiverPath>& rivers, Ogre::uint32 width, Ogre::uint32 height)
    {
        if (rivers.empty())
        {
            return;
        ...
        std::vector<float> influenceMap(width * height, 0.0f);
        ...
        for (const auto& river : rivers)
        {
            ...
            {
                int cx = static_cast<int>(river.points[i].x + 0.5f);
                int cy = static_cast<int>(river.points[i].y + 0.5f);
                float halfWidth = river.widthsPixels[i] * 0.5f;
                float blendW = halfWidth * 2.0f;   // shoulder = same width as channel
                float depth = river.depthsNorm[i]; // relative to local terrain
                int radius = static_cast<int>(blendW) + 1;
                ...
                for (int dy = -radius; dy <= radius; ++dy)
                {
                    for (int dx = -radius; dx <= radius; ++dx)
                    {
                        ...
                        // Cosine influence: 1 at centerline, 0 at shoulder edge
                        float inf;
                        if (dist <= halfWidth)
                        {
                            inf = 1.0f;
                        ...
                        {
                            float t = (dist - halfWidth) / (blendW - halfWidth);
                            inf = 0.5f + 0.5f * std::cos(t * Ogre::Math::PI);
                        }

                        int idx = py * width + px;
                        if (inf > influenceMap[idx])
                        {
                            influenceMap[idx] = inf;
                            depthMap[idx] = depth;
                        }
                    }
                }
        ...
        // Apply: carve terrain DOWN by depth * influence (relative to local height)
        int carved = 0;
        for (Ogre::uint32 i = 0; i < width * height; ++i)
        {
            if (influenceMap[i] > 0.001f)
            {
                float newH = heights[i] - depthMap[i] * influenceMap[i];
                // Never carve below terrain minimum (no inverted spikes)
        ...
        // Smooth only carved pixels — soften bank transitions
        ...
        for (Ogre::uint32 y = 1; y < height - 1; ++y)
        {
            ...
            {
                int idx = y * width + x;
                if (influenceMap[idx] < 0.001f)
                {
                    continue;
                }

                smoothed[idx] = heights[y * width + x] * 0.40f + heights[(y - 1) * width + x] * 0.10f + heights[(y + 1) * width + x] * 0.10f + heights[y * width + (x - 1)] * 0.10f + heights[y * width + (x + 1)] * 0.10f +
                                heights[(y - 1) * width + (x - 1)] * 0.05f + heights[(y - 1) * width + (x + 1)] * 0.05f + heights[(y + 1) * width + (x - 1)] * 0.05f + heights[(y + 1) * width + (x + 1)] * 0.05f;
            }
        }
        heights = smoothed;

        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Rivers carved " + Ogre::StringConverter::toString(carved) + " pixels (" + Ogre::StringConverter::toString(carved * 100.0f / (width * height)) + "%)");
    }
    ...
    void ProceduralTerrainCreationComponent::applyCanyonsToTerrain(std::vector<float>& heights, const std::vector<CanyonPath>& canyons, Ogre::uint32 width, Ogre::uint32 height)
    {
            ...
            for (size_t i = 0; i < canyon.points.size(); ++i)
            {
                ...
                for (int dy = -radius; dy <= radius; ++dy)
                {
                    for (int dx = -radius; dx <= radius; ++dx)
                    {
                        ...
                        // Gentle profile to prevent too-steep walls
                        float normalizedDist = distance / (canyonWidth * 0.5f);

                        // Use parabolic profile for gentle slopes
                        // steepness < 1.0 = gentler, > 1.0 = steeper
                        float steepness = Ogre::Math::Clamp(this->canyonSteepness->getReal(), 0.3f, 2.0f);

                        float profile = 1.0f - std::pow(normalizedDist, steepness);
                        profile = std::max(0.0f, profile);

                        // Don't directly subtract - blend for smoother result
                        float erosion = (canyonDepth / this->cachedHeightRange) * profile;
        ...
        // Multi-pass smoothing to prevent sharp edges
        for (int pass = 0; pass < 3; ++pass)
        {
            ...
            for (Ogre::uint32 y = 1; y < height - 1; ++y)
            {
                for (Ogre::uint32 x = 1; x < width - 1; ++x)
                {
                    ...
                    float neighbors = (heights[(y - 1) * width + x] + heights[(y + 1) * width + x] + heights[y * width + (x - 1)] + heights[y * width + (x + 1)]) * 0.125f;
    ...
    void ProceduralTerrainCreationComponent::simulateHydraulicErosion(std::vector<float>& heights, Ogre::uint32 width, Ogre::uint32 height)
    {
        ...
        for (Ogre::uint32 i = 0; i < numParticles; ++i)
        {
            ...
            for (int step = 0; step < 30 && particle.water > 0.01f; ++step)
            {
                ...
                // Check bounds
                if (particle.position.x < 0 || particle.position.x >= width - 1 || particle.position.y < 0 || particle.position.y >= height - 1)
                {
                    break;
                }
            ...
            if (particle.sediment > 0 && particle.position.x >= 0 && particle.position.x < width && particle.position.y >= 0 && particle.position.y < height)
            {
    ...
    void ProceduralTerrainCreationComponent::updateParticle(ErosionParticle& particle, std::vector<float>& heights, Ogre::uint32 width, Ogre::uint32 height)
    {
        ...
        // Scale gravity and friction for realistic behavior
        ...
        // Realistic erosion/deposition
        if (particle.sediment < capacity)
        {
            // Erode - but only if moving and on a slope
            if (speed > minSlope)
            {
                float amountToErode = this->erosionStrength->getReal() * (capacity - particle.sediment) * speed;
    ...
    void ProceduralTerrainCreationComponent::applyIslandMask(std::vector<float>& heights, Ogre::uint32 width, Ogre::uint32 height)
    {
        ...
        for (Ogre::uint32 y = 0; y < height; ++y)
        {
            for (Ogre::uint32 x = 0; x < width; ++x)
            {
                ...
                // Use smooth gradient, not harsh cutoff
                ...
                // Apply mask - Use multiplication, not addition
                int index = y * width + x;
                heights[index] *= mask;
                ...
                if (mask < 0.3f)
                {
                    ...
                    heights[index] = Ogre::Math::lerp(-oceanDepth, heights[index], mask / 0.3f);
                }
            }
        }
    }
    ...
    void ProceduralTerrainCreationComponent::generateProceduralTerrain(void)
    {
        ...
        this->cachedPixelsPerMeter = static_cast<float>(width) / xzDimensions.x;
        this->cachedHeightRange = terraHeightRange;
        // World Y=25 expressed in normalized [0,1] heightmap space
        // terraOrigin.y is the BOTTOM of the terrain (-25m in your setup)
        this->cachedSeaLevelNorm = (0.0f - terraOrigin.y) / terraHeightRange;
        ...
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] ========================================");
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Resolution: " + Ogre::StringConverter::toString(width));
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Terra XZ: " + Ogre::StringConverter::toString(xzDimensions.x) + "m x " + Ogre::StringConverter::toString(xzDimensions.y) + "m");
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Terra Height: " + Ogre::StringConverter::toString(terraMinHeight) + "m to " + Ogre::StringConverter::toString(terraMaxHeight) + "m");
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Pixels/meter: " + Ogre::StringConverter::toString(width / xzDimensions.x));
        ...
        for (Ogre::uint32 y = 0; y < height; ++y)
        {
            for (Ogre::uint32 x = 0; x < width; ++x)
            {
                heightData[y * width + x] = perlinNoise(static_cast<float>(x), static_cast<float>(y));
            }
        }
        ...
        if (this->enableRoads->getBool())
        {
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Generating " + Ogre::StringConverter::toString(numRoads) + " road(s)");
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Road: " + Ogre::StringConverter::toString(roadWidthMeters) + "m = " + Ogre::StringConverter::toString(roadWidthMeters * pixelsPerMeter) + " pixels");
            ...
            else
            {
                ...
                for (Ogre::uint32 i = 0; i < numRoads; ++i)
                {
                    // Each road picks a start edge and connects to the OPPOSITE edge.
                    // This guarantees roads traverse the terrain rather than scribbling inside.
                    int startEdge = static_cast<int>(i % 4); // cycle: top/right/bottom/left
                    int endEdge = (startEdge + 2) % 4;       // always opposite edge

                    float margin = width * 0.1f;
                    std::uniform_real_distribution<float> along(margin, static_cast<float>(width) - margin);

                    auto pickEdgePoint = [&](int edge) -> Ogre::Vector2
                    {
                        switch (edge)
                        {
                        case 0:
                            return Ogre::Vector2(along(rng), margin); // Top
                        case 1:
                            return Ogre::Vector2(static_cast<float>(width) - margin, along(rng)); // Right
                        case 2:
                            return Ogre::Vector2(along(rng), static_cast<float>(height) - margin); // Bottom
                        default:
                            return Ogre::Vector2(margin, along(rng)); // Left
                        }
                    };

                    Ogre::Vector2 start = pickEdgePoint(startEdge);
                    Ogre::Vector2 end = pickEdgePoint(endEdge);
                    roads.push_back(generateRoadPath(start, end, 300));
                }
            }

            this->calculateRoadHeights(roads, heightData, width, height);

            // Compute road geometry in pixels
            float roadWidthPixels = this->roadWidth->getReal() * this->cachedPixelsPerMeter;
            float halfWidth = roadWidthPixels * 0.5f;
            float blendWidth = halfWidth + roadWidthPixels * this->roadSmoothness->getReal();
            float roadDepthNorm = this->roadDepth->getReal();
            int stampRadius = static_cast<int>(blendWidth) + 1;

            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
                "[ProceduralTerrain] Road stamp: width=" + Ogre::StringConverter::toString(roadWidthPixels) + "px  blend=" + Ogre::StringConverter::toString(blendWidth) + "px  depth=" + Ogre::StringConverter::toString(roadDepthNorm));

            // Pre-bake: for each road point stamp a disc into the influence + height maps.
            // Where multiple roads overlap, highest influence wins.
            std::vector<float> roadInfluenceMap(width * height, 0.0f);
            std::vector<float> roadHeightMap(width * height, 0.0f);

            for (const auto& road : roads)
            {
                for (size_t i = 0; i < road.size(); ++i)
                {
                    int cx = static_cast<int>(road[i].position.x + 0.5f);
                    int cy = static_cast<int>(road[i].position.y + 0.5f);
                    float roadH = road[i].targetHeight - roadDepthNorm; // grade target minus drainage cut
                    ...
                    for (int dy = -stampRadius; dy <= stampRadius; ++dy)
                    {
                        for (int dx = -stampRadius; dx <= stampRadius; ++dx)
                        {
                            ...
                            // Cosine falloff: 1.0 on centerline, 0.0 at shoulder edge
                            float inf;
                            if (dist <= halfWidth)
                            {
                                inf = 1.0f;
                            ...
                            {
                                float t = (dist - halfWidth) / (blendWidth - halfWidth); // 0..1
                                inf = 0.5f + 0.5f * std::cos(t * Ogre::Math::PI);        // 1..0
                            }

                            int idx = py * width + px;
                            if (inf > roadInfluenceMap[idx])
                            {
                                roadInfluenceMap[idx] = inf;
                                roadHeightMap[idx] = roadH;
                            }
                        }
                    }
                }
            ...
            // Apply: blend terrain toward road grade.
            // influence=1 -> full road height (cuts hills, fills valleys)
            // influence=0 -> untouched terrain
            int affected = 0;
            for (Ogre::uint32 i = 0; i < width * height; ++i)
            {
                if (roadInfluenceMap[i] > 0.001f)
                {
                    float orig = heightData[i];
                    float target = roadHeightMap[i];
                    heightData[i] = orig + (target - orig) * roadInfluenceMap[i];
                    if (std::abs(heightData[i] - orig) > 0.001f)
                    {
                        ++affected;
                    }
                }
            ...
            float percent = (static_cast<float>(affected) * 100.0f) / static_cast<float>(width * height);
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Roads carved " + Ogre::StringConverter::toString(percent) + "% of terrain");
        }

        // Apply features (all your existing code stays the same!)
        if (this->enableIsland->getBool())
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Applying island mask...");
            this->applyIslandMask(heightData, width, height);
        }
        ...
        if (this->enableCanyons->getBool())
        {
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Carving " + Ogre::StringConverter::toString(canyons.size()) + " canyons...");
            this->applyCanyonsToTerrain(heightData, canyons, width, height);
        }

        if (this->enableErosion->getBool())
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Simulating erosion...");
            this->simulateHydraulicErosion(heightData, width, height);
        }

        if (this->enableRivers->getBool())
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Tracing " + Ogre::StringConverter::toString(this->riverCount->getUInt()) + " rivers...");

            std::vector<RiverPath> rivers = traceRiverPaths(heightData, width, height);

            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Traced " + Ogre::StringConverter::toString(rivers.size()) + " valid river paths");

            this->carveRiversStamp(heightData, rivers, width, height);
        }
        ...
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Generated heights: " + Ogre::StringConverter::toString(minH) + "m to " + Ogre::StringConverter::toString(maxH) + "m");

        // Convert float [0,1] to uint16 — same format setHeightData expects
        std::vector<Ogre::uint16> heightDataU16(width * height);
        for (Ogre::uint32 i = 0; i < width * height; ++i)
        {
            float value = Ogre::Math::Clamp(heightData[i], 0.0f, 1.0f);
            heightDataU16[i] = static_cast<Ogre::uint16>(value * 65535.0f);
        }

        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Uploading heightmap via setHeightData...");

        // setHeightData must run on render thread — it touches GPU staging textures
        // and calls updateHeightTextures() which updates the normal mapper workspace.
        // Move the data into the lambda to avoid dangling reference after this function returns.
        NOWA::GraphicsModule::RenderCommand renderCmd = [terra, heightDataU16 = std::move(heightDataU16)]()
        {
            terra->setHeightData(heightDataU16);
        };
        NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCmd), "ProceduralTerrainCreationComponent::generateProceduralTerrain");

        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] Complete!");
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralTerrain] ========================================");
    ...
    {
        if (hillFrequency < 0.0001f)
        {
            hillFrequency = 0.0001f;
        }
        if (hillFrequency > 20.0f)
        {
            hillFrequency = 20.0f;
        }
    ...
    {
        if (roadCurviness < 0.0f)
        {
            roadCurviness = 0.0f;
        }
        if (roadCurviness > 1.0f)
        {
            roadCurviness = 1.0f;
        }
    ...
    ProceduralTerrainCreationComponent* getProceduralTerrainCreationComponentFromIndex(GameObject* gameObject, unsigned int occurrenceIndex)
    {
    ...
    void ProceduralTerrainCreationComponent::createStaticApiForLua(lua_State* lua, class_<GameObject>& gameObjectClass, class_<GameObjectController>& gameObjectControllerClass)
    {
        ...
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralTerrainCreationComponent", "void setSedimentCapacity(float sedimentCapacity)",
            "How much sediment a droplet can carry before it starts depositing. Higher = more transport, less deposition.");
        ...
        LuaScriptApi::getInstance()->addClassToCollection("GameObjectController", "ProceduralTerrainCreationComponent castProceduralTerrainCreationComponent(ProceduralTerrainCreationComponent other)",
