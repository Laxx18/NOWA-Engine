...
namespace NOWA
{
    ...
    ProceduralWallComponent::ProceduralWallComponent() :
        ...
        isSnapToWall(false),
        snapToWallPoint(Ogre::Vector3::ZERO),
        snapToWallSegmentIdx(-1),
        snapToWallT(0.0f),
        ...
        physicsArtifactComponent(nullptr)
    {
        ...
        this->editMode = new Variant(ProceduralWallComponent::AttrEditMode(), std::vector<Ogre::String>{"Object", "Segment"}, this->attributes);
        ...
        this->editMode->setDescription("Object: click-drag to build walls.\n"
                                       "Segment: LMB to select a segment, X to delete, E to extend.");
        this->editMode->addUserData(GameObject::AttrActionNeedRefresh());
    ...
    bool ProceduralWallComponent::mousePressed(const OIS::MouseEvent& evt, OIS::MouseButtonID id)
    {
        ...
        {
            // Confirm extension drag if in progress
            if (this->isExtendingFromSegment && this->buildState == BuildState::DRAGGING)
            {
                ...
                if (wasSnapping)
                {
                    ...
                    this->currentSegment.groundHeightEnd = this->snapToWallPoint.y;
                    this->isSnapToWall = false;
                    this->snapToWallSegmentIdx = -1;
                }
                this->confirmWall();

                if (!wasSnapping)
                {
                    // Stay in dragging for further extension
                }
                else
                {
    ...
    bool ProceduralWallComponent::mouseMoved(const OIS::MouseEvent& evt)
    {
        ...
        if (this->raycastGround(screenX, screenY, hitPosition))
        {
            ...
            // Snap detection
            const Ogre::Real snapR = this->wallThickness->getReal() * 2.0f + 1.0f;
            this->detectSnapToWall(hitPosition, snapR);

            const Ogre::Vector3 previewPos = this->isSnapToWall ? this->snapToWallPoint : hitPosition;

            this->updateWallPreview(previewPos);
    ...
    bool ProceduralWallComponent::keyPressed(const OIS::KeyEvent& evt)
    {
        ...
        if (this->getEditModeEnum() == EditMode::SEGMENT)
        {
            ...
            {
                const WallSegment& sel = this->wallSegments[this->selectedSegmentIndex];

                this->currentSegment.startPoint = sel.endPoint;
                this->currentSegment.startPoint.y = 0.0f;
                this->currentSegment.endPoint = sel.endPoint;
                this->currentSegment.endPoint.y = 0.0f;
                this->currentSegment.groundHeightStart = sel.groundHeightEnd;
                this->currentSegment.groundHeightEnd = sel.groundHeightEnd;
                this->currentSegment.hasStartPillar = false;
                this->currentSegment.hasEndPillar = this->createPillars->getBool();

                this->buildState = BuildState::DRAGGING;
                this->lastValidPosition = sel.endPoint;
                this->isShiftPressed = true;
                ...
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
                    "[ProceduralWallComponent] Extend from segment " + Ogre::StringConverter::toString(this->selectedSegmentIndex) + " endpoint: " + Ogre::StringConverter::toString(sel.endPoint));
                return false;
            }
    ...
    void ProceduralWallComponent::startWallPlacement(const Ogre::Vector3& worldPosition)
    {
        ...
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
            "[ProceduralWallComponent] Started wall placement at: " + Ogre::StringConverter::toString(worldPosition) + ", ground height: " + Ogre::StringConverter::toString(this->currentSegment.groundHeightStart));
    }
    ...
    int ProceduralWallComponent::findNearestSegmentWithinRadius(const Ogre::Vector3& worldPos, Ogre::Real radius) const
    {
        if (this->wallSegments.empty())
        {
            return -1;
        ...
        for (int si = 0; si < static_cast<int>(this->wallSegments.size()); ++si)
        {
            const WallSegment& seg = this->wallSegments[si];
            const Ogre::Vector3 a(seg.startPoint.x, 0.0f, seg.startPoint.z);
            const Ogre::Vector3 b(seg.endPoint.x, 0.0f, seg.endPoint.z);
            const Ogre::Vector3 p(worldPos.x, 0.0f, worldPos.z);

            const Ogre::Vector3 ab = b - a;
            const float abLen2 = ab.dotProduct(ab);
            const float t = (abLen2 > 1e-6f) ? Ogre::Math::Clamp((p - a).dotProduct(ab) / abLen2, 0.0f, 1.0f) : 0.0f;
            const float dist = (a + ab * t - p).length();
    ...
    void ProceduralWallComponent::deleteSelectedSegment(void)
    {
        if (this->selectedSegmentIndex < 0 || this->selectedSegmentIndex >= static_cast<int>(this->wallSegments.size()))
        {
            return;
        }
        ...
        this->wallSegments.erase(this->wallSegments.begin() + this->selectedSegmentIndex);
        this->selectedSegmentIndex = -1;
        ...
        boost::shared_ptr<NOWA::EventDataCommandTransactionBegin> evtBegin(new NOWA::EventDataCommandTransactionBegin("Delete Wall Segment"));
        NOWA::AppStateManager::getSingletonPtr()->getEventManager()->queueEvent(evtBegin);

        boost::shared_ptr<EventDataWallModifyEnd> evtMod(new EventDataWallModifyEnd(oldData, newData, this->gameObjectPtr->getId()));
        ...
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "[ProceduralWallComponent] Deleted segment, remaining: " + Ogre::StringConverter::toString(this->wallSegments.size()));
    }

    void ProceduralWallComponent::createSegmentOverlay(void)
    {
        NOWA::GraphicsModule::RenderCommand cmd = [this]()
        {
            ...
            this->segOverlayObject->setName("WallSegOverlay_" + Ogre::StringConverter::toString(this->gameObjectPtr->getId()));
            ...
            this->snapOverlayNode = this->gameObjectPtr->getSceneManager()->getRootSceneNode()->createChildSceneNode();
            this->snapOverlayObject = this->gameObjectPtr->getSceneManager()->createManualObject();
            this->snapOverlayObject->setRenderQueueGroup(NOWA::RENDER_QUEUE_V2_MESH);
            this->snapOverlayObject->setName("WallSnapOverlay_" + Ogre::StringConverter::toString(this->gameObjectPtr->getId()));
            this->snapOverlayObject->setQueryFlags(0u);
            this->snapOverlayObject->setCastShadows(false);
            this->snapOverlayNode->attachObject(this->snapOverlayObject);
            this->snapOverlayNode->setVisible(false);
        };
        NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(cmd), "ProceduralWallComponent::createSegmentOverlay");
    }

    void ProceduralWallComponent::destroySegmentOverlay(void)
    {
        NOWA::GraphicsModule::RenderCommand cmd = [this]()
        {
            ...
            this->snapOverlayNode->detachAllObjects();
            if (this->snapOverlayObject)
            {
                this->gameObjectPtr->getSceneManager()->destroyManualObject(this->snapOverlayObject);
                this->snapOverlayObject = nullptr;
            }
            this->snapOverlayNode->getParentSceneNode()->removeAndDestroyChild(this->snapOverlayNode);
            this->snapOverlayNode = nullptr;
        };
        NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(cmd), "ProceduralWallComponent::destroySegmentOverlay");
    }
    ...
    void ProceduralWallComponent::scheduleSegmentOverlayUpdate(void)
    {
        ...
        if (!segmentMode || this->wallSegments.empty())
        {
        ...
        std::vector<LV> lines;
        lines.reserve(this->wallSegments.size() * 20);

        const Ogre::Real pushY = 0.1f;
        const Ogre::Real halfT = this->wallThickness->getReal() * 0.5f;
        const Ogre::Real crossR = std::max(0.2f, halfT * 2.0f);
        ...
        for (int si = 0; si < static_cast<int>(this->wallSegments.size()); ++si)
        {
            ...
            const Ogre::ColourValue& lc = selected ? cSelected : cGrey;

            const Ogre::Vector3 w0(seg.startPoint.x, seg.groundHeightStart + pushY, seg.startPoint.z);
            const Ogre::Vector3 w1(seg.endPoint.x, seg.groundHeightEnd + pushY, seg.endPoint.z);
            ...
            if (selected)
            {
                ...
                const Ogre::Vector3 perp = Ogre::Vector3::UNIT_Y.crossProduct(dir).normalisedCopy();

                addLine(w0 + perp * halfT, w1 + perp * halfT, cSelected);
                addLine(w0 + perp * -halfT, w1 + perp * -halfT, cSelected);
                addLine(w0 + perp * halfT, w0 + perp * -halfT, cSelected);
                addLine(w1 + perp * halfT, w1 + perp * -halfT, cSelected);

                // Endpoint crosses
                for (const Ogre::Vector3& ep : {w0, w1})
                {
        ...
        NOWA::GraphicsModule::RenderCommand drawCmd = [this, lines = std::move(lines)]()
        {
            ...
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[ProceduralWallComponent] Overlay FAILED: " + e.getDescription());
            }
        ...
        NOWA::GraphicsModule::getInstance()->enqueue(std::move(drawCmd), "ProceduralWallComponent::segOverlay_draw");
    }

    bool ProceduralWallComponent::detectSnapToWall(const Ogre::Vector3& worldPos, Ogre::Real radius)
    {
        if (this->wallSegments.empty())
        {
            this->isSnapToWall = false;
        ...
        // Current drag start — exclude this from snapping (avoid self-snap)
        const Ogre::Vector3 dragStartXZ(this->currentSegment.startPoint.x, 0.0f, this->currentSegment.startPoint.z);
        ...
        for (int si = 0; si < static_cast<int>(this->wallSegments.size()); ++si)
        {
            ...
            // Check front endpoint
            {
                const Ogre::Vector3 ep(seg.startPoint.x, 0.0f, seg.startPoint.z);
                // Skip if this is our own drag start
                if (ep.distance(dragStartXZ) < 0.05f)
                {
                    goto checkBack;
                }
                const float dist = ep.distance(Ogre::Vector3(worldPos.x, 0.0f, worldPos.z));
                if (dist < bestDist)
                {
                    bestDist = dist;
                    bestSeg = si;
                    bestT = 0.0f;
                    bestPt = Ogre::Vector3(seg.startPoint.x, seg.groundHeightStart, seg.startPoint.z);
                }
            }
        checkBack:
            // Check back endpoint
            {
                const Ogre::Vector3 ep(seg.endPoint.x, 0.0f, seg.endPoint.z);
                if (ep.distance(dragStartXZ) < 0.05f)
                {
                    continue;
                }
                ...
                if (dist < bestDist)
                {
                    ...
                    bestSeg = si;
                    bestT = 1.0f;
                    bestPt = Ogre::Vector3(seg.endPoint.x, seg.groundHeightEnd, seg.endPoint.z);
                }
            }
        }

        this->isSnapToWall = (bestSeg >= 0);
        this->snapToWallSegmentIdx = bestSeg;
        this->snapToWallT = bestT;
        this->snapToWallPoint = bestPt;
        return this->isSnapToWall;
    }

    void ProceduralWallComponent::scheduleSnapIndicatorUpdate(void)
    {
        if (!this->snapOverlayObject)
        {
            return;
        ...
        if (!this->isSnapToWall)
        {
            NOWA::GraphicsModule::RenderCommand cmd = [this]()
            {
                if (this->snapOverlayObject)
                {
                    this->snapOverlayObject->clear();
                }
                if (this->snapOverlayNode)
                {
                    this->snapOverlayNode->setVisible(false);
            ...
            NOWA::GraphicsModule::getInstance()->enqueue(std::move(cmd), "snapWall_hide");
            return;
        }

        const Ogre::Vector3 centre = this->snapToWallPoint;
        const Ogre::Real r = this->wallThickness->getReal() + 0.3f;
        const int segs = 16;
        const Ogre::Real pushY = 0.15f;
        ...
        NOWA::GraphicsModule::RenderCommand cmd = [this, lines = std::move(lines)]()
        {
            ...
            try
            {
                this->snapOverlayObject->begin("WhiteNoLightingBackground", Ogre::OT_LINE_LIST);
                Ogre::uint32 idx = 0;
                for (const auto& v : lines)
                {
                    this->snapOverlayObject->position(v.pos);
                    this->snapOverlayObject->colour(v.col);
                    this->snapOverlayObject->index(idx++);
                }
                this->snapOverlayObject->end();
            }
            catch (Ogre::Exception&)
            {
            }
    ...
    void ProceduralWallComponent::createWallMesh(void)
    {
        ...
        for (size_t segIdx = 0; segIdx < this->wallSegments.size(); ++segIdx)
        {
            ...
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_NORMAL,
                "[ProceduralWallComponent] Generating segment " + Ogre::StringConverter::toString(segIdx) + ": start(" + Ogre::StringConverter::toString(segment.startPoint) + ") -> end(" + Ogre::StringConverter::toString(segment.endPoint) + ")");
        ...
        const std::vector<Ogre::uint32>& pillarIndices, size_t numPillarVertices, const Ogre::Vector3& wallOrigin)
    {
        ...
        this->wallMesh = Ogre::MeshManager::getSingleton().createManual(meshName, groupName, &NOWA::gDummyMeshLoader);
        ...
        Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
            "[ProceduralWallComponent] Wall mesh created with " + Ogre::StringConverter::toString(numWallVertices) + " wall vertices and " + Ogre::StringConverter::toString(numPillarVertices) + " pillar vertices, attached to scene");
    }
    ...
    bool ProceduralWallComponent::loadWallDataFromFile(void)
    {
        ...
        try
        {
            ...
            if (!this->wallSegments.empty())
            {
                ...
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL,
                    "[ProceduralWallComponent] Loaded wall endpoint for continuation: " + Ogre::StringConverter::toString(this->loadedWallEndpoint) + ", height: " + Ogre::StringConverter::toString(this->loadedWallEndpointHeight));
            }
        ...
        try
        {
            ...
            if (fileSize != expectedSize)
            {
                Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                    "[ProceduralWallComponent] setWallData: size mismatch: expected " + Ogre::StringConverter::toString(expectedSize) + " got " + Ogre::StringConverter::toString(fileSize));
                return;
            }
    ...
    ProceduralWallComponent* getProceduralWallComponent(GameObject* go)
    {
        return NOWA::makeStrongPtr(go->getComponent<ProceduralWallComponent>()).get();
    }

    ProceduralWallComponent* getProceduralWallComponentFromName(GameObject* go, const Ogre::String& name)
    {
        return NOWA::makeStrongPtr(go->getComponentFromName<ProceduralWallComponent>(name)).get();
    }

    void ProceduralWallComponent::createStaticApiForLua(lua_State* lua, luabind::class_<GameObject>& gameObjectClass, luabind::class_<GameObjectController>& gameObjectControllerClass)
    {
        luabind::module(lua)[luabind::class_<ProceduralWallComponent, GameObjectComponent>("ProceduralWallComponent")

                // ── Activation ────────────────────────────────────────────────
                .def("setActivated", &ProceduralWallComponent::setActivated)
                .def("isActivated", &ProceduralWallComponent::isActivated)

                // ── Wall dimensions ───────────────────────────────────────────
                .def("setWallHeight", &ProceduralWallComponent::setWallHeight)
                .def("getWallHeight", &ProceduralWallComponent::getWallHeight)
                .def("setWallThickness", &ProceduralWallComponent::setWallThickness)
                .def("getWallThickness", &ProceduralWallComponent::getWallThickness)

                // ── Wall style ────────────────────────────────────────────────
                .def("setWallStyle", &ProceduralWallComponent::setWallStyle)
                .def("getWallStyle", &ProceduralWallComponent::getWallStyle)

                // ── Grid / snap ───────────────────────────────────────────────
                .def("setSnapToGrid", &ProceduralWallComponent::setSnapToGrid)
                .def("getSnapToGrid", &ProceduralWallComponent::getSnapToGrid)
                .def("setGridSize", &ProceduralWallComponent::setGridSize)
                .def("getGridSize", &ProceduralWallComponent::getGridSize)

                // ── Terrain adaptation ────────────────────────────────────────
                .def("setAdaptToGround", &ProceduralWallComponent::setAdaptToGround)
                .def("getAdaptToGround", &ProceduralWallComponent::getAdaptToGround)

                // ── Pillars ───────────────────────────────────────────────────
                .def("setCreatePillars", &ProceduralWallComponent::setCreatePillars)
                .def("getCreatePillars", &ProceduralWallComponent::getCreatePillars)
                .def("setPillarSize", &ProceduralWallComponent::getPillarSize)
                .def("getPillarSize", &ProceduralWallComponent::getPillarSize)

                // ── Datablocks ────────────────────────────────────────────────
                .def("setWallDatablock", &ProceduralWallComponent::setWallDatablock)
                .def("getWallDatablock", &ProceduralWallComponent::getWallDatablock)
                .def("setPillarDatablock", &ProceduralWallComponent::setPillarDatablock)
                .def("getPillarDatablock", &ProceduralWallComponent::getPillarDatablock)

                // ── UV tiling ─────────────────────────────────────────────────
                .def("setUVTiling", &ProceduralWallComponent::setUVTiling)
                .def("getUVTiling", &ProceduralWallComponent::getUVTiling)

                // ── Style-specific parameters ─────────────────────────────────
                .def("setFencePostSpacing", &ProceduralWallComponent::setFencePostSpacing)
                .def("setBattlementWidth", &ProceduralWallComponent::setBattlementWidth)
                .def("setBattlementHeight", &ProceduralWallComponent::setBattlementHeight)

                // ── Segment management ────────────────────────────────────────
                .def("getSegmentCount", &ProceduralWallComponent::getSegmentCountLua)
                .def("addWallSegment", &ProceduralWallComponent::addWallSegment)
                .def("removeLastSegment", &ProceduralWallComponent::removeLastSegment)];

        // ── LuaScriptApi documentation ─────────────────────────────────────────
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "class inherits GameObjectComponent", ProceduralWallComponent::getStaticInfoText());
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setActivated(bool activated)", "Activates or deactivates the wall component.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setWallHeight(float height)", "Sets the wall height in world units.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setWallThickness(float thickness)", "Sets the wall thickness in world units.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setWallStyle(string style)", "Sets wall style. Values: 'Solid', 'Fence', 'Battlement', 'Arch'.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setAdaptToGround(bool adapt)", "If true, wall base follows terrain height at each segment endpoint.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setCreatePillars(bool create)", "If true, corner pillars are generated at segment endpoints.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setWallDatablock(string name)", "Sets the PBS datablock name for the wall surface material.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setPillarDatablock(string name)", "Sets the PBS datablock name for pillar material.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setUVTiling(Vector2 tiling)", "Sets UV tiling multiplier. x = horizontal repeat, y = vertical repeat.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setFencePostSpacing(float spacing)", "Sets spacing between fence posts. Only used in Fence style.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setBattlementWidth(float width)", "Sets the width of each battlement notch. Only used in Battlement style.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void setBattlementHeight(float height)", "Sets the height of battlements above the wall top. Only used in Battlement style.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "int getSegmentCount()", "Returns the number of wall segments currently placed.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void addWallSegment(Vector3 start, Vector3 end)", "Adds a single wall segment between two world positions and rebuilds the mesh.");
        LuaScriptApi::getInstance()->addClassToCollection("ProceduralWallComponent", "void removeLastSegment()", "Removes the most recently added wall segment.");

        // ── Register on GameObject and GameObjectController ────────────────────
        gameObjectClass.def("getProceduralWallComponent", (ProceduralWallComponent * (*)(GameObject*)) & getProceduralWallComponent);
        gameObjectClass.def("getProceduralWallComponentFromName", &getProceduralWallComponentFromName);
        gameObjectControllerClass.def("castProceduralWallComponent", &GameObjectController::cast<ProceduralWallComponent>);

        LuaScriptApi::getInstance()->addClassToCollection("GameObject", "ProceduralWallComponent getProceduralWallComponent()", "Gets the ProceduralWallComponent from this GameObject.");
        LuaScriptApi::getInstance()->addClassToCollection("GameObject", "ProceduralWallComponent getProceduralWallComponentFromName(string name)", "Gets a named ProceduralWallComponent from this GameObject.");
        LuaScriptApi::getInstance()->addClassToCollection("GameObjectController", "ProceduralWallComponent castProceduralWallComponent(ProceduralWallComponent other)", "Casts for Lua auto-completion support.");
    }
