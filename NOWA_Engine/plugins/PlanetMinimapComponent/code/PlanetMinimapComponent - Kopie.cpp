...
namespace NOWA
{
    ...
    PlanetMinimapComponent::PlanetMinimapComponent() :
		...
		compassTargetWidget(nullptr),
		compassDistanceText(nullptr),
		...
		compassTargetId(new Variant(PlanetMinimapComponent::AttrCompassTargetId(), static_cast<unsigned long>(0), this->attributes, true)),
		compassTargetImage(new Variant(PlanetMinimapComponent::AttrCompassTargetImage(), "", this->attributes)),
        ...
        rotateMinimap(new Variant(PlanetMinimapComponent::AttrRotateMinimap(), false, this->attributes))
    {
		...
		this->compassTargetId->setDescription("Sets the game object id of a second tracked object (e.g. the player's ship) to find. In WholeSceneVisible mode shown as a second marker at its true position on the baked overview "
			"(always representable, the bake covers the whole sphere). In follow mode shown as a marker wandering around the rim of the local minimap, pointing in the direction to walk. 0 disables it.");
		this->compassTargetImage->addUserData(GameObject::AttrActionFileOpenDialog(), "Minimap");
		this->compassTargetImage->setDescription("Sets the icon image for CompassTargetId (added to the Minimap.xml in the MyGui/Minimap folder like MinimapMask). Left empty, no compass marker is shown even if CompassTargetId is set.");
    ...
    bool PlanetMinimapComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
		...
		if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == "CompassTargetId")
		{
			this->compassTargetId->setValue(XMLConverter::getAttribUnsignedLong(propertyElement, "data"));
			propertyElement = propertyElement->next_sibling("property");
		}

		if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == "CompassTargetImage")
		{
			this->compassTargetImage->setValue(XMLConverter::getAttrib(propertyElement, "data"));
			propertyElement = propertyElement->next_sibling("property");
		}
        ...
        if (false == notSimulating && nullptr != this->planetGameObject)
        {
            ...
            auto closureFunction = [this, targetId](Ogre::Real renderDt)
            {
                ...
                if (false == this->wholeSceneVisible->getBool())
                {
                    ...
                    if (0 != this->compassTargetId->getULong())
                    {
                        // updateCompassTarget() touches a MyGUI widget directly though, so unlike updateFollowCamera()
                        // above it DOES need to run on the render thread. Runs every tick (not throttled) so the rim
                        // marker keeps up smoothly as the player turns/moves, same reasoning as the camera update itself.
                        // Passing the camera transform updateFollowCamera just computed THIS tick, rather than letting
                        // updateCompassTarget read it back from the Camera object itself (see updateFollowCamera's
                        // comment for why that read-back was unreliable).
                        this->updateCompassTarget(targetPos, followCameraPosition, followCameraOrientation, followCameraDistance);
                    }
                }
                else
                {
                    ...
                    if (this->timeSinceLastUpdate >= 0.25f)
                    {
                        ...
                        // Overview mode's branch inside updateCompassTarget never touches the camera transform
                        ...
                        this->updateCompassTarget(targetPos, Ogre::Vector3::ZERO, Ogre::Quaternion::IDENTITY, this->cameraHeight->getReal());
    ...
    void PlanetMinimapComponent::setupPlanetMinimap(void)
    {
        ...
        NOWA::GraphicsModule::RenderCommand setupRdCmd = [this]
        {
			...
			if (0 != this->compassTargetId->getULong() && false == this->compassTargetImage->getString().empty())
			{
				// Compass target marker (e.g. the player's ship) -- valid in BOTH modes: a true-position blip on
				// the baked overview when WholeSceneVisible is true (the bake covers the whole sphere, so the ship
				// is always representable, no matter how far away), or a fixed-radius rim marker in follow mode.
				// Created as a child of minimapWidget so it always shares the exact same pixel rect as the map
				// texture/markers above, regardless of which mode is active.
				this->compassTargetWidget = this->minimapWidget->createWidget<MyGUI::ImageBox>("ImageBox", MyGUI::IntCoord(0, 0, 32, 32), MyGUI::Align::Default);
				this->compassTargetWidget->setImageTexture(this->compassTargetImage->getString());
				this->compassTargetWidget->setNeedMouseFocus(false);
				this->compassTargetWidget->setDepth(3);
				this->compassTargetWidget->setVisible(false);

				// Live distance-to-target readout (e.g. "342 m" / "1.2 km"), repositioned alongside the marker
				// every tick in updateCompassTarget. Created as a sibling (not a child of compassTargetWidget) so
				// its own position can be offset independently (placed just below the marker icon).
				this->compassDistanceText = this->minimapWidget->createWidget<MyGUI::TextBox>("TextBox", MyGUI::IntCoord(0, 0, 90, 16), MyGUI::Align::Default);
				this->compassDistanceText->setTextAlign(MyGUI::Align::Center);
				this->compassDistanceText->setFontHeight(12);
				this->compassDistanceText->setNeedMouseFocus(false);
				this->compassDistanceText->setDepth(4);
				this->compassDistanceText->setVisible(false);
			}
        };
    ...
    void PlanetMinimapComponent::updateFogOfWarTexture(const Ogre::Vector3& position, const Ogre::Quaternion& targetOrientation, const Ogre::Vector3& targetDefaultDirection, Ogre::Real radius)
    {
		...
		const int radiusInPixels = static_cast<int>((angularRadiusRad / Ogre::Math::PI) * static_cast<float>(texSize));
	...
	void PlanetMinimapComponent::removeWorkspace(void)
    {
        ...
        NOWA::GraphicsModule::RenderCommand cmd = [this]()
        {
            ...
            if (nullptr != this->compassTargetWidget)
            {
                MyGUI::Gui::getInstancePtr()->destroyWidget(this->compassTargetWidget);
                this->compassTargetWidget = nullptr;
            }

            if (nullptr != this->compassDistanceText)
            {
                MyGUI::Gui::getInstancePtr()->destroyWidget(this->compassDistanceText);
                this->compassDistanceText = nullptr;
            }
	...
	void PlanetMinimapComponent::updateCompassTarget(const Ogre::Vector3& targetWorldPosition, const Ogre::Vector3& followCameraPosition, const Ogre::Quaternion& followCameraOrientation, Ogre::Real followCameraDistance)
	{
		// Threadsafe from the outside -- call only from inside a GraphicsModule render command/tracked closure,
		// since this touches a MyGUI widget.
		if (nullptr == this->compassTargetWidget || nullptr == this->planetGameObject)
		{
			return;
		}

		unsigned long compassId = this->compassTargetId->getULong();
		if (0 == compassId)
		{
			this->compassTargetWidget->setVisible(false);
			if (nullptr != this->compassDistanceText)
		...
		{
			// Ship not currently spawned/already destroyed -- hide rather than show a stale position.
			this->compassTargetWidget->setVisible(false);
			if (nullptr != this->compassDistanceText)
			{
				this->compassDistanceText->setVisible(false);
		...
		// True distance from the tracked player/target to the compass target, independent of the follow camera's
		// CameraHeight offset -- this is what the player actually cares about ("how far is my ship"), not the
		// distance from wherever the minimap camera happens to be floating.
		...
		if (true == this->wholeSceneVisible->getBool())
		{
			// Overview mode: the bake covers the ENTIRE sphere, so the ship is always representable as a true
			...
			const int markerHalfWidth = static_cast<int>(this->compassTargetWidget->getWidth() / 2);
			const int markerHalfHeight = static_cast<int>(this->compassTargetWidget->getHeight() / 2);
			this->compassTargetWidget->setVisible(true);
			this->compassTargetWidget->setPosition(static_cast<int>(pixel.x) - markerHalfWidth, static_cast<int>(pixel.y) - markerHalfHeight);

			if (nullptr != this->compassDistanceText)
			{
				this->compassDistanceText->setCaption(distanceText);
				this->compassDistanceText->setVisible(true);
				const int textHalfWidth = static_cast<int>(this->compassDistanceText->getWidth() / 2);
				this->compassDistanceText->setPosition(static_cast<int>(pixel.x) - textHalfWidth, static_cast<int>(pixel.y) + markerHalfHeight + 2);
			}
		}
		else
		{
			// Follow mode: TRUE position when the ship is within the local view's footprint, clamped onto the rim
			// ring (at the correct bearing) once it would fall outside it -- the standard "off-screen indicator"
			...
			// transforms the RAW (non-normalized) world-space offset to the ship through the minimap camera's
			// CURRENT actual orientation (camOrientation.Inverse() * toShipWorld). That guarantees this always
			// matches whatever the camera is actually showing right now, with no separate logic to keep in sync if
			// either of those change later.
			if (nullptr == this->minimapWidget)
			{
				return;
			}

			// IMPORTANT: do NOT normalize this -- the magnitude (world-space distance to the ship) is exactly what
			// lets the same calculation place the marker at its TRUE position when nearby and clamp it onto the rim
			// ring only once it's actually out of view, instead of always snapping to the ring regardless of
			// distance. Uses the camera position freshly computed by updateFollowCamera THIS SAME TICK (passed in as
			// followCameraPosition), not read back from the Camera object -- see updateFollowCamera's own comment for
			// why that read-back was unreliable.
			Ogre::Vector3 toShipWorld = compassWorldPosition - followCameraPosition;
			if (toShipWorld.squaredLength() < 0.000001f)
			{
				this->compassTargetWidget->setVisible(false);
				if (nullptr != this->compassDistanceText)
				{
			...
			Ogre::Vector3 toShipLocal = followCameraOrientation.Inverse() * toShipWorld;

			// World-unit offset on the camera's local screen plane. IMPORTANT: this is local X (right) and local Y
			// (up) -- NOT local Z. Z is the camera's forward/depth axis (the direction it's actually looking,
			// i.e. "how far below the camera" for this top-down view), not a screen-plane axis; an earlier version
			// of this code mistakenly used X and -Z, discarding the genuinely-relevant Y component (which is
			// frequently comparable in magnitude to X/Z, not negligible) and using depth as if it were a screen
			// coordinate, which produced movement that didn't track the camera's actual screen orientation.
			Ogre::Real worldOffsetX = toShipLocal.x;
			Ogre::Real worldOffsetY = toShipLocal.y;
			...
			// minimap actually show edge-to-edge". Reused unscaled as an approximation for the ship's distance too,
			// same constant-world-to-pixel-scale convention an ordinary 2D minimap uses.
			// fovY/aspectRatio still come live from the Camera (intrinsic projection parameters, not affected by any
			// position/orientation staleness concern), but cameraDistance uses the value updateFollowCamera computed THIS
			// tick (followCameraDistance) rather than re-reading this->cameraHeight, in case CameraHeight changes
			// mid-tick from another source -- keeps the footprint calculation consistent with the camera position
			// actually used a few lines above.
			...
			// minimap needs a wide ellipse, not a circle capped at the SMALLER of the two half-dimensions (a circle
			// inscribed in a 512x206 box never reaches the left/right edges at all, which read as "marker doesn't
			// go to the edge / moves strangely" since most of the horizontal travel range was unreachable).
			...
			const Ogre::Real ringMarginX = static_cast<Ogre::Real>(this->compassTargetWidget->getWidth()) * 0.5f + 2.0f;
			const Ogre::Real ringMarginY = static_cast<Ogre::Real>(this->compassTargetWidget->getHeight()) * 0.5f + 2.0f;
			...
			// calculation covers both cases: ship in view -> true position; ship out of view -> rim, at the correct
			// angle. Normalising each axis by its own ellipse radius before measuring magnitude is what makes the
			// clamp elliptical instead of circular: scaling pixelOffsetX/Y by 1/normalizedMagnitude lands the point
			// exactly on the ellipse boundary (normalizedMagnitude == 1 afterwards), preserving the true bearing.
			...
			this->compassTargetWidget->setPosition(markerCenterX - markerHalfWidth, markerCenterY - markerHalfHeight);
			...
			if (nullptr != this->compassDistanceText)
			{
				...
				this->compassDistanceText->setVisible(true);
				const int textHalfWidth = static_cast<int>(this->compassDistanceText->getWidth() / 2);
				this->compassDistanceText->setPosition(markerCenterX - textHalfWidth, markerCenterY + markerHalfHeight + 2);
			}

			// Throttled diagnostic (roughly once a second at 60 closure calls/sec) -- prints every value this
			// calculation depends on, so a wrong result can be traced to the exact step that produced it instead of
			// guessed at again.
			this->compassLogCounter++;
			if (0u == (this->compassLogCounter % 60u))
			{
				Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "[PlanetMinimapComponent] '" + this->gameObjectPtr->getName() + "' compass(follow): compassWorldPosition="
					...
					+ " toShipWorld=" + Ogre::StringConverter::toString(toShipWorld) + " toShipLocal=" + Ogre::StringConverter::toString(toShipLocal)
					...
					+ " finalWidgetPos=" + Ogre::StringConverter::toString(static_cast<int>(halfWidth + pixelOffsetX) - markerHalfWidth) + ","
					+ Ogre::StringConverter::toString(static_cast<int>(halfHeight - pixelOffsetY) - markerHalfHeight));
			}
		}
	}
        ...
        Ogre::Vector3& outCameraPosition, Ogre::Quaternion& outCameraOrientation, Ogre::Real& outCameraDistance)
    {
		...
		if (false == this->followCameraLogged)
		{
    ...
    void PlanetMinimapComponent::actualizeValue(Variant* attribute)
    {
		...
		else if (PlanetMinimapComponent::AttrCompassTargetId() == attribute->getName())
		{
			this->setCompassTargetId(attribute->getULong());
		}
		else if (PlanetMinimapComponent::AttrCompassTargetImage() == attribute->getName())
		{
			this->setCompassTargetImage(attribute->getString());
		}
    ...
    void PlanetMinimapComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
    {
		...
		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "12"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "CompassTargetId"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->compassTargetId->getULong())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "7"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "CompassTargetImage"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->compassTargetImage->getString())));
		propertiesXML->append_node(propertyXML);
	...
	void PlanetMinimapComponent::setCompassTargetId(unsigned long compassTargetId)
	{
		this->compassTargetId->setValue(compassTargetId);

		if (nullptr != this->compassTargetWidget && 0 == compassTargetId)
		{
			// Cheap immediate feedback so the marker disappears right away rather than waiting up to one tick for
			// updateCompassTarget()'s own 0-check to catch up.
			NOWA::GraphicsModule::RenderCommand cmd = [this]
			{
				if (nullptr != this->compassTargetWidget)
				{
			...
			NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(cmd), "PlanetMinimapComponent::setCompassTargetId hide");
		}
	}

	unsigned long PlanetMinimapComponent::getCompassTargetId(void) const
	{
		return this->compassTargetId->getULong();
	}

	void PlanetMinimapComponent::setCompassTargetImage(const Ogre::String& compassTargetImage)
	{
		this->compassTargetImage->setValue(compassTargetImage);

		if (nullptr != this->compassTargetWidget)
		{
			NOWA::GraphicsModule::RenderCommand cmd = [this, compassTargetImage]
			{
				if (nullptr != this->compassTargetWidget)
				{
					this->compassTargetWidget->setImageTexture(compassTargetImage);
					this->compassTargetWidget->setImageRect(MyGUI::IntRect(0, 0, this->compassTargetWidget->getImageSize().width, this->compassTargetWidget->getImageSize().height));
				}
			};
			NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(cmd), "PlanetMinimapComponent::setCompassTargetImage");
		}
	}

	Ogre::String PlanetMinimapComponent::getCompassTargetImage(void) const
	{
		return this->compassTargetImage->getString();
	}
    ...
    void setLuaCompassTargetId(PlanetMinimapComponent* instance, const Ogre::String& targetId)
    {
        instance->setCompassTargetId(Ogre::StringConverter::parseUnsignedLong(targetId));
    }

    Ogre::String getLuaCompassTargetId(PlanetMinimapComponent* instance)
    {
        return Ogre::StringConverter::toString(instance->getCompassTargetId());
    }
    ...
    void PlanetMinimapComponent::createStaticApiForLua(lua_State* lua, luabind::class_<GameObject>& gameObjectClass, luabind::class_<GameObjectController>& gameObjectControllerClass)
    {
        ...
        [
            ...
            .def("setCompassTargetId", &setLuaCompassTargetId)
            .def("getCompassTargetId", &getLuaCompassTargetId)
			.def("setCompassTargetImage", &PlanetMinimapComponent::setCompassTargetImage)
			...
			.def("getCompassTargetImage", &PlanetMinimapComponent::getCompassTargetImage)
		...
		LuaScriptApi::getInstance()->addClassToCollection("PlanetMinimapComponent", "void setCompassTargetId(number id)", "Sets the game object id of a second tracked object (e.g. the player's ship) to find. Shown as a true-position marker on the baked overview in WholeSceneVisible mode, or as a marker wandering the rim of the local minimap in follow mode. 0 disables it.");
		LuaScriptApi::getInstance()->addClassToCollection("PlanetMinimapComponent", "number getCompassTargetId()", "Gets the game object id of the compass target.");
		LuaScriptApi::getInstance()->addClassToCollection("PlanetMinimapComponent", "void setCompassTargetImage(String image)", "Sets the icon image for CompassTargetId (same folder as MinimapMask). Left empty, no compass marker is shown even if CompassTargetId is set.");
		...
		LuaScriptApi::getInstance()->addClassToCollection("PlanetMinimapComponent", "String getCompassTargetImage()", "Gets the icon image for CompassTargetId.");
