...
namespace NOWA
{
    ...
    public:
        ...
        /**
         * @brief Sets the game object id of a second tracked object (e.g. the player's ship) shown alongside the
         *		  main tracked target: in WholeSceneVisible mode as a second marker at its true position on the
         *		  baked overview (the bake covers the entire sphere, so this is always representable); in follow
         *		  mode as a marker wandering around the rim of the local minimap, pointing in the direction to walk,
         *		  derived directly from the minimap camera's current actual orientation. 0 disables it.
         */
        void setCompassTargetId(unsigned long compassTargetId);

        unsigned long getCompassTargetId(void) const;

        void setCompassTargetImage(const Ogre::String& compassTargetImage);
        ...
        Ogre::String getCompassTargetImage(void) const;
    ...
    public:
		...
		static const Ogre::String AttrCompassTargetId(void) { return "Compass Target Id"; }
		static const Ogre::String AttrCompassTargetImage(void) { return "Compass Target Image"; }
    ...
    private:
        ...
        /**
         * @brief Repositions (and hides if unresolved) the compass target widget (e.g. the player's ship). In
         *		  WholeSceneVisible mode this is a true position on the baked overview map, same mechanism as
         *		  updateOverviewTargetMarker -- followCameraPosition/Orientation/Distance are unused in that mode,
         *		  pass any value (e.g. ZERO/IDENTITY/0). In follow mode this is the target's TRUE screen position
         *		  when it is within the local view's footprint, clamped onto the rim ring (at the correct bearing)
         *		  once it would fall outside it. followCameraPosition/Orientation/Distance MUST be the exact values
         *		  updateFollowCamera computed THIS SAME TICK (its own out-parameters) -- NOT read back from the
         *		  Camera object, which may be stale or in a different coordinate space depending on
         *		  CameraComponent's internal threading/parenting.
         *		  targetWorldPosition is the tracked player/target's true position, used (independent of the camera
         *		  offset) to compute and display the real distance in meters to the compass target.
         */
        void updateCompassTarget(const Ogre::Vector3& targetWorldPosition, const Ogre::Vector3& followCameraPosition, const Ogre::Quaternion& followCameraOrientation, Ogre::Real followCameraDistance);
         ...
         *		  outCameraOrientation/outCameraDistance, so callers (updateCompassTarget) can use the authoritative
    ...
    private:
        ...
        MyGUI::ImageBox* compassTargetWidget;
        MyGUI::TextBox* compassDistanceText;
        ...
        Variant* compassTargetId;
        Variant* compassTargetImage;
