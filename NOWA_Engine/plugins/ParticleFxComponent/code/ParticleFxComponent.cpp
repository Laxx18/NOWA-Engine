...
namespace NOWA
{
        ...
        fadeOutTime(new Variant(ParticleFxComponent::AttrFadeOutTime(), 1000.0f, this->attributes))
    {
        ...
        this->blendingMethod
            ->setDescription("Alpha Hashing: Fast, no sorting needed. Alpha Hashing + A2C: Best quality with MSAA. Alpha Blending: Traditional transparency. From Material: A lookup in the corresponding is made for the configured blending method.");
    ...
    void ParticleFxComponent::setActivated(bool activated)
    {
        ...
        if (activated)
        {
            // Push the current world transform into the module BEFORE playing, so that
            // createParticleEffect() can place the freshly created node correctly.
            this->updateParticleTransform();
    ...
    void ParticleFxComponent::setParticleScale(const Ogre::Vector2& scale)
    {
        ...
        this->particleScale->setValue(tempScale);
    ...
    void ParticleFxComponent::updateParticleTransform(void)
    {
        ...
        // Attention: do NOT bail out when particleNode is still null. The node is created
        // lazily by ParticleFxModule::createParticleEffect() on the first play. The module
        // stores the world transform and applies it as soon as the node exists. Returning
        // early here was the reason the effect stayed at the world origin.
        ParticleFxData* particleData = this->particleFxModule->getParticle(this->uniqueParticleName);
    ...
    void ParticleFxComponent::recreateParticle(void)
    {
        ...
        this->particleFxModule->createParticleSystem(this->uniqueParticleName, templateName, this->particleInitialPlayTime->getReal(), orientation, this->particleOffsetPosition->getVector3(), this->particleScale->getVector2(),
            this->repeat->getBool(), this->particlePlaySpeed->getReal(), this->getBlendingMethod(), this->fadeIn->getBool(), this->fadeInTime->getReal(), this->fadeOut->getBool(), this->fadeOutTime->getReal());
    ...
    void ParticleFxComponent::createStaticApiForLua(lua_State* lua, luabind::class_<GameObject>& gameObjectClass, luabind::class_<GameObjectController>& gameObjectControllerClass)
    {
        module(lua)[class_<ParticleFxComponent, GameObjectComponent>("ParticleFxComponent")
                ...
                .enum_("ParticleBlendingMethod")[luabind::value("AlphaHashing", ParticleBlendingMethod::AlphaHashing), luabind::value("AlphaHashingA2C", ParticleBlendingMethod::AlphaHashingA2C),
                    luabind::value("AlphaBlending", ParticleBlendingMethod::AlphaBlending), luabind::value("AlphaTransparent", ParticleBlendingMethod::AlphaTransparent), luabind::value("FromMaterial", ParticleBlendingMethod::FromMaterial)]];
        ...
        LuaScriptApi::getInstance()->addClassToCollection("ParticleFxComponent", "void setBlendingMethod(ParticleBlendingMethod method)",
            "Sets the blending method (AlphaHashing=0, AlphaHashingA2C=1, AlphaBlending=2, AlphaTransparent=3, FromMaterial=4).");
