...
namespace NOWA
{
    ...
    TransformEaseComponent::TransformEaseComponent() :
        ...
        rotationFinishedReported(false),
        translationFinishedReported(false),
        scaleFinishedReported(false),
        ...
        scaleEaseFunction(new Variant(TransformEaseComponent::AttrScaleEaseFunction(), Interpolator::getInstance()->getAllEaseFunctionNames(), this->attributes))
    {
        ...
        // D.h. Duration abh�ngig von Duration und easeFunction
    ...
    bool TransformEaseComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
    ...
    bool TransformEaseComponent::connect(void)
    {
        ...
        this->rotationFinishedReported = false;
        ...
        this->translationFinishedReported = false;
        ...
        this->scaleFinishedReported = false;
        ...
        if (false == notSimulating && true == this->activated->getBool())
        {
            ...
            if (true == this->rotationActivated->getBool())
            {
                if (Ogre::Math::RealEqual(this->rotationOppositeDir, 1.0f))
                {
                    ...
                    if (this->rotationProgress >= this->rotationDuration->getReal())
                    {
                        ...
                        // Capped: with repeat on this incremented forever, and 'short' overflows
                        // after 32767 rounds - signed overflow is undefined behaviour.
                        if (this->rotationRound < 2)
                        {
                            this->rotationRound++;
                        }
                    }
                ...
                {
                    // Latched: the condition above stays true for every following frame once
                    // repeat is off, so the callback used to fire continuously instead of once.
                    if (false == this->rotationFinishedReported)
                    {
                        this->rotationFinishedReported = true;
                ...
                {
                    // A duration of zero produced inf or NaN here and propagated straight into
                    // the transform.
                    const Ogre::Real rotationDurationValue = this->rotationDuration->getReal();
                    Ogre::Real t = (rotationDurationValue > 0.0001f) ? (this->rotationProgress / rotationDurationValue) : 1.0f;
            ...
            if (true == this->translationActivated->getBool())
            {
                ...
                if (this->translationRound == 2 && this->translationProgress >= 0.0f)
                {
                    // Latched: the condition above stays true for every following frame once
                    // repeat is off, so the callback used to fire continuously instead of once.
                    if (false == this->translationFinishedReported)
                    {
                        this->translationFinishedReported = true;
                        someFunctionFinished = true;
                    }
                ...
                if (true == this->translationRepeat->getBool() || true == this->internalTranslationDirectionChange || 0 == this->translationRound)
                {
                    // A duration of zero produced inf or NaN here and propagated straight into
                    // the transform.
                    const Ogre::Real translationDurationValue = this->translationDuration->getReal();
                    Ogre::Real t = (translationDurationValue > 0.0001f) ? (this->translationProgress / translationDurationValue) : 1.0f;
                    Ogre::Vector3 resultVec =
                        Interpolator::getInstance()->applyEaseFunction(this->translationAxis->getVector3() * this->translationMin->getReal(), this->translationAxis->getVector3() * this->translationMax->getReal(), this->translationEaseFunctions, t);
            ...
            if (true == this->scaleActivated->getBool())
            {
                ...
                if (this->scaleRound == 2 && this->scaleProgress >= 0.0f)
                {
                    // Latched: the condition above stays true for every following frame once
                    // repeat is off, so the callback used to fire continuously instead of once.
                    if (false == this->scaleFinishedReported)
                    {
                        this->scaleFinishedReported = true;
                        someFunctionFinished = true;
                    }
                    // if repeat is of, only change the direction one time, to get back to its origin and leave
                ...
                if (true == this->scaleRepeat->getBool() || true == this->internalScaleDirectionChange || 0 == this->scaleRound)
                {
                    // A duration of zero produced inf or NaN here and propagated straight into
                    // the transform.
                    const Ogre::Real scaleDurationValue = this->scaleDuration->getReal();
                    Ogre::Real t = (scaleDurationValue > 0.0001f) ? (this->scaleProgress / scaleDurationValue) : 1.0f;
            ...
            {
                // The getLuaScript() gate is gone: the closure belongs to whichever script
                // registered it, not to this game object - which may well have no script of
                // its own, in which case the callback never fired at all.
                if (this->functionFinished.is_valid())
                {
                    // The closure object is deliberately NOT copied: disconnect() clears it
                    // and the is_valid() check inside the command notices that. The weak
                    // pointer covers the component being destroyed rather than merely
                    // disconnected.
                    ...
                    NOWA::AppStateManager::LogicCommand logicCommand = [this, weakThis]()
                    {
                        ...
                        if (false == this->functionFinished.is_valid())
                        {
                            return;
                        }
                        ...
                        catch (luabind::error& error)
                        {
                            ...
                            Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[TransformEaseComponent] Caught error in 'reactOnFunctionFinished' Error: " + Ogre::String(error.what()) + " details: " + msg.str());
                        }
                    };
    ...
    void TransformEaseComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
    {
        // 2 = int
        // 6 = real
        // 7 = string
    ...
    TransformEaseComponent* getTransformEaseComponentFromIndex(GameObject* gameObject, unsigned int occurrenceIndex)
    {
    ...
    {
        module(lua)[class_<TransformEaseComponent, GameObjectComponent>("TransformEaseComponent")
        ...
        LuaScriptApi::getInstance()->addClassToCollection("TransformEaseComponent", "void reactOnFunctionFinished(func closure)",
            "Sets whether to react at the moment when one of the functions (translate, rotate, scale) or all etc. have finished their round.");
        ...
        gameObjectClass.def("getTransformEaseComponentFromIndex", (TransformEaseComponent * (*)(GameObject*, unsigned int)) & getTransformEaseComponentFromIndex);

        LuaScriptApi::getInstance()->addClassToCollection("GameObject", "TransformEaseComponent getTransformEaseComponentFromIndex(unsigned int occurrenceIndex)",
