...
namespace NOWA
{
    ...
    InventoryItemComponent::InventoryItemComponent() :
        ...
        gameObjectId(new Variant(InventoryItemComponent::AttrGameObjectId(), static_cast<unsigned long>(0), this->attributes)),
        alreadyAdded(false),
        alreadyRemoved(false)
    {
        ...
        gameObjectId->setDescription("Optional: The id of the GameObject (e.g. a building template) associated with this inventory item. "
                                     "Set to 0 for none. When this item is placed via MyGUIItemBoxComponent, the id is passed to the reactOnMouseButtonClick callback "
                                     "so the template can be resolved generically without hardcoded names.");
    }
    ...
    bool InventoryItemComponent::init(rapidxml::xml_node<>*& propertyElement)
    {
        ...
        if (propertyElement && XMLConverter::getAttrib(propertyElement, "name") == "GameObjectId")
        {
            this->gameObjectId->setValue(XMLConverter::getAttribUnsignedLong(propertyElement, "data"));
            propertyElement = propertyElement->next_sibling("property");
        }
        return true;
    }
    ...
    GameObjectCompPtr InventoryItemComponent::clone(GameObjectPtr clonedGameObjectPtr)
    {
        ...
        clonedCompPtr->setGameObjectId(this->gameObjectId->getULong());
    ...
    void InventoryItemComponent::actualizeValue(Variant* attribute)
    {
        ...
        else if (InventoryItemComponent::AttrGameObjectId() == attribute->getName())
        {
            this->setGameObjectId(attribute->getULong());
        }
    }
    ...
    void InventoryItemComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc)
    {
        ...
        propertyXML = doc.allocate_node(node_element, "property");
        propertyXML->append_attribute(doc.allocate_attribute("type", "2"));
        propertyXML->append_attribute(doc.allocate_attribute("name", "GameObjectId"));
        propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(doc, this->gameObjectId->getULong())));
        propertiesXML->append_node(propertyXML);
    }
    ...
    {
        this->gameObjectId->setValue(gameObjectId);
    }

    unsigned long InventoryItemComponent::getGameObjectId(void) const
    {
        return this->gameObjectId->getULong();
    }
    ...
    void InventoryItemComponent::removeQuantityFromInventory(unsigned long gameObjectId, const Ogre::String& componentName, int quantity, bool once)
    {
        ...
        else
        {
            Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL,
                "[InventoryItemComponent] Error cannot remove quantity from inventory. Because the the main game object has no inventory, for game object: " + this->gameObjectPtr->getName());
        }
    }
    ...
    void setGameObjectId(InventoryItemComponent* instance, const Ogre::String& id)
    {
    ...
    void InventoryItemComponent::createStaticApiForLua(lua_State* lua, class_<GameObject>& gameObjectClass, class_<GameObjectController>& gameObjectControllerClass)
    {
        ...
        [
            ...
            .def("setGameObjectId", &setGameObjectId)
            .def("getGameObjectId", &InventoryItemComponent::getGameObjectId)
        ];
        ...
        LuaScriptApi::getInstance()->addClassToCollection("InventoryItemComponent", "void addQuantityToInventory(String inventoryIdGameObject, int quantity, bool once)",
            "Increases the quantity of this inventory item in the inventory game object. "
        ...
        LuaScriptApi::getInstance()->addClassToCollection("InventoryItemComponent", "void removeQuantityFromInventory(String inventoryIdGameObject, int quantity, bool once)",
            "Decreases the quantity of this inventory item in the inventory game object. "
        ...
        LuaScriptApi::getInstance()->addClassToCollection("InventoryItemComponent", "void addQuantityToInventory2(String inventoryIdGameObject, String componentName, int quantity, bool once)",
            "Increases the quantity of this inventory item in the inventory game object. "
        ...
        LuaScriptApi::getInstance()->addClassToCollection("InventoryItemComponent", "void removeQuantityFromInventory2(String inventoryIdGameObject, String componentName, int quantity, bool once)",
            "Decreases the quantity of this inventory item in the inventory game object. "
        ...
        LuaScriptApi::getInstance()->addClassToCollection("InventoryItemComponent", "void setGameObjectId(String gameObjectId)",
            "Sets the id of the GameObject associated with this inventory item (e.g. a building template). "
            "Use '0' for none. This id is also configurable in NOWA-Design and is forwarded to reactOnMouseButtonClick on the MyGUIItemBoxComponent "
            "so the template can be resolved without hardcoded names: "
            "'local go = AppStateManager:getGameObjectController():getGameObjectFromId(inventoryItem:getGameObjectId())'");
        LuaScriptApi::getInstance()->addClassToCollection("InventoryItemComponent", "String getGameObjectId()", "Gets the id of the associated GameObject. Returns '0' if none has been set.");
