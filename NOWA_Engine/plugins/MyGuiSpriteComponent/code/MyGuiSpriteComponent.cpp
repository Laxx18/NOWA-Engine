...
namespace NOWA
{
    ...
    bool MyGuiSpriteComponent::postInit(void)
    {
        ...
        {
            this->widget = MyGUI::Gui::getInstancePtr()->createWidgetReal<MyGUI::ImageBox>("ImageBox", this->position->getVector2().x, this->position->getVector2().y, this->size->getVector2().x, this->size->getVector2().y,
        ...
        */
        ...
        if (true == this->inverseDirection->getBool())
        {
            ...
            if (this->currentFrame < startIndex)
            {
                ...
                {
                    this->finished = true;
                    // Notify listeners (e.g. MyGUIItemBoxComponent) that the animation has completed.
                    // 'this' is used as an opaque identity token — never dereferenced by listeners.
                    auto event = EventDataPtr(new EventDataSpriteAnimationFinished(this->gameObjectPtr->getId(), this));
                    AppStateManager::getSingletonPtr()->getEventManager()->triggerEvent(event);
                }
                else
                {
    ...
    {
        if (nullptr == this->widget || false == this->activated->getBool())
        {
            return;
        }
        ...
        int userRows = static_cast<int>(this->rowsCols->getVector2().x);
        int userCols = static_cast<int>(this->rowsCols->getVector2().y);

        if (0 == userRows || 0 == userCols)
        {
            ...
            const MyGUI::IntSize& imageSize = widget->castType<MyGUI::ImageBox>()->getImageSize();
            numCols = imageSize.width / tileSizeX;
            numRows = imageSize.height / tileSizeY;
        }
        else
        {
            numRows = userRows;
        ...
        if (nullptr != this->widget)
        {
            ...
            {
                MyGUI::IntCoord tileCoord(this->currentCol * static_cast<int>(this->tileSize->getVector2().x), this->currentRow * static_cast<int>(this->tileSize->getVector2().y), static_cast<int>(this->tileSize->getVector2().x),
                    static_cast<int>(this->tileSize->getVector2().y));
    ...
    void MyGuiSpriteComponent::setInverseDirection(bool inverseDirection)
    {
        ...
        if (true == inverseDirection)
        {
            ...
            {
                // BUG FIX 2: guard against null widget or image not yet loaded
                if (nullptr == this->widget)
                {
                    return;
                }
                ...
                {
                    return; // image not ready yet; currentFrame stays 0, advanceFrame will clamp it
                }
                ...
                this->currentFrame = imageSize.width / tileSizeX;
                this->currentFrame *= imageSize.height / tileSizeY;
                this->currentFrame -= 1;
            }
        }
        else
            ...
            if (this->mouseButtonClickClosureFunction.is_valid())
            {
                NOWA::AppStateManager::LogicCommand logicCommand = [this]()
                {
                    ...
                    catch (luabind::error& error)
                    {
                        ...
                        Ogre::LogManager::getSingleton().logMessage(Ogre::LML_CRITICAL, "[MyGuiSpriteComponent] Caught error in 'reactOnMouseButtonClick' Error: " + Ogre::String(error.what()) + " details: " + msg.str());
                    }
                };
    ...
    void MyGuiSpriteComponent::positionOverCell(const MyGUI::IntCoord& cellAbsoluteCoord)
    {
        ...
        const MyGUI::IntSize screen = MyGUI::RenderManager::getInstance().getViewSize();
        if (screen.width <= 0 || screen.height <= 0)
        {
            return;
        }

        const float realX = static_cast<float>(cellAbsoluteCoord.left) / static_cast<float>(screen.width);
        const float realY = static_cast<float>(cellAbsoluteCoord.top) / static_cast<float>(screen.height);
        const float realW = static_cast<float>(cellAbsoluteCoord.width) / static_cast<float>(screen.width);
        const float realH = static_cast<float>(cellAbsoluteCoord.height) / static_cast<float>(screen.height);

        this->widget->setRealPosition(realX, realY);
        this->widget->setRealSize(realW, realH);

        // Keep Variants in sync so the component stays consistent
        this->position->setValue(Ogre::Vector2(realX, realY));
        this->size->setValue(Ogre::Vector2(realW, realH));
    }
    ...
    void MyGuiSpriteComponent::createStaticApiForLua(lua_State* lua,luabind::class_<GameObject>& gameObjectClass,luabind::class_<GameObjectController>& gameObjectControllerClass)
    {
        ...
        LuaScriptApi::getInstance()->addClassToCollection("GameObject", "MyGuiSpriteComponent getMyGuiSpriteComponentFromIndex(unsigned int occurrenceIndex)",
