...
namespace NOWA
{
	...
	void KeyholeEffectComponent::update(Ogre::Real dt, bool notSimulating)
	{
		if (false == notSimulating)
		{
			...
			{
				this->tempRadius += dt;  // Increase by 20% per second
				// this->tempRadius += 0.2f * dt;  // Increase by 20% per second
			}
			else
			{
				this->tempRadius -= dt;  // Decrease by 20% per second
			}
			...
			if (nullptr != this->pass)
			{
				...
				// fragmentParams->setNamedConstant("center", this->center->getVector2());
				// fragmentParams->setNamedConstant("screenSize", Ogre::Vector2(mWindow->getWidth(), mWindow->getHeight()));
				// TODO: maker materialptr as pointer and set this constant once, as it does not change
				// fragmentParams->setNamedConstant("textureKeyhole", this->shapeTexture); // or texture_keyhole?
			}
		}
	}
	...
	void KeyholeEffectComponent::createStaticApiForLua(lua_State* lua, class_<GameObject>& gameObjectClass, class_<GameObjectController>& gameObjectControllerClass)
	{
		...
		gameObjectClass.def("getKeyholeEffectComponent2", (KeyholeEffectComponent * (*)(GameObject*, unsigned int)) & getKeyholeEffectComponent);

		LuaScriptApi::getInstance()->addClassToCollection("GameObject", "KeyholeEffectComponent getKeyholeEffectComponent2(unsigned int occurrenceIndex)", "Gets the component by the given occurence index, since a game object may this component maybe several times.");
