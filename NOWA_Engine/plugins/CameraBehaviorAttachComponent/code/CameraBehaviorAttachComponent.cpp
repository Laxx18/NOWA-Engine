...
namespace NOWA
{
    ...
    void CameraBehaviorAttachComponent::setActivated(bool activated)
    {
        if (true == activated && nullptr == this->baseCamera)
        {
            ...
            this->baseCamera = new AttachCamera(AppStateManager::getSingletonPtr()->getCameraManager()->getCameraBehaviorId(), this->gameObjectPtr->getSceneNode(), this->offsetPosition->getVector3(),
                MathHelper::getInstance()->degreesToQuat(this->offsetOrientation->getVector3()), this->smoothValue->getReal(), this->gameObjectPtr->getDefaultDirection());
        }
    ...
    void CameraBehaviorAttachComponent::createStaticApiForLua(lua_State* lua,luabind::class_<GameObject>& gameObjectClass,luabind::class_<GameObjectController>& gameObjectControllerClass)
    {
        ...
        LuaScriptApi::getInstance()->addClassToCollection("CameraBehaviorAttachComponent", "void setSmoothValue(float smoothValue)",
        ...
        LuaScriptApi::getInstance()->addClassToCollection("GameObject", "CameraBehaviorAttachComponent getCameraBehaviorAttachComponentFromIndex(unsigned int occurrenceIndex)",
        ...
        LuaScriptApi::getInstance()->addClassToCollection("GameObjectController", "CameraBehaviorAttachComponent castCameraBehaviorAttachComponent(CameraBehaviorAttachComponent other)",
