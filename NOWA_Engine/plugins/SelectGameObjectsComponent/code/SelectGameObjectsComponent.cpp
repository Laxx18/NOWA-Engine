...
namespace NOWA
{
	...
	bool SelectGameObjectsComponent::init(rapidxml::xml_node<>*& propertyElement, const Ogre::String& filename)
	{
	...
	void SelectGameObjectsComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc, const Ogre::String& filePath)
	{
		// 2 = int
		// 6 = real
		// 7 = string
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->useMultiSelection->getBool())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "12"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "UseSelectionRectangle"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->useSelectionRectangle->getBool())));
		propertiesXML->append_node(propertyXML);
	}
	...
	void SelectGameObjectsComponent::createStaticApiForLua(lua_State* lua, class_<GameObject>& gameObjectClass, class_<GameObjectController>& gameObjectControllerClass)
	{
		...
		LuaScriptApi::getInstance()->addClassToCollection("SpeechBubbleComponent", "void reactOnGameObjectsSelected(func closureFunction, table[GameObject])",
