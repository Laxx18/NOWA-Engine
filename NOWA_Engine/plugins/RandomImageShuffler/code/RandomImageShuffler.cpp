...
namespace NOWA
{
    ...
    bool RandomImageShuffler::connect(void)
    {
        ...
        if (true == this->activated->getBool())
        {
            ENQUEUE_RENDER_COMMAND("RandomImageShuffler::connect", {
                this->imageWidget = MyGUI::Gui::getInstancePtr()->createWidgetReal<MyGUI::ImageBox>("ImageBox",
                    MyGUI::FloatCoord(this->geometry->getVector4().x, this->geometry->getVector4().y, this->geometry->getVector4().z, this->geometry->getVector4().w), MyGUI::Align::HCenter, "Overlapped");

                this->initialGeometry = this->imageWidget->getAbsoluteRect();

                if (false == this->maskImage->getString().empty())
                {
                    this->maskImageWidget = MyGUI::Gui::getInstancePtr()->createWidgetReal<MyGUI::ImageBox>("ImageBox",
                        MyGUI::FloatCoord(this->geometry->getVector4().x, this->geometry->getVector4().y, this->geometry->getVector4().z, this->geometry->getVector4().w), MyGUI::Align::Stretch, "Overlapped");
                    ...
                    else
                    {
                        if (this->timeSinceLastImageShown >= this->gradientDelay * 0.001f)
                        {
                            ...
                            else
                            {
                                ...
                                if (nullptr != luaScript && false == this->onImageChosenFunctionName->getString().empty())
                                {
                                    // No MyGUI access here: the widget values were snapshotted by switchImage() /
                                    // slideImage() on the render thread. They used to be read INSIDE the logic
                                    // command, which touched MyGUI from the logic thread.
                                    const Ogre::String capturedFunctionName = this->onImageChosenFunctionName->getString();
                                    const Ogre::String capturedTextureName = this->currentTextureName;
                                    const Ogre::String capturedImageIndex = this->currentImageIndexString;

                                    boost::weak_ptr<GameObjectComponent> weakThis = this->shared_from_this();

                                    NOWA::AppStateManager::LogicCommand logicCommand = [this, weakThis, capturedFunctionName, capturedTextureName, capturedImageIndex]()
                                    {
                                        ...
                                        // callTableFunction dereferences the script itself, so this check has to stay.
                                        ...
                                        currentLuaScript->callTableFunction(capturedFunctionName, capturedTextureName, capturedImageIndex);
                                    };
    ...
    void RandomImageShuffler::switchImage(void)
    {
        ...
        if (nullptr == this->imageWidget || true == this->images.empty())
        {
            return;
        }
        ...
        // Runs on the render thread, so reading the widget here is allowed. The lua
        // callbacks read these copies instead of touching MyGUI from the logic thread.
        this->currentTextureName = this->imageWidget->_getTextureName();
        this->currentImageIndexString = Ogre::StringConverter::toString(index);
    }
    ...
    void RandomImageShuffler::slideImage(Ogre::Real dt)
    {
        ...
        if (this->scrollPositionX >= this->imageWidget->getImageSize().width * 2.0f)
        {
            ...
            // Same snapshot as in switchImage().
            this->currentTextureName = this->imageWidget->_getTextureName();
            this->currentImageIndexString = Ogre::StringConverter::toString(this->currentImageIndex);
        ...
        if (true == this->stopShuffle)
        {
            ...
            // Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_CRITICAL, "--->dist " + Ogre::StringConverter::toString(distanceToNextImage) + " this->scrollPositionX: " + Ogre::StringConverter::toString(this->scrollPositionX) + " width: " +
            // Ogre::StringConverter::toString(this->imageWidget->getImageSize().width));
    ...
    void RandomImageShuffler::createStaticApiForLua(lua_State* lua, luabind::class_<GameObject>& gameObjectClass, luabind::class_<GameObjectController>& gameObjectControllerClass)
    {
        module(lua) 
        [
        ...
        LuaScriptApi::getInstance()->addClassToCollection("GameObject", "RandomImageShuffler getRandomImageShuffler2(unsigned int occurrenceIndex)",
