...
{
	float3 sampleColor = prevFrame.SampleLevel( trilinearSampler, samplePos, mipChannel ).xyz;
...
{
	float4 result = mul( p_textureSpaceToViewSpace, float4( ssPosXY, zDepth, 1.0 ) );
	result.xyz /= result.w;
	return result.xyz;
}*/
...
) : SV_Target0
{
	...
	/*float3 nNormal = gBuf_normals.Load( int3( gl_FragCoord.xy , 0 ) ).xyz * 2.0 - 1.0;
