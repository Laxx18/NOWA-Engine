...
struct Params
{
	...
	float c_weights[@value( kernel_radius_plus1 )];
												...
												texture2d<@insertpiece(texture0_pf_type)> inputImage )
{
	...
	f2SamplePosition *= p.g_f4OutputSize.zw;
	///return inputImage.SampleLevel( inputSampler, f2SamplePosition, 0 ).x;
								 ...
								 texture2d<@insertpiece(uav0_pf_type), access::write> outputImage )
{
	...
	@property( !downscale_lq )
		@foreach( 4, iPixel )
			firstSmpl[ @iPixel ] = RDI[ @iPixel ];
			outColour[ @iPixel ] = p.c_weights[ @value( kernel_radius ) ];@end
	@end @property( downscale_lq )
		@foreach( 2, iPixel )
			firstSmpl[ @iPixel ] = RDI[ @iPixel * 2 ];
			outColour[ @iPixel ] = p.c_weights[ @value( kernel_radius ) ];@end
	@end
	...
	@foreach( kernel_radius, iIteration )
		@property( !downscale_lq )
			@foreach( 4, iPixel )
				outColour[ @iPixel ] += exp(@value(K)*(RDI[ @iPixel ] - firstSmpl[ @iPixel ])) * p.c_weights[ @iIteration ];@end
		@end @property( downscale_lq )
			@foreach( 2, iPixel )
				outColour[ @iPixel ] += exp(@value(K)*(RDI[ @iPixel * 2 ] - firstSmpl[ @iPixel ])) * p.c_weights[ @iIteration ];@end
		@end
		@foreach( 3, iPixel )
	...
	@foreach( kernel_radius2x_plus1, iIteration, kernel_radius_plus1 )
		@property( !downscale_lq )
			@foreach( 4, iPixel )
				outColour[ @iPixel ] += exp(@value(K)*(RDI[ @iPixel ] - firstSmpl[ @iPixel ])) * p.c_weights[ @value( kernel_radius2x ) - @iIteration ];@end
		@end @property( downscale_lq )
			@foreach( 2, iPixel )
				outColour[ @iPixel ] += exp(@value(K)*(RDI[ @iPixel * 2 ] - firstSmpl[ @iPixel ])) * p.c_weights[ @value( kernel_radius2x ) - @iIteration ];@end
		@end
		@foreach( 3, iPixel )
			RDI[ @iPixel ] = RDI[ @iPixel + ( 1 ) ];@end
		@foreach( 1, iPixel )
...
@property( horizontal_pass )
	...
	@foreach( samples_per_thread, i )
		g_f3LDS[ iLineOffset ][ iSampleOffset + @i ] = sampleTex( i2Coord + int2( @i, gl_LocalInvocationID.y ) , float2( 0.5f, 0.0f ),
																		  p, inputSampler, inputImage );@end
	...
	if( i2Coord.x < int(p.g_f4OutputSize.x) )
	{
...
@end @property( !horizontal_pass )
	...
	@foreach( samples_per_thread, i )
		g_f3LDS[ iLineOffset ][ iSampleOffset + @i ] = sampleTex( i2Coord + int2( gl_LocalInvocationID.y, @i ) , float2( 0.0f, 0.5f ),
																		  p, inputSampler, inputImage );@end
	...
	if( i2Coord.y < int(p.g_f4OutputSize.y) )
	{
