...
@piece( image_sample )
	return inputImage.SampleLevel( inputSampler, f2SamplePosition, srcLodIdx ).xyz;
@end
...
@piece( image_store )
	...
	@end @property( !downscale_lq )
		@foreach( 2, iPixel )
			outputImage[i2Center +  @iPixel * i2Inc] = float4( (outColour[ @iPixel * 2 ] +
																outColour[ @iPixel * 2 + 1 ]) * 0.5, 1.0 );@end
	@end
@end
