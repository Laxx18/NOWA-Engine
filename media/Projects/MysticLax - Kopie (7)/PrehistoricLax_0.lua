...
function attachWeapon()
    ...
    prehistoricLax:getLuaScriptComponent():callDelayedMethod(function()
        ...
        -- Note: Since this is called as delayed method, it can be, if the user ends the application in the middle of attaching, there are no ragdolling pieces
        rightUpperArm = physicsRagdollComponent:getRagBone("RightUpperArm");
        ...
        picker:setTargetId(prehistoricLax:getReferenceComponentFromName("RightHandWeapon"):getTargetId());
...
PrehistoricLax_0["connect"] = function(gameObject)
    ...
    prehistoricLax:getAreaOfInterestComponentFromName("MechanicsArea"):reactOnEnter(function(hitGameObject) 
        if (hitGameObject ~= nil) then
            ...
            if (hitGameObject:getTagName() == "Lever") then
                ...
                -- No more required! Is used the same way as Handle below
                if (hingeActuator ~= nil) then
            ...
            elseif (hitGameObject:getTagName() == "Handle") then
                ...
                -- Activate lever motion (maybe but only once)
                if (hitGameObject:getAttributesComponent() ~= nil) then
                    ...
                    if (firstTimeActivated:getValueBool() == false) then
                        do return end;
                    end
                    
                    hitGameObject:getAttributesComponent():getAttributeValueByName("FirstTimeActivated"):setValueBool(false);
                end
                ...
                handle:getAiPathFollowComponent():reactOnPathGoalReached(function()
                    ...
                    handle:getJointHingeComponent():getPredecessorJointComponent():reactOnTargetAngleReached(function(newAngle)
                        ...
                        -- Maybe activate other game object components if the mechanics is finished
                        local referenceId = hitGameObject:getReferenceId();
                        AppStateManager:getGameObjectController():activateGameObjectComponentsFromReferenceId(referenceId, true);
    ...
    if (EventType.PlayerDeadEvent ~= nil) then
        AppStateManager:getScriptEventManager():registerEventListener(EventType.PlayerDeadEvent, PrehistoricLax_0["onPlayerDead"]);
    end
...
PrehistoricLax_0["onPlayerDead"] = function(eventData)
...
RagDollState["execute"] = function(gameObject, dt)
    ...
    if (ragDollTime >= 4) then
        if (direction == LEFT) then
            playerController:getPhysicsComponent():applyRequiredForceForVelocity(Vector3(4, 0, 0));
...
AttackState["exit"] = function(gameObject)
    log("enter: Finished AttackState");
