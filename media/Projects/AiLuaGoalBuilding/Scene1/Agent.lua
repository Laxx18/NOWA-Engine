...
Player["connect"] = function(gameObject)
    ...
    selectGameObjectsComponent:reactOnGameObjectsSelected(function(selectedGameObjects)
        ...
        local townhallSelected = false

        for index, currentGameObject in pairs(selectedGameObjects) do
            currentGameObject =AppStateManager:getGameObjectController():castGameObject(currentGameObject);
            if (currentGameObject:getTagName() == "Worker") then
                workerSelected = true
                break
            elseif (currentGameObject:getTagName() == "Townhall") then
                townhallSelected = true;
            end
        end

        if (workerSelected) then
            itemBoxComponent:setActivated(true)
            statusText:setCaption("Left-click an item in the inventory to place a building.")
        elseif (townhallSelected) then
           itemBoxComponent:setActivated(true)
            statusText:setCaption("Left-click an item in the inventory to Create a new worker.")
        else
    ...
    gameObjectPlaceComponent:reactOnGameObjectPlaced(function(newGameObjectId)
        ...
        clonedGO = AppStateManager:getGameObjectController():castGameObject(clonedGo);
        if clonedGO ~= nil then
            buildPosition = clonedGO:getPosition()
            -- Activates all components
            clonedGO:setActivated(true);
            
            if (clonedGO:getTagName() == "Townhall") then
                local townhallItemBox = clonedGO:getMyGUIItemBoxComponent();
                
                townhallItemBox:reactOnMouseButtonClick(function(resourceName, gameObjectId, mouseButtonId)
                    local workerGameObject = AppStateManager:getGameObjectController():getGameObjectFromId(gameObjectId)
                    workerGameObject = AppStateManager:getGameObjectController():getGameObjectFromId(workerGameObject);
                    if workerGameObject == nil then
                         statusText:setCaption("Worker GameObject id " .. gameObjectId .. " not found in scene.");
                         log("[Player] ERROR: worker id " .. gameObjectId .. " not found.");
                         return;
                     end
                     
                     local targetPosition = clonedGO:getPosition() + Vector3(1, 0, 1);
                     AppStateManager:getGameObjectController():clone(workerGameObject:getId(), nil, gameObjectId, targetPosition, Quaternion.IDENTITY, Scale.UNIT_SCALE);
                 end)
