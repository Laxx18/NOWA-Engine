...
PrehistoricLax_0["connect"] = function(gameObject)
	 ...
	 playerWeapon:getPhysicsActiveComponent():setPosition(rightHand:getPosition());
		...
		if (hitGameObject ~= nil) then
			...
			if matches(hitGameObject:getName(), "Lever*") then
				...
				if (hingeActuator ~= nil and firstTimeActivated:getValueBool() == true) then
					...
					animationBlender:blendAndContinue(AnimationBlender.ANIM_ACTION_1);
					-- Pulling the lever should only be possible one time!
					firstTimeActivated:setValueBool(false);
					-- React when animation is finished
					playerController:reactOnAnimationFinished(prehistoricLax, "ActionFinished", hitGameObject, true);
			...
			elseif matches(hitGameObject:getName(), "Item*") then
				...
				if (inventoryItem ~= nil) then
					animationBlender:blendAndContinue(AnimationBlender.ANIM_PICKUP_1);
					playerController:reactOnAnimationFinished(prehistoricLax, "PickupFinished", hitGameObject, true);
					playerController:lockMovement("mechanics", true);
				end
			...
			animationBlender:blendAndContinue(AnimationBlender.ANIM_NO_IDEA);
		end
	end);
...
-- Attention: Here maybe check which category, if something is weird in game
	...
	if ((playerController:getMoveWeight() == 0) or (inputDeviceComp:isActionDown(NOWA_A_UP) == false and inputDeviceComp:isActionDown(NOWA_A_DOWN) == false
...
-- Attention: Due to rotation of 90 degree at once, commented this out
	...
	if (inputDeviceComp:isActionDown(NOWA_A_LEFT) or inputDeviceComp:isActionDown(NOWA_A_RIGHT) and atObstacle == false) then
		...
		if (animationBlender:isAnimationActive(animId) == false
			...
			and inAir == false and jumpKeyPressed == false and isJumping == false and playerController:getMoveWeight() == 1 and inputDeviceComp:isActionDown(NOWA_A_UP) == false) then
...
---------------------------------------------------------------------------------------------------
	...
	if (inAir == false and playerController:getPhysicsComponent():getVelocity().y < -2 and onTrampolin == false and onSlider == false and isJumping == false) then
		...
		-- playerController:getAnimationBlender()->getSource()->setTimePosition(playerController:getAnimationBlender()->getSource()->getLength() - 0.1f);
...
PrehistoricLax_0["ActionFinished"] = function(hitGameObject)
...
PrehistoricLax_0["PickupFinished"] = function(hitGameObject)

	local inventoryItem = hitGameObject:getInventoryItemComponent();
	if (inventoryItem ~= nil) then
		inventoryItem:addQuantityToInventory(MAIN_GAMEOBJECT_ID, 1);
		AppStateManager:getGameObjectController():deleteGameObjectWithUndo(hitGameObject:getId());
	end
	playerController:lockMovement("mechanics", false);
