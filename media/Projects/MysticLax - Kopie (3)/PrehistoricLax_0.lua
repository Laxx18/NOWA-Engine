...
PrehistoricLax_0["connect"] = function(gameObject)
	...
	prehistoricLax:getAreaOfInterestComponentFromName("MechanicsArea"):reactOnEnter(function(hitGameObject) 
        if (oldMechanicsGameObject ~= nil) then
            log("--->Deactivate mechanics"); 
        ...
        elseif (hitGameObject ~= nil) then
            ...
            elseif (hitGameObject:getTagName() == "Handle") then
				-- Remembers the old game object, in order to check next time, if it exists to end the mechanics approach
                oldMechanicsGameObject = hitGameObject;
                log("--->Activate Handle!"); 
                --playerWeapon:getJointHingeComponent():releaseJoint(false)
                --rightLowerArmJoint:setPredecessorId(pointToPoint:getId());
                --playerWeapon:getJointHingeComponent():setPredecessorId(pointToPoint:getId());
                --local targetPosition = hitGameObject:getJointHingeComponent():getUpdatedJointPosition() - Vector3(rightLowerArm:getSize().x * 0.5, 0, 0);
                local targetPosition = hitGameObject:getJointHingeComponent():getUpdatedJointPosition();
                --rightHand:getJointKinematicComponent():setTargetPosition(targetPosition);
                playerWeapon:getJointKinematicComponent():setTargetPosition(targetPosition);
                playerWeapon:getJointKinematicComponent():setTargetRotation(Quaternion(Degree(-90), Vector3.UNIT_Z));
...
PortalState["enter"] = function(gameObject)
	...
	prehistoricLax:getAiPathFollowComponent():reactOnPathGoalReached(function()
