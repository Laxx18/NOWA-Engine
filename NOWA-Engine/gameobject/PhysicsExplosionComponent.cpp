...
namespace NOWA
{
	...
	bool PhysicsExplosionComponent::init(rapidxml::xml_node<>*& propertyElement, const Ogre::String& filename)
	{
...
// Check that
	...
	void PhysicsExplosionComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc, const Ogre::String& filePath)
	{
		// 2 = int
		// 6 = real
		// 7 = string
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->countDown->getReal())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->strength->getUInt())));
		propertiesXML->append_node(propertyXML);
	}
