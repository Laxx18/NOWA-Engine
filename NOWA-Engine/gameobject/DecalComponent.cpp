...
namespace NOWA
{
	...
	bool DecalComponent::init(rapidxml::xml_node<>*& propertyElement, const Ogre::String& filename)
	{
...
// Attention: size of pictures must be the same, check it! and reject with error message box in mygui
	...
	void DecalComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc, const Ogre::String& filePath)
	{
		// 2 = int
		// 6 = real
		// 7 = string
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->ignoreAlpha->getBool())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->rectSize->getVector3())));
		propertiesXML->append_node(propertyXML);
	}
