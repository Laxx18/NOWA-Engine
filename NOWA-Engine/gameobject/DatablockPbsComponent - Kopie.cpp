...
namespace NOWA
{
	...
	bool DatablockPbsComponent::readDatablockEntity(Ogre::v1::Entity* entity)
	{
		...
		{
			this->datablock = dynamic_cast<Ogre::HlmsPbsDatablock*>(this->originalDatablock->clone(originalDataBlockName
	...
	bool DatablockPbsComponent::readDatablockItem(Ogre::Item* item)
	{
		...
		if (this->subEntityIndex->getUInt() >= static_cast<unsigned int>(item->getNumSubItems()))
		{
			this->datablock = nullptr;
			Ogre::String message = "[DatablockPbsComponent] Datablock reading failed, because there is no such sub entity index: "
				+ Ogre::StringConverter::toString(this->subEntityIndex->getUInt()) + " for game object: "
		...
		// Get the cloned data block, so that it can be manipulated individually
	    // Get it from game object, because the correct datablock is stored there. (Ogre somehow does store Missing for planes, hence use the name from game object)
		Ogre::String dataBlock = this->gameObjectPtr->getAttribute(GameObject::AttrDataBlock() + Ogre::StringConverter::toString(this->subEntityIndex->getUInt()))->getString();
		...
		if ("Missing" == originalDataBlockName)
		{
			// originalDataBlockName = this->gameObjectPtr->getDatablockNames()[this->subEntityIndex->getUInt()];
	...
	void DatablockPbsComponent::writeXML(xml_node<>* propertiesXML, xml_document<>& doc, const Ogre::String& filePath)
	{
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->indexOfRefraction->getVector4())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "RefractionStrength"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->refractionStrength->getReal())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->alphaTest->getListSelectedValue())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->normalMapWeight->getReal())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "ClearCoat"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->clearCoat->getReal())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "6"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "ClearCoatRoughness"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->clearCoatRoughness->getReal())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->reflectionTextureName->getListSelectedValue())));
		propertiesXML->append_node(propertyXML);
		...
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->useEmissiveAsLightMap->getBool())));
		propertiesXML->append_node(propertyXML);

		propertyXML = doc.allocate_node(node_element, "property");
		propertyXML->append_attribute(doc.allocate_attribute("type", "7"));
		propertyXML->append_attribute(doc.allocate_attribute("name", "ShadowConstBias"));
		propertyXML->append_attribute(doc.allocate_attribute("data", XMLConverter::ConvertString(this->shadowConstBias->getReal())));
		propertiesXML->append_node(propertyXML);
	...
	{
		if (textureName == attribute->getString())
		{
			return;
		}
	...
	{
		if ("skipCreation" == this->gameObjectPtr->getCustomDataString())
		{
			return;
		}
	...
	{
		if (workflow == this->workflow->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(metalness, this->metalness->getReal()))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(roughness, this->roughness->getReal()))
		{
			return;
		}
		if (roughness < 0.001f)
	...
	{
		if (MathHelper::getInstance()->vector3Equals(fresnel, Ogre::Vector3(this->fresnel->getVector4().x, this->fresnel->getVector4().y, this->fresnel->getVector4().z)) 
			&& this->fresnel->getVector4().w == static_cast<Ogre::Real>(separateFresnel))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->vector3Equals(refractionIdx, Ogre::Vector3(this->indexOfRefraction->getVector4().x, this->indexOfRefraction->getVector4().y, this->indexOfRefraction->getVector4().z)) 
			&& this->indexOfRefraction->getVector4().w == static_cast<Ogre::Real>(separateFresnel))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(refractionStrength, this->refractionStrength->getReal()))
		{
			return;
		}
	...
	{
		if (brdf == this->brdf->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (twoSided == this->twoSidedLighting->getBool())
		{
			return;
		}
	...
	{
		if (cullingMode == this->oneSidedShadowCastCullingMode->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (alphaTest == this->alphaTest->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(threshold, this->alphaTestThreshold->getReal()))
		{
			return;
		}
	...
	{
		if (receiveShadows == this->receiveShadows->getBool())
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->vector3Equals(diffuseColor, this->diffuseColor->getVector3()))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->vector4Equals(backgroundColor, this->backgroundColor->getVector4()))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->vector3Equals(specularColor, this->specularColor->getVector3()))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->vector3Equals(emissiveColor, this->emissiveColor->getVector3()))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(normalMapWeight, this->normalMapWeight->getReal()))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(clearCoat, this->clearCoat->getReal()))
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(clearCoatRoughness, this->clearCoatRoughness->getReal()))
		{
			return;
		}
...
// Attention: There is no detail type anymore, what type is correct?
	...
	{
		if (blendMode0 == this->blendMode0->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (blendMode1 == this->blendMode1->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (blendMode2 == this->blendMode2->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (blendMode3 == this->blendMode3->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (transparencyMode == this->transparencyMode->getListSelectedValue())
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(transparency, this->transparency->getReal()))
		{
			return;
		}
	...
	{
		if (useAlphaFromTextures == this->useAlphaFromTextures->getBool())
		{
			return;
		}
	...
	{
		if (uvScale == this->uvScale->getUInt())
		{
			return;
		}
	...
	{
		if (MathHelper::getInstance()->realEquals(shadowConstBias, this->shadowConstBias->getReal()))
		{
			return;
		}
	...
	{
		if (useEmissiveAsLightMap == this->useEmissiveAsLightMap->getBool())
		{
			return;
		}
