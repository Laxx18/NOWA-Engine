...
namespace NOWA
{
	...
	{
	private:
		struct SDragDropTempData
		{
			SDragDropTempData(wraps::DDItemInfo& info)
				: info(info)
			{

			}

			NOWA::ItemData* receiverData = nullptr;
			wraps::DDItemInfo& info;
			NOWA::ItemData* senderData = nullptr;
			bool result = false;
			unsigned int maxBuyableQuantity = 0;
		};
	public:
	...
	private:
		...
		void endRequestDrop(wraps::DDItemInfo& _info, NOWA::ItemData* receiverData, NOWA::ItemData* senderData, bool& _result, unsigned int chosenQuantity);
		...
		void notifyScrollChangePosition(MyGUI::ScrollBar* sender, size_t position);
	private:
		int getIndexFromResourceName(const Ogre::String& resourceName);

		void internalShowBuyAmountDialog(wraps::DDItemInfo& _info, NOWA::ItemData* receiverData, NOWA::ItemData* senderData, bool& result, unsigned int maxBuyableQuantity);
	private:
		ToolTip* toolTip;
		...
		bool dropFinished;
		MyGUI::Window* buyAmountDialog;
		MyGUI::VectorWidgetPtr buyAmountWidgets;
	};
