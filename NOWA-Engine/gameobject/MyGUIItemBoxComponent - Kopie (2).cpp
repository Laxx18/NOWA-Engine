...
namespace NOWA
{
		...
		buyAmountDialog(nullptr)
	{
		...
		this->mouseClickEventName->setDescription("Sets the mouse click event name, because several MyGUI Components can have an event in the same lua script, so name is necessary. E.g. onInventory1MouseClick(resourceName, mouseButtonId)");
	}
	...
	MyGUIItemBoxComponent::~MyGUIItemBoxComponent()
	{
		...
		if (nullptr != this->buyAmountDialog)
		{
			auto okButton = MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::Button>("okButton");
			okButton->eventMouseButtonClick -= MyGUI::newDelegate(this, &MyGUIItemBoxComponent::buttonHit);
			MyGUI::Gui::getInstancePtr()->destroyWidget(this->buyAmountDialog);
			this->buyAmountDialog = nullptr;
			MyGUI::LayoutManager::getInstancePtr()->unloadLayout(this->buyAmountWidgets);
		}
	}
	...
	GameObjectCompPtr MyGUIItemBoxComponent::clone(GameObjectPtr clonedGameObjectPtr)
	{
		...
		clonedCompPtr->setMouseClickEventName(this->mouseClickEventName->getString());
		clonedCompPtr->setMouseEnterEventName(this->mouseEnterEventName->getString());
		clonedCompPtr->setMouseLeaveEventName(this->mouseLeaveEventName->getString());
	...
	void MyGUIItemBoxComponent::notifyRequestDrop(wraps::BaseLayout* _sender, wraps::DDItemInfo _info, bool& _result)
	{
		...
		if (senderData->getQuantity() > 1)
		{
			dynamic_cast<MyGUI::DDContainer*>(this->itemBoxWindow->getItemBox())->setStartDrop(false);
			this->internalShowBuyAmountDialog(_info, receiverData, senderData, _result, senderData->getQuantity());
			this->endRequestDrop(_info, receiverData, senderData, _result, 0);
			
			return;
		}

		this->endRequestDrop(_info, receiverData, senderData, _result, 1);
	}

	void MyGUIItemBoxComponent::endRequestDrop(wraps::DDItemInfo& _info, NOWA::ItemData* receiverData, NOWA::ItemData* senderData, bool& _result, unsigned int chosenQuantity)
	{
		if (0 == chosenQuantity)
		{
			_result = false;
			...
			if (receiverData->getInventoryOwnerId() != this->gameObjectPtr->getId())
			{
				...
				if (nullptr != myGuiItemBoxCompPtr)
				{
					if (myGuiItemBoxCompPtr->closureFunction.is_valid())
					{
						try
						{
							this->dragDropData->quantity = chosenQuantity;
	...
	void MyGUIItemBoxComponent::buttonHit(MyGUI::Widget* sender)
	{
		MyGUI::InputManager::getInstancePtr()->removeWidgetModal(this->buyAmountDialog);
		this->buyAmountDialog->setVisible(false);

		unsigned int chosenQuantity = Ogre::StringConverter::parseUnsignedInt(MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::EditBox>("amountEdit")->getOnlyText());

		MyGUIItemBoxComponent::SDragDropTempData* dragDropTempData = sender->getUserData<MyGUIItemBoxComponent::SDragDropTempData>(false);

		this->endRequestDrop(dragDropTempData->info, dragDropTempData->receiverData, dragDropTempData->senderData, dragDropTempData->result, chosenQuantity);
	}

	void MyGUIItemBoxComponent::internalShowBuyAmountDialog(wraps::DDItemInfo& _info, NOWA::ItemData* receiverData, NOWA::ItemData* senderData, bool& result, unsigned int maxBuyableQuantity)
	{
		if (0 == this->buyAmountWidgets.size())
		{
			this->buyAmountWidgets = MyGUI::LayoutManager::getInstancePtr()->loadLayout("BuyAmountWindow.layout");
			auto okButton = MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::Button>("okButton");

			okButton->eventMouseButtonClick += MyGUI::newDelegate(this, &MyGUIItemBoxComponent::buttonHit);

			MyGUIItemBoxComponent::SDragDropTempData dragDropTempData(_info);
			dragDropTempData.receiverData = receiverData;
			dragDropTempData.senderData = senderData;
			dragDropTempData.maxBuyableQuantity = maxBuyableQuantity;
			dragDropTempData.result = result;

			okButton->setUserData(MyGUI::Any(dragDropTempData));
		}
		...
		if (nullptr == this->buyAmountDialog)
		{
			...
			this->buyAmountDialog = MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::Window>("buyAmountWindow");
			this->buyAmountDialog->setRealPosition(windowPosition);
			this->buyAmountDialog->setVisible(true);

			MyGUI::ScrollBar* slider = MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::ScrollBar>("amountSlider");
			slider->setMoveToClick(true);
			// Scroll range must be always one more (see internal implementation of scroll bar)
			slider->setScrollRange(maxBuyableQuantity);
			slider->setScrollPosition(1);
			slider->eventScrollChangePosition += MyGUI::newDelegate(this, &MyGUIItemBoxComponent::notifyScrollChangePosition);

			MyGUI::EditBox* edit = MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::EditBox>("amountEdit");
			edit->setOnlyText("1");
		}

		MyGUI::InputManager::getInstancePtr()->addWidgetModal(this->buyAmountDialog);
	}
		...
		if (_result && false == this->dropFinished)
		{
			...
			if (receiverData->getInventoryOwnerId() != this->gameObjectPtr->getId())
			{
				...
				if (nullptr != myGuiItemBoxCompPtr)
				{
					...
					myGuiItemBoxCompPtr->increaseQuantity(_info.receiver_index, this->dragDropData->quantity);
			...
			{
				this->quantities[_info.sender_index]->setValue(currentQuantity - this->dragDropData->quantity);
			}
			else
			{
			...
			if (receiverData->getInventoryOwnerId() != this->gameObjectPtr->getId())
			{
				...
				if (nullptr != myGuiItemBoxCompPtr)
				{
					// The other game objects inventory
					myGuiItemBoxCompPtr->setResourceName(_info.receiver_index, senderData->getResourceName());
					myGuiItemBoxCompPtr->increaseQuantity(_info.receiver_index, this->dragDropData->quantity);
					myGuiItemBoxCompPtr->setSellValue(_info.receiver_index, senderData->getSellValue());
					myGuiItemBoxCompPtr->setBuyValue(_info.receiver_index, senderData->getBuyValue());
			...
			{
				// The same inventory itself (Reorganizing own inventory)
				this->setResourceName(_info.receiver_index, senderData->getResourceName());
				this->increaseQuantity(_info.receiver_index, senderData->getQuantity());
				this->setSellValue(_info.receiver_index, senderData->getSellValue());
				this->setBuyValue(_info.receiver_index, senderData->getBuyValue());
			...
			senderData->removeQuantity(this->dragDropData->quantity);
	...
	void MyGUIItemBoxComponent::notifyNotifyItem(wraps::BaseLayout* _sender, const MyGUI::IBNotifyItemData& _info)
	{
		if (_info.index != MyGUI::ITEM_NONE)
		{
			if (_info.notify == MyGUI::IBNotifyItemData::NotifyItem::MousePressed)
			{
				...
				if (nullptr != this->gameObjectPtr->getLuaScript() && false == this->mouseClickEventName->getString().empty())
				{
					this->gameObjectPtr->getLuaScript()->callTableFunction(this->mouseClickEventName->getString(), resourceName, buttonId);
				}
			}
		}
	}
	...
	void MyGUIItemBoxComponent::notifyScrollChangePosition(MyGUI::ScrollBar* sender, size_t position)
	{
		MyGUI::EditBox* edit = MyGUI::Gui::getInstancePtr()->findWidget<MyGUI::EditBox>("amountEdit");
