...
protected:
    ...
    static const Ogre::String GLOW_MAP_PATTERN;
    static const Ogre::String CUBE_MAP_PATTERN;
    ...
    {
        PassProperties() : isDeferred(true), cubeMap(0),normalMap(0),specularMap(0),glowMap(0), isSkinned(false) {}

        bool isDeferred;
        Ogre::vector<Ogre::TextureUnitState*>::type regularTextures;
        Ogre::TextureUnitState* normalMap;
        Ogre::TextureUnitState* specularMap;
        Ogre::TextureUnitState* glowMap;
        Ogre::TextureUnitState* cubeMap;
        bool isSkinned;
    ...
    //Check if a texture is a specualr map, and fill property sheet accordingly
    bool checkSpecularMap(Ogre::TextureUnitState* tus, PassProperties& props);
    //Check if a texture is a glow map, and fill property sheet accordingly
    bool checkGlowMap(Ogre::TextureUnitState* tus, PassProperties& props);
    //Check if a texture is a glow map, and fill property sheet accordingly
    bool checkCubeMap(Ogre::TextureUnitState* tus, PassProperties& props);
};
