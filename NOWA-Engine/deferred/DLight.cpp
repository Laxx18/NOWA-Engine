...
#include "OgreLight.h"
#include "deferred/GeomUtils.h"
#include "deferred/LightMaterialGenerator.h"
...
DLight::DLight(MaterialGenerator *sys, Ogre::Light* parentLight) :
	mParentLight(parentLight), bIgnoreWorld(false), mGenerator(sys), mPermutation(0), handled(false)
{
	// Set up geometry
...
//-----------------------------------------------------------------------
MaterialPtr DLight::getMaterial(void)
{
	Ogre::MaterialPtr material = Ogre::MaterialPtr((mGenerator->getMaterial(mPermutation)));
	if (mParentLight->getType() == Light::LT_SPOTLIGHT && mPermutation & LightMaterialGenerator::MI_GOBO_LIGHT)
	{
		Ogre::MaterialPtr matc = MaterialManager::getSingleton().getByName(("testgobo_" + mParentLight->getName()));
		...
		if (matc.get() == NULL)
		{
			...
			matc = material.get()->clone("testgobo_" + mParentLight->getName());
			*matc.get()->getTechnique(0)->getPass(0)->getTextureUnitState("GoboMap") = *mGobo.get()->getTechnique(0)->getPass(0)->getTextureUnitState(0);
...
void DLight::updateFromParent()
{
	...
	Ogre::Any any = mParentLight->getUserObjectBindings().getUserAny("Gobo");

	if (!any.isEmpty())
	{
		mGobo = MaterialManager::getSingleton().getByName(*(any_cast<Ogre::String *>(any)));
		if ((mGobo.get() != NULL) && (mGobo.get()->getTechnique(0)->getPass(0)->getNumTextureUnitStates() != 0))
		{
			ENABLE_BIT(mPermutation, LightMaterialGenerator::MI_GOBO_LIGHT);
	...
	else
	{
		...
		DISABLE_BIT(mPermutation, LightMaterialGenerator::MI_GOBO_LIGHT);
	}
	if (getCastChadows())
	{
...
bool DLight::getCastChadows() const
{
	return
		...
		(mParentLight->getType() == Light::LT_DIRECTIONAL || mParentLight->getType() == Light::LT_SPOTLIGHT || mParentLight->getType() == Light::LT_POINT);
}
//-----------------------------------------------------------------------
void DLight::updateFromCamera(Ogre::Camera* camera)
{
	//Set shader params
	const Ogre::MaterialPtr& mat = getMaterial();
	Ogre::LogManager::getSingletonPtr()->logMessage(Ogre::LML_TRIVIAL, "------>DLight updateFromCamera MaterialName " + mat->getName());
	if (!mat->isLoaded())
	{
		mat->load();
	}
