...
void PropertiesPanel::clearProperties(void)
{
    ...
    NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "PropertiesPanel::clearProperties");
}
...
void PropertiesPanel::showProperties(std::vector<NOWA::GameObject*> gameObjects, unsigned int componentIndex)
{
    ...
    // ---- Build all component panels synchronously, right here, right now ----
    // (Reverted incremental/multi-frame batching - it introduced a real
    // use-after-free race between showProperties() re-entry and the tracked
    // render-thread closure still touching a stale this->propertiesPanelInfo.
    // A properties panel rebuild is not a per-frame hot path; doing it in one
    // shot is simpler and correct.)
    ...
    if (true == componentsAreSame)
    {
        ...
        for (unsigned int j = 0; j < count; j++)
        {
            ...
            Ogre::String componentName = thisComponent->getClassName();
            ...
            PropertiesPanelComponent* componentPropertiesPanel = new PropertiesPanelComponent(gameObjects, gameObjectComponents, componentName + " (" + Ogre::StringConverter::toString(thisComponent->getOccurrenceIndex()) + ")");
            ...
            // Add the properties
            auto compAttributes = thisComponent->getAttributes();
            for (auto it = compAttributes.begin(); it != compAttributes.end(); ++it)
            {
                const auto found2 = sameValues.find(it->first);
    ...
    this->propertiesPanelInfo->listData(gameObjects[0]);
...
{
    NOWA::GraphicsModule::RenderCommand renderCommand = [this, info]()
    {
        this->propertyInfo->setOnlyText(info);
    };
    NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "PropertiesPanelInfo::setInfo");
}
...
void PropertiesPanelDynamic::initialise()
{
    ...
    NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "PropertiesPanelDynamic::initialise");
}
...
{
    NOWA::GraphicsModule::RenderCommand renderCommand = [this, count]()
    {
        const int heightStep = 28;
        int heightCurrent = 0;
...
void PropertiesPanelDynamic::requestDrawItem(MyGUI::ItemBox* sender, MyGUI::Widget* item, const MyGUI::IBDrawItemInfo& info)
{
    ...
    NOWA::GraphicsModule::RenderCommand renderCommand = [this, info, sender, data, resourceName]()
    {
        if (info.update)
        {
    ...
    NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "PropertiesPanelDynamic::requestDrawItem");
}
...
void PropertiesPanelDynamic::notifyColourCancel(MyGUI::ColourPanel* sender)
{
    ...
    NOWA::GraphicsModule::getInstance()->enqueue(std::move(renderCommand), "PropertiesPanelDynamic::notifyColourCancel");
}
...
void PropertiesPanelGameObject::notifyColourAccept(MyGUI::ColourPanel* sender)
{
    ...
    NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "PropertiesPanelGameObject::notifyColourAccept");
}
...
void PropertiesPanelComponent::notifyColourAccept(MyGUI::ColourPanel* sender)
{
    ...
    NOWA::GraphicsModule::getInstance()->enqueueAndWait(std::move(renderCommand), "PropertiesPanelComponent::notifyColourAccept");
}
